...
t_e_H_L_opts() { # {{{
   ...
   ${MAILX} ${ARGS} -S on-folder-open=fh-test -X 'define fh-test {
...
t_newmail_on_folder() { # {{{
   ...
   ${MAILX} ${ARGS} -Y '#
      set noautosort noshowto \
         on-folder-open=ofo on-folder-newmail=ofn \
         on-folder-open-+t1x.mbox=ofo-z on-folder-newmail-+t1x.mbox=ofn-z \
         on-folder-open-+t1y.mbox=ofo-z on-folder-newmail-+t1y.mbox=ofn-z
      define ofo {
...
PASS Sway
' &&
      pop3_logged_in; } | ../net-test t.sh > ./t1 2>&1
   check 1 0 ./t1 '3754674759 160'
...
' &&
         pop3_logged_in; } | ../net-test t.sh > ./t2 2>&1
      check 2 0 ./t2 '3754674759 160'
   else
      t_echoskip '2:[!MD5]'
   fi

   if have_feat tls; then
      t__net_script t.sh pop3s \
         -Spop3-auth=xoauth2 -Spop3-use-starttls
      { printf '\002
+OK Dovecot ready. <314.1.5d6ad59f.Rq8miBAdE0uUT/0GGKg2bA==@arch-2019>
\001
AUTH XOAUTH2 dXNlcj1zdGVmZmVuAWF1dGg9QmVhcmVyIFN3YXkBAQ==
' &&
         pop3_logged_in; } | ../net-test -S t.sh > ./t3 2>&1
      check 3 0 ./t3 '3754674759 160'
...
AUTH XOAUTH2 dXNlcj1zdGVmZmVuAWF1dGg9QmVhcmVyIFN3YXkBAQ==
' &&
         pop3_logged_in; } | ../net-test -s t.sh > ./t4 2>&1
      check 4 0 ./t4 '3754674759 160'
   else
      t_echoskip '{3,4}:[!TLS]'
   fi
   # }}}
...
' \
     ...
     t__net_script t.sh imaps -Simap-auth=external \
         -Stls-config-pairs=Certificate=client-pair.pem -vv
...
steffen
' &&
         imap_logged_in; } | ../net-test -v -S -U t.sh > ./t6 2>&1
      check 6 0 ./t6 '4233548649 160'
...
' &&
      imap_logged_in; } | ../net-test t.sh > ./t1 2>&1
   check 1 0 ./t1 '4233548649 160'
...
' &&
         imap_logged_in; } | ../net-test t.sh > ./t2 2>&1
      check 2 0 ./t2 '4233548649 160'
   else
      t_echoskip '2:[!MD5]'
   fi

   if have_feat tls; then
      t__net_script t.sh imaps \
         -Simap-auth=xoauth2 -Simap-use-starttls
      { imap_hello && printf '\001
...
' &&
         imap_logged_in 3; } | ../net-test -s t.sh > ./t4 2>&1
      check 4 0 ./t4 '4233548649 160'

      t__net_script t.sh imap \
         -Simap-auth=external -Simap-use-starttls \
         -Stls-config-pairs=Certificate=client-pair.pem
      { { imap_hello_tls && printf '\001
T3 AUTHENTICATE EXTERNAL steffen
' &&
         imap_logged_in 3; } | ../net-test -s -U t.sh; } > ./t5 2>&1
      check 5 0 ./t5 '4233548649 160'
...
steffen
' &&
         imap_logged_in; } | ../net-test -S -U t.sh > ./t6 2>&1
      check 6 0 ./t6 '4233548649 160'

      t__net_script t.sh imaps -Simap-auth=externanon \
         -Stls-config-pairs=Certificate=client-pair.pem
      { { imap_hello && printf '\001
T2 AUTHENTICATE EXTERNAL =
' &&
         imap_logged_in; } | ../net-test -S -U t.sh; } > ./t7 2>&1
      check 7 0 ./t7 '4233548649 160'

      t__net_script t.sh imap \
         -Simap-auth=oauthbearer -Simap-use-starttls
      { imap_hello_tls && printf '\001
T3 AUTHENTICATE OAUTHBEARER bixhPXN0ZWZmZW4sAWhvc3Q9bG9jYWxob3N0AXBvcnQ9NTAwMDABYXV0aD1CZWFyZXIgU3dheQEB
' &&
         imap_logged_in 3; } | ../net-test -s ./t.sh > ./t8 2>&1
      check 8 0 ./t8 '4233548649 160'
