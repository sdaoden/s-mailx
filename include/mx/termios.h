...
/*   mx_TERMIOS_CMD_SET_PGRP = 1u,*/
	 ...
	 * An environment carries the terminal mode as well as a possibly installed on_state_change hook; if such
	 * environment is reentered, the state change hook gets called to resume after the mode has been reestablished.
	 * Likewise, if it is left, it gets called to suspend first.
	 * XXX Any state change requires this, only RAW and RAW_TIMEOUT may be switched back and forth on the same
	 * XXX level (otherwise state_change needs cmd argument, plus plus plus) */
...
enum mx_termios_state_change{
	 ...
	 * For HANDS_OFF handlers this will be called in a RESET even in already suspended state, so no _STATE_SUSPEND
	 * is set, then! */
 ...
 * Return value indicates whether level shall be CMD_POPped: it is only honoured if TERMIOS_STATE_SUSPEND and
 * TERMIOS_STATE_SIGNAL are both set.
 * TODO This "to-pop" return will vanish in v15, we only need it due to longjmp and of course it sucks since how many
 * TODO levels does the jump cross?  We do not know except when installing setjmps on each and every level, but we do
 * TODO not; it is ok for this MUA today, but a real generic solution in v15 will simply not care for signal jumps at
 * TODO all, they suck more */
...
/* For long iterative output, like `list', tabulator-completion, etc., determine the screen width that should be used */
#define mx_TERMIOS_WIDTH_OF_LISTS() (mx_termios_dimen.tiosd_width - (mx_termios_dimen.tiosd_width >> 3))

/* Installs signal handlers etc.  Early! */
EXPORT void mx_termios_controller_setup(enum mx_termios_setup what);

/* Install a state change hook for the current environment, which will receive cookie as its user argument.
 * May not be used in and for the top level */
EXPORT void mx_termios_on_state_change_set(mx_termios_on_state_change hdl, up cookie);

/* tiosc is a bitmix of mx_termios_cmd values.
 * For _RAW and _RAW_TIMEOUT a1 describes VMIN and VTIME, respectively, for SET_PGRP it is the PID */
