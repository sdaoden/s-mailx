...
/* Call _ENTER_SWITCH() with the according flags, it will take care for the rest and also set the jump buffer -
 * it returns 0 if anything went fine and a signal number if a jump occurred, in which case all handlers requested in
 * flags are temporarily SIG_IGN.
 * _cleanup_ping() informs the condome that no jumps etc. shall be performed until _leave() is called in the following
 * -- to be (optionally) called right before the local jump label is reached which is jumped to after a long jump
 * occurred, straight code flow provided, e.g., to avoid destructors to be called twice.
 * _leave() must always be called last, reraise_flags will be used to decide how signal handling has to continue */
