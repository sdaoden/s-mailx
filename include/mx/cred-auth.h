...
enum{
	...
	mx_CRED_AUTHTYPE_MECH_AUTO_MASK = mx_CRED_AUTHTYPE_CRAM_MD5 | mx_CRED_AUTHTYPE_LOGIN | mx_CRED_AUTHTYPE_PLAIN,
 ...
 * It will call protocol specific _config() function if possible: the callee should not use URL except for using it to
 * lookup variable chains.
 * Return whether credentials are available and valid (for chosen auth) */
EXPORT boole mx_cred_auth_lookup(struct mx_cred_ctx *credp, struct mx_url *urlp);

/* Find a (usr config, else official) type name; return it or NIL, or -1 if name is "allmechs" (only with usr config).
 * (Recall the EXTERNAL/EXTERNANON problem, as above.)
 * The latter finds a type, and returns it or NIL (unsupported) */
EXPORT struct mx_cred_authtype_info const *mx_cred_auth_type_find_name(char const *name, boole usr);
EXPORT struct mx_cred_authtype_info const *mx_cred_auth_type_find_type(u32 type);

/* Verify all cred_authtype .cavc_mechplusbits for the given .cavc_proto, with bells and whistles.
 * maybe_tls is an indication whether the transport could eventually be secured -- that "whether" should solely be based
 * upon the protocol configuration, not to an actual URL.
 * Return whether any usable mechanism remains in .cavc_mechbits.
 * Returns TRUM1 if the list includes all possible mechanisms. */
EXPORT boole mx_cred_auth_type_verify_bits(struct mx_cred_authtype_verify_ctx *cavcp, boole maybe_tls);
 ...
 * mechplusbits should have been adjusted already to be compatible with the server offerings.
 * assume_tls states whether the connection is secured, or not.
 * Returns the "best" match, or U32_MAX upon error aka no authentication mechanism can be used */
EXPORT u32 mx_cred_auth_type_select(enum cproto proto, u32 mechplusbits, boole assume_tls);
