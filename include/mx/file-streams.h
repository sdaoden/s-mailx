 ...
 * count - maximum characters to read.  May be NIL.  TODO is uz, must be s64!
...
EXPORT char *fgetline(char **line, uz *linesize, uz *count, uz *llen_or_nil, FILE *fp, int appendnl  su_DVL_LOC_ARGS_DECL);
#ifdef su_HAVE_DVL_LOC_ARGS
# define fgetline(A,B,C,D,E,F) fgetline(A, B, C, D, E, F  su_DVL_LOC_ARGS_INJ)
#endif
 ...
 * n is the number of characters already read and present in *linebuf; it will be treated as _the_ line if no more
 * (successful) reads are possible.
 * Manages the n_PS_READLINE_NL hack */
EXPORT int readline_restart(FILE *ibuf, char **linebuf, uz *linesize, uz n  su_DVL_LOC_ARGS_DECL);
#ifdef su_HAVE_DVL_LOC_ARGS
# define readline_restart(A,B,C,D) readline_restart(A, B, C, D  su_DVL_LOC_ARGS_INJ)
#endif
