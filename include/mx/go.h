...
# define mx_COLOUR_IS_ACTIVE() \
	(/*mx_go_data->gc_data.gdc_colour != NIL &&*/ /*mx_go_data->gc_data.gdc_colour->ce_enabled*/\
...
/* Returns true if force_eof() has been set -- it is set automatically if an input context enters EOF state (rather
 * than error, as in ferror(3)) */
 ...
 * reproducibility.
 * If histok_or_nil is set it will be updated to FAL0 if input shall not be placed in history.
...
/* The name or identifier of the current go context, as well as the callee of the current go context, or NIL if unknown
 * or target context neither of macro or file */
 ...
 * pop() returns FAL0 if gccp is not the stack top; the cleanup call will happen at a later time, then */
