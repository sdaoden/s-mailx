...
EXPORT struct mx_name *mx__name_parse(char const *line, BITENUM(u32,gfield) ntype, boole not_a_list);
...
INLINE struct mx_name *mx_name_parse(char const *line, BITENUM(u32,gfield) ntype){
   return mx__name_parse(line, ntype, FAL0);
}

/* Interprets entire line as one address: identical to name_parse() but only returns one (or none) name (or NIL) */
INLINE struct mx_name *mx_name_parse_as_one(char const *line, BITENUM(u32,gfield) ntype){
   return mx__name_parse(line, ntype, TRU1);
}
