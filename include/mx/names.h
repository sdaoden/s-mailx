...
/* Duplicate np */
EXPORT struct mx_name *n_namelist_dup(struct mx_name const *np, enum gfield ntype);
...
/* Like extract() unless line contains anyof ",\"\\(<|", then comma-separated list extraction is used instead */
EXPORT struct mx_name *lextract(char const *line, enum gfield ntype);

/* Interprets entire line as one address: identical to extract() and lextract() but only returns one (or none) name.
...
EXPORT struct mx_name *grab_names(u32/*mx_go_input_flags*/ gif, char const *field, struct mx_name *np, int comma,
		enum gfield gflags);
...
/* Check all addresses in np and delete invalid ones; if set_on_error is not NIL it will be set to TRU1 for error or -1
 * for "hard fail" error */
EXPORT struct mx_name *checkaddrs(struct mx_name *np, enum expand_addr_check_mode eacm, s8 *set_on_error);
