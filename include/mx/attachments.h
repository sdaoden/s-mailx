...
EXPORT struct mx_attachment *mx_attachments_remove(struct mx_attachment *aplist, struct mx_attachment *ap);

/* Find by file-name.  If any path component exists in name then an exact match of the creation-path is used directly;
 * if instead the basename of that path matches all attachments are traversed to find an exact match first, the first
 * of all basename matches is returned as a last resort; If no path component exists the filename= parameter is
 * searched (and also returned) in preference over the basename, otherwise likewise.  If name is in fact a message
 * number the first match is taken.  If stat_or_null is given: FAL0 on NIL return, TRU1 for exact/single match, TRUM1
