...
/*! Is equal? */
INLINE boole su_timespec_is_EQ(struct su_timespec const *self, struct su_timespec const *tp){
	ASSERT_RET(self != NIL, (tp == NIL));
...
/*! Is not equal? */
INLINE boole su_timespec_is_NE(struct su_timespec const *self, struct su_timespec const *tp){
	ASSERT_RET(self != NIL, (tp != NIL));
	ASSERT_RET(tp != NIL, FAL0);
	return (self->ts_sec != tp->ts_sec || self->ts_nano != tp->ts_nano);
}
...
/*! Is less-than? */
INLINE boole su_timespec_is_LT(struct su_timespec const *self, struct su_timespec const *tp){
	s64 x;
	ASSERT_RET(self != NIL, (tp != NIL));
	ASSERT_RET(tp != NIL, FAL0);
	x = self->ts_sec - tp->ts_sec;
...
INLINE boole su_timespec_is_LE(struct su_timespec const *self, struct su_timespec const *tp){
	s64 x;
	ASSERT_RET(self != NIL, (tp != NIL));
	ASSERT_RET(tp != NIL, FAL0);
	x = self->ts_sec - tp->ts_sec;
...
INLINE boole su_timespec_is_GE(struct su_timespec const *self, struct su_timespec const *tp){
	s64 x;
	ASSERT_RET(self != NIL, (tp == NIL));
...
/*! Is greater-than? */
INLINE boole su_timespec_is_GT(struct su_timespec const *self, struct su_timespec const *tp){
	s64 x;
	ASSERT_RET(self != NIL, FAL0);
	ASSERT_RET(tp != NIL, TRU1);
	x = self->ts_sec - tp->ts_sec;
...
/*! Add \a{tp} to \SELF.
 * (Does not prevent integer wraparounds.) */
INLINE struct su_timespec *su_timespec_add(struct su_timespec *self, struct su_timespec const *tp){
	s64 sec;
	sz nano;
 ...
 * (Does not prevent integer wraparounds.) */
INLINE struct su_timespec *su_timespec_sub(struct su_timespec *self, struct su_timespec const *tp){
	s64 sec;
	sz nano;
	ASSERT_RET(self != NIL, self);
	ASSERT_RET(tp != NIL, self);
...
EXPORT_DATA char const su_time_weekday_names_abbrev[su_TIME_WEEKDAY_SATURDAY +1][su_TIME_WEEKDAY_NAMES_ABBREV_LEN +1]; /*!< \_ */

#define su_TIME_MONTH_NAMES_ABBREV_LEN 3u /*!< \_ */
EXPORT_DATA char const su_time_month_names_abbrev[su_TIME_MONTH_DECEMBER +1][su_TIME_MONTH_NAMES_ABBREV_LEN +1]; /*!< \_ */
...
EXPORT s64 su_time_gregor_to_epoch(u32 y, u32 m, u32 d, u32 hour, u32 min, u32 sec);
 ...
 * if available.} */
EXPORT s32 su_time_nsleep(struct su_timespec const *dur, struct su_timespec *rem_or_nil);
/* }}} */
/*! @} *//* }}} */
