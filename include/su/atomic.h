 ...
 *
 * An atomic operation is an operation which is executed by one thread at a time only, no matter how many CPUs exist,
 * and how many threads try to perform the operation concurrently.
 * Atomic operations cannot be implemented in and through the C programming language, but only on the hardware level.
 * Due to this detailed hardware (and compiler) knowledge is necessary to be able to provide truly atomic operations.
 * Newer C standards also support atomic operations optionally: the functions found here should be expected to be of a
 * \c{memory_order_seq_cst} kind, in standard terms.
 *
 * Due to these peculiarities the availability macro \r{su_ATOMIC_IS_REAL} exists, and involved storage must use the
 * \r{su_ATOMIC} qualifier.
 * Atomicity is ensured via a global mutex if no backing implementation exists.
 * Without \r{su_HAVE_MT} these act, non-atomically.
 *
 * \remarks{In general \SU expects that assigning a value to an integer type smaller than or equal to \r{su_uz}
 * (\r{su_UZ_BITS}) that has been qualified as \r{su_ATOMIC} is atomic in that either the the storage contains the old
 ...
 * This means either there exists a special implementation, or at \SU compile time there was ISO C11 (ISO9899:2011)
 * available (and lack of \c{__STDC_NO_ATOMICS__} announced availability). */

#if defined su_ATOMIC_IS_REAL || defined su_HAVE_MT
# define a_X(X,Y) EXPORT X CONCAT(su__atomic_g_xchg_,Y)(X ATOMIC *store, X nval);
a_X(u8,8) a_X(u16,16) a_X(u32,32) a_X(u64,64) a_X(uz,z) a_X(up,p)

# undef a_X
# define a_X(X,Y) EXPORT X CONCAT(su__atomic_g_busy_xchg_,Y)(X ATOMIC *store, X nval);
a_X(u8,8) a_X(u16,16) a_X(u32,32) a_X(u64,64) a_X(uz,z) a_X(up,p)

# undef a_X
# define a_X(X,Y) EXPORT boole CONCAT(su__atomic_g_cas_,Y)(X ATOMIC *store, X oval, X nval);
a_X(u8,8) a_X(u16,16) a_X(u32,32) a_X(u64,64) a_X(uz,z) a_X(up,p)

# undef a_X
# define a_X(X,Y) EXPORT void CONCAT(su__atomic_g_busy_cas_,Y)(X ATOMIC *store, X oval, X nval);
a_X(u8,8) a_X(u16,16) a_X(u32,32) a_X(u64,64) a_X(uz,z) a_X(up,p)
...
# define a_X(X,Y) \
	INLINE X CONCAT(su__atomic_xchg_,Y)(X ATOMIC *store, X nval){return CONCAT(su__atomic_g_xchg_,Y)(store, nval);}
a_X(u8,8) a_X(u16,16) a_X(u32,32) a_X(u64,64) a_X(uz,z) a_X(up,p)
