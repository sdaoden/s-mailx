 ...
 * SipHash was designed in 2012 by Jean-Philippe Aumasson (\xln{https://aumasson.jp}) and
 * Daniel J. Bernstein (\xln{http://cr.yp.to}); see \xln{https://github.com/veorq/SipHash.git}.
...
EXPORT void su_siphash_update(struct su_siphash *self, void const *dat, uz dat_len);

/*! Calculate the digest, and place it in \a{digest_store}, which must be large enough for the chosen digest size. */
EXPORT void su_siphash_end(struct su_siphash *self, void *digest_store);

/*! The plain algorithm from setup to finalization,
 * with \a{crounds} (2 if 0) compression and \a{drounds} (4 if 0) finalization rounds.
 * \a{digest_store} must be large enough to hold the chosen digest size.
 * \a{key} must be \r{su_SIPHASH_KEY_SIZE} bytes.
 * \a{dat} can be \NIL if \a{dat_len} is 0. */
EXPORT void su_siphash_once(void *digest_store, enum su_siphash_digest digest_type, void const *key,
...
public:
	...
	static void once(void *digest_store, void const *key, void const *dat, uz dat_len,
		...
		su_siphash_once(digest_store, S(su_siphash_digest,digest_type), key, dat, dat_len, crounds, drounds);
	}
};
/* }}} */
