 ...
 *
 * Copyright (c) 2001 - 2026 Steffen Nurpmeso <steffen@sdaoden.eu>.
...
EXPORT uz su__cs_first_x_of_cbuf_cbuf(boole x, char const *cp, uz cplen, char const *xp, uz xplen);

/*! This actually tests for 7-bit cleanliness. */
INLINE boole su_cs_is_ascii(s32 x) {return (S(u32,x) <= S8_MAX);}

#undef a_X
#define a_X(X,F) return (su_cs_is_ascii(X) && (su__cs_ctype[S(u32,X)] & su_CONCAT(su_CS_CTYPE_,F)) != 0)
 ...
 * This should be considered an attackable hash, for now Chris Torek's hash algorithm is used,
 * the resulting hash is stirred as shown by Bret Mulvey.
...
/*! Hash a string (buffer), case-insensitively, otherwise identical to \r{su_cs_hash_cbuf()}.
...
/*! Strong hash creators (like \r{su_cs_hash_strong_cbuf()}) that are (more) proof against algorithmic complexity
 * attacks on hashes use a random-seeded built-in \r{MD} template that is created once needed first.
 * By giving a non-\NIL \a{tp} one can instead set the template explicitly: \a{tp} must be fully setup in order to
 * be usable for copy-assignment, and the object backing \a{tp} must remain accessible.
 *
...
/*! Hash a string (buffer) with a strong algorithm, see \r{su_cs_hash_strong_setup()} for the complete picture,
 * and \r{su_cs_hash_strong_case_cbuf()} for case-insensitivity.
 * \remarks{Only available with \r{su_HAVE_MD}.}
 * \remarks{May abort the program unless initialization was successfully asserted via \r{su_cs_hash_strong_setup()}.} */
...
/*! Search \a{x} within \a{cp}, starting at end, return pointer to location or \NIL. */
EXPORT char *su_cs_rfind_c(char const *cp, char x);

/*! Find the next \a{sep}arator in *\a{iolist}, terminate the resulting substring and return it.
 * \r{su_cs_is_space()} surrounding the result will be trimmed away.
 * If \a{ignore_empty} is set, empty results will be skipped over.
 * \a{iolist} will be updated for the next round, \NIL will be placed if the input string is exhausted.
...
/*! Like \r{su_cs_sep_c()}, but supports escaping of \a{sep}arators via reverse solidus characters.
 * \remarks{Whereas reverse solidus characters are supposed to escape the next character, including reverse solidus
...
INLINE s32 su_cs_to_lower(s32 x) {return (S(u32,x) <= S8_MAX ? su__cs_tolower[x] : x);}

/*! Uppercasing variant of \r{su_cs_to_lower()}. */
INLINE s32 su_cs_to_upper(s32 x) {return (S(u32,x) <= S8_MAX ? su__cs_toupper[x] : x);}
/*! @} *//* }}} */
...
public:
	/*! \cd{su_cs_ctype} */
	enum ctype{
		ctype_none = su_CS_CTYPE_NONE, /*!< \cd{su_CS_CTYPE_NONE} */
		ctype_alnum = su_CS_CTYPE_ALNUM, /*!< \cd{su_CS_CTYPE_ALNUM} */
		ctype_alpha = su_CS_CTYPE_ALPHA, /*!< \cd{su_CS_CTYPE_ALPHA} */
		ctype_blank = su_CS_CTYPE_BLANK, /*!< \cd{su_CS_CTYPE_BLANK} */
		ctype_cntrl = su_CS_CTYPE_CNTRL, /*!< \cd{su_CS_CTYPE_CNTRL} */
		ctype_digit = su_CS_CTYPE_DIGIT, /*!< \cd{su_CS_CTYPE_DIGIT} */
		ctype_graph = su_CS_CTYPE_GRAPH, /*!< \cd{su_CS_CTYPE_GRAPH} */
		ctype_lower = su_CS_CTYPE_LOWER, /*!< \cd{su_CS_CTYPE_LOWER} */
		ctype_print = su_CS_CTYPE_PRINT, /*!< \cd{su_CS_CTYPE_PRINT} */
		ctype_punct = su_CS_CTYPE_PUNCT, /*!< \cd{su_CS_CTYPE_PUNCT} */
		ctype_space = su_CS_CTYPE_SPACE, /*!< \cd{su_CS_CTYPE_SPACE} */
		ctype_upper = su_CS_CTYPE_UPPER, /*!< \cd{su_CS_CTYPE_UPPER} */
		ctype_white = su_CS_CTYPE_WHITE, /*!< \cd{su_CS_CTYPE_WHITE} */
		ctype_xdigit = su_CS_CTYPE_XDIGIT /*!< \cd{su_CS_CTYPE_XDIGIT} */
	};
	...
	/*! \cd{su_cs_toolbox_case} */
	static NSPC(su)type_toolbox<char*> const * const type_toolbox_case;
	/*! \cd{su_cs_toolbox_case} */
	static NSPC(su)type_toolbox<char const*> const * const const_type_toolbox_case;
	...
	/*! \cd{su_cs_cmp_n()} */
	static sz cmp(char const *cp1, char const *cp2, uz n) {return su_cs_cmp_n(cp1, cp2, n);}

	/*! \cd{su_cs_cmp_case()} */
	static sz cmp_case(char const *cp1, char const *cp2) {return su_cs_cmp_case(cp1, cp2);}

	/*! \cd{su_cs_cmp_case_n()} */
	static sz cmp_case(char const *cp1, char const *cp2, uz n) {return su_cs_cmp_case_n(cp1, cp2, n);}

	/*! \cd{su_cs_copy_n()} */
	static char *copy(char *dst, char const *src, uz n) {return su_cs_copy_n(dst, src, n);}

	/*! \cd{su_cs_dup_cbuf()} */
	static char *dup(char const *buf, uz len, u32 estate=state::none) {return su_cs_dup_cbuf(buf, len, estate);}

	/*! \cd{su_cs_dup()} */
	static char *dup(char const *cp, u32 estate=state::none) {return su_cs_dup(cp, estate);}

	/*! \cd{su_cs_ends_with_case()} */
	static boole ends_with_case(char const *cp, char const *x) {return su_cs_ends_with_case(cp, x);}
	...
	/*! \cd{su_cs_hash_case_cbuf()} */
	static uz hash_case(char const *buf, uz len) {return su_cs_hash_case_cbuf(buf, len);}

	/*! \cd{su_cs_hash_case()} */
	static uz hash_case(char const *cp) {return su_cs_hash_case(cp);}

#ifdef su_HAVE_MD
	/*! \cd{su_cs_hash_strong_setup()} */
	...
	/*! \cd{su_cs_hash_strong_cbuf()} */
	static uz hash_strong(char const *buf, uz len) {return su_cs_hash_strong_cbuf(buf, len);}

	/*! \cd{su_cs_hash_strong()} */
	static uz hash_strong(char const *cp) {return su_cs_hash_strong(cp);}

	/*! \cd{su_cs_hash_strong_case_cbuf()} */
	static uz hash_strong_case(char const *buf, uz len) {return su_cs_hash_strong_case_cbuf(buf, len);}

	/*! \cd{su_cs_hash_strong_case()} */
	static uz hash_strong_case(char const *cp) {return su_cs_hash_strong_case(cp);}
#endif /* su_HAVE_MD */
	...
	/*! \cd{su_cs_pcopy()} */
	static char *pcopy(char *dst, char const *src) {return su_cs_pcopy(dst, src);}

	/*! \cd{su_cs_pcopy_n()} */
	static char *pcopy(char *dst, char const *src, uz n) {return su_cs_pcopy_n(dst, src, n);}

	/*! \cd{su_cs_rfind_c()} */
	static char *rfind(char const *cp, char x) {return su_cs_rfind_c(cp, x);}

	/*! \cd{su_cs_sep_c()} */
	static char *sep(char **iolist, char sep, boole ignore_empty) {return su_cs_sep_c(iolist, sep, ignore_empty);}

	/*! \cd{su_cs_sep_escable_c()} */
	...
	/*! \cd{su_cs_starts_with()} */
	static boole starts_with(char const *cp, char const *x) {return su_cs_starts_with(cp, x);}

	/*! \cd{su_cs_starts_with_n()} */
	static boole starts_with(char const *cp, char const *x, uz n) {return su_cs_starts_with_n(cp, x, n);}

	/*! \cd{su_cs_starts_with_case()} */
	static boole starts_with_case(char const *cp, char const *x) {return su_cs_starts_with_case(cp, x);}

	/*! \cd{su_cs_starts_with_case_n()} */
...
public:
	/*! \_ */
	static type_toolbox<char*> const *get_instance(void) { return cs::type_toolbox;}
};
...
public:
	/*! \_ */
	static type_toolbox<char const*> const *get_instance(void) {return cs::const_type_toolbox;}
};
