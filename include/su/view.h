...
protected:
	view__base &assign(view__base const &t) {m_view = t.m_view; return *this;}
	view__base &operator=(view__base const &t) {return assign(t);}

public:
	boole is_setup(void) const {return m_view.is_setup();}

	boole is_same_parent(view__base const &t) const {return m_view.is_same_parent(t.m_view);}

	boole is_valid(void) const {return m_view.is_valid();}
	operator boole(void) const {return is_valid();}

protected:
	view__base &reset(void) {(void)m_view.reset(); return *this;}
...
// To be (hopefully..) safe use a C-style cast xxx replaced ~20 years later
...
\
	...
	~su__VIEW_NAME(void) {}\
\
	su__VIEW_NAME &assign(su__VIEW_NAME const &t) {return S(myself&,base::assign(t));}\
...
#define su__VIEW_IMPL_CONST /*{{{*/\
	...
	explicit su__VIEW_NAME(su__VIEW_NAME_NONCONST<VIEWTRAITS,GBASEVIEWT> &t) : base(t) {}\
	explicit su__VIEW_NAME(su__VIEW_NAME_NONCONST<VIEWTRAITS,GBASEVIEWT> const &t) : base(t) {}\
\
	su__VIEW_NAME &assign(su__VIEW_NAME_NONCONST<VIEWTRAITS,GBASEVIEWT> const &t){\
		return S(myself&,base::assign(t));\
	}\
	su__VIEW_NAME &operator=(su__VIEW_NAME_NONCONST<VIEWTRAITS,GBASEVIEWT> const &t) {return assign(t);}\
\
	/* (We need to cast away the 'const', but it is preserved logically..) */\
...
#define su__VIEW_IMPL_NONASSOC_NONCONST /*{{{*/\
	...
	s32 insert(base &startpos, base const &endpos){\
		...
		return m_view.insert_range(su__VIEW_DOWNCAST(startpos).m_view, su__VIEW_DOWNCAST(endpos).m_view);\
	}\
\
