...
#define su__MUTEX_ARGS_DECL su_DVLDBG(su_COMMA char const *file su_COMMA u32 line)
...
# define su_MUTEX_FLAT_I9R(NAME_OR_NIL) su__MUTEX_I9R(su_MUTEX_FLAT, NAME_OR_NIL)
#endif /* !DOXYGEN */

/*! Uses \r{su_DVLDBGOR()} to select the according mutex lock interface. */
#define su_MUTEX_LOCK(SELF) su_DVLDBGOR(su_mutex_lock_dbg,su_mutex_lock)(SELF su__MUTEX_ARGS_INJ)

/*! Uses \r{su_DVLDBGOR()} to select the according mutex lock interface. */
#define su_MUTEX_LOCK_TS(SELF,TSP) su_DVLDBGOR(su_mutex_lock_ts_dbg,su_mutex_lock_ts)(SELF, TSP su__MUTEX_ARGS_INJ)

/*! Uses \r{su_DVLDBGOR()} to select the according mutex trylock interface. */
#define su_MUTEX_TRYLOCK(SELF) su_DVLDBGOR(su_mutex_trylock_dbg,su_mutex_trylock)(SELF su__MUTEX_ARGS_INJ)

/*! Uses \r{su_DVLDBGOR()} to select the according mutex trylock interface. */
#define su_MUTEX_TRYLOCK_TS(SELF,TSP) su_DVLDBGOR(su_mutex_trylock_ts_dbg,su_mutex_trylock_ts)(SELF, TSP su__MUTEX_ARGS_INJ)

/*! Uses \r{su_DVLDBGOR()} to select the according mutex unlock interface. */
#define su_MUTEX_UNLOCK(SELF) su_DVLDBGOR(su_mutex_unlock_dbg,su_mutex_unlock)(SELF su__MUTEX_ARGS_INJ)

/* */
#if DVLDBGOR(1, 0)
EXPORT boole su__mutex_check(struct su_mutex *self, enum su__mutex_xfn mf, struct su_thread *tsp, char const *file, u32 line);
#endif
...
EXPORT void su__mutex_lock(struct su_mutex *self, struct su_thread *tsp  su__MUTEX_ARGS_DECL);
EXPORT boole su__mutex_trylock(struct su_mutex *self, struct su_thread *tsp  su__MUTEX_ARGS_DECL);
EXPORT void su__mutex_unlock(struct su_mutex *self  su__MUTEX_ARGS_DECL);
#endif

/*! Creates a recursive mutex.
 * \ESTATE_RV. */
INLINE s32 su_mutex_create(struct su_mutex *self, char const *name_or_nil, u32 estate){
	s32 rv;
	ASSERT(self);
	UNUSED(estate);
...
INLINE void su_mutex_lock_ts_dbg(struct su_mutex *self, struct su_thread *tsp, char const *file, u32 line){
	ASSERT(self);
...
INLINE void su_mutex_lock_dbg(struct su_mutex *self, char const *file, u32 line){
	ASSERT(self);
	su_mutex_lock_ts_dbg(self, su_thread_self(), file, line);
}

/*! \_ */
INLINE void su_mutex_lock(struct su_mutex *self) {su_mutex_lock_dbg(self, __FILE__, __LINE__);}

/*! Debug variant of \r{su_mutex_trylock_ts()}. */
INLINE boole su_mutex_trylock_ts_dbg(struct su_mutex *self, struct su_thread *tsp, char const *file, u32 line){
	ASSERT(self);
	ASSERT_EXEC(tsp == su_thread_self(), tsp = su_thread_self());
	UNUSED(file);
	UNUSED(line);
...
/*! Try to lock mutex, but do not wait if it cannot be taken immediately, return whether it could be taken.
 * \remarks{\a{tsp} must be \r{su_thread_self()}.} */
...
INLINE boole su_mutex_trylock_dbg(struct su_mutex *self, char const *file, u32 line){
	ASSERT(self);
	return su_mutex_trylock_ts_dbg(self, su_thread_self(), file, line);
}

/*! Debug variant of \r{su_mutex_unlock()}. */
INLINE void su_mutex_unlock_dbg(struct su_mutex *self, char const *file, u32 line){
	ASSERT(self);
	UNUSED(file);
	UNUSED(line);
