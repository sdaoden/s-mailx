...
enum su_random_type{
    ...
    * Periodically seed can be mixed, adjust \r{su_random::rm_seed_mix_after}
    * accordingly. */
   su_RANDOM_TYPE_SP,
...
struct su_random{
    ...
    * \r{su_random_vsp_install()}ing, automatic seed mixing after generating
 ...
 * seeder object.
 * The internal built-in seeder is of type \r{su_RANDOM_TYPE_SP}, but with
 * enabled seed mixing; The built-in seeder itself is not hookable.
 *
 * This function may block for some time dependent upon the used seeding (and
 * the state of the seeder object).
 * This function may fail when seeding \r{su_RANDOM_TYPE_VSP} with
 * a \r{su_random_vsp_install()}ed hook.
 * (The hook is only called if the cookie is not yet set for \SELF, that is to
 * say that a hooked random generator is not assumed to need seeding.) */
 ...
 * and for the reasons shown for \r{su_random_seed()} that is called if \SELF
 * is not seeded yet, or needs seed mixing to satisfy the request.
...
public:
   ...
   /*! \copydoc{su_random::rm_seed_mix_after} */
   u32 seed_mix_after(void) const {return rm_seed_mix_after;}

   /*! \copydoc{su_random::rm_seed_mix_after} */
   random &set_seed_mix_after(u32 count){
      ASSERT_RET(type() == type_sp || type() == type_vsp, *this);
      rm_seed_mix_after = count;
      return *this;
   }
