...
enum su_random_type{
    ...
    * Returns mixed output bytes of a permutating ARC4 pool.
    * Uses simplified seeding:
    * this type generates reproducible sequences of random numbers. */
   su_RANDOM_TYPE_R,
   /*! Pseudo random number generator (fast).
    * Returns mixed output bytes of a permutating ARC4 pool.
    * Uses simplified seeding, but ARC4 pool access offsets and an initial
    * permutation count are randomized. */
   su_RANDOM_TYPE_P,
   /*! Strong pseudo (slower).
    * Returns message digests of the mixed output of a permutating ARC4 pool
    * that was \r{su_random_seed()}ed by the seeder object.
    * Periodic reseeding can be enabled via \r{su_random::rm_reseed_after}.
    * Currently \r{MD_SIPHASH}-2-4 is used, with only key set
    * (per round 16 bytes input for up to 16 bytes output). */
   su_RANDOM_TYPE_SP,
   /*! Very strong pseudo (even slower).
    * Returns message digests of the mixed output of a permutating ARC4 pool
    * that was \r{su_random_seed()}ed by the seeder object.
    * Periodic reseeding is by default enabled.
    * Currently \r{MD_SIPHASH}-4-8 is used, with key and key-sized data set
    * (per round 32 bytes input for up to 16 bytes output).
    *
 ...
 * seeder object.
 * The \r{su_RANDOM_TYPE_R} type is (re)set to its initial state, ignores
 * \a{with_or_nil}, and the function always succeeds.
 * For other types it may block for some time dependent upon the used seeding
 * and the state of the (given) seeder object.
...
/*! Like \r{su_random_seed()}, but applies to internal objects, either the
 * \a{seeder}, or the random generator.
...
public:
   ...
   static boole builtin_seed(boole seeder=FAL0){
      return su_random_builtin_seed(seeder);
   }
};
/* }}} */
