...
/* NetBSD usr.bin/ftp/ruserpass.c uses 100 bytes, we need four concurrently (dummy, host, user, pass), make it a KB */
#define a_NETRC_TOKEN_MAXLEN (1024u / 4)

/* Dictionary stores a_netrc_entry, not owned */
#define a_NETRC_FLAGS (su_CS_DICT_CASE | su_CS_DICT_HEAD_RESORT | su_CS_DICT_ERR_PASS)
#define a_NETRC_THRESHOLD 3
...
a_netrc_create(void){
	...
	a_netrc_dp = su_cs_dict_set_threshold(su_cs_dict_create(&a_netrc__d, a_NETRC_FLAGS, NIL), a_NETRC_THRESHOLD);
...
jm_h:
		...
		*machine = '\0';
		if(!(f & a_SEEN_DEFAULT) && (t = a_netrc__token(fi, machine, &nl_last)) != a_NETRC_INPUT)
			goto jenotinput;
...
jenotinput:
		...
		if(!(f & a_SEEN_DEFAULT) && (f & (a_LOGIN | a_PASSWORD))){
			...
			nrcep = su_ALLOC(VSTRUCT_SIZEOF(struct a_netrc_entry,nrce_dat) + llen +1 + plen +1);
			if(nrcep == NIL)
				goto jerrdoc;
			nrcep->nrce_next = NIL;
			...
			if(f & a_PASSWORD)
				su_mem_copy(&nrcep->nrce_dat[nrcep->nrce_password_idx = llen], password, ++plen);
			else
...
a_netrc_gut(boole gut_dp){
	...
	if(a_netrc_dp != NIL){
		...
		su_CS_DICT_FOREACH(a_netrc_dp, &csdv){
			...
			for(nrcep = S(struct a_netrc_entry*,su_cs_dict_view_data(&csdv)); nrcep != NIL;){
				tmp = nrcep;
...
a_netrc_dump(char const *cmdname, char const *key, void const *dat){
	...
	for(nrcep = S(struct a_netrc_entry const*,dat); nrcep != NIL; nrcep = nrcep->nrce_next){
		...
		if(nrcep->nrce_password_idx != U32_MAX){
			...
			s = n_string_push_cp(s, a_netrc_bsd_quote(&nrcep->nrce_dat[nrcep->nrce_password_idx]));
		}
	}
...
jlookup:{
	...
	if(!mx_url_parse(&url, CPROTO_NONE, argv[1])){
		...
		fprintf(n_stdout, "netrc: lookup: %s: machine %s", url.url_u_h.s, a_netrc_bsd_quote(nrce.nrce_machine));
		if(nrce.nrce_login != NIL)
			fprintf(n_stdout, " login %s", a_netrc_bsd_quote(nrce.nrce_login));
		if(nrce.nrce_password != NIL)
			fprintf(n_stdout, " password %s", a_netrc_bsd_quote(nrce.nrce_password));
		putc('\n', n_stdout);
	}else{
...
jlist:
	...
	if(!load_only){
		...
		slp = NIL;
		if(!(mx_xy_dump_dict("netrc", a_netrc_dp, &slp, NIL, &a_netrc_dump) &&
