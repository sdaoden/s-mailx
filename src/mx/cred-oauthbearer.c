...
boole
mx_oauthbearer_create_icr(struct str *res, uz pre_len, struct mx_url const *urlp, struct mx_cred_ctx const *ccp,
		boole is_xoauth2){
	union {uz u; int s; struct str *xres; boole rv;} j;
	char *cp;
	NYD_IN;

	STRUCT_ZERO(struct str, res);
	cp = NIL;

	/* Calculate required storage */
#define a_MAX (2 + sizeof("T18446744073709551615 AUTHENTICATE OAUTHBEARER " \
		"n,a=,\001host=\001port=65535\001auth=Bearer \001\001" NETNL))

	j.u = UZ_MAX - a_MAX;
	if(pre_len >= j.u || ccp->cc_user.l >= (j.u -= pre_len) || ccp->cc_pass.l >= (j.u -= ccp->cc_user.l) ||
...
#undef a_MAX
	...
	/* Credential string */
	if(!is_xoauth2)
		j.s = snprintf(cp, j.u, "n,a=%s,\001host=%s\001port=%hu\001auth=Bearer %s\001\001",
				ccp->cc_user.s, urlp->url_host.s, urlp->url_portno, ccp->cc_pass.s);
	else
		j.s = snprintf(cp, j.u, "user=%s\001auth=Bearer %s\001\001", ccp->cc_user.s, ccp->cc_pass.s);
	if(j.s < 0)
		goto jerr_io;
