...
CTA(((S(u32,a_CSOP_CMD__MAX | a_CSOP_ERR__MAX) << a_CSOP__FSHIFT) & ~a_CSOP__FCMDMASK) == 0, "Bit ranges overlap");
...
a_csop_cmd(struct a_csop_ctx *cscp){
	...
	case a_CSOP_CMD_HASH:
		...
		i = ((cscp->csc_flags & a_CSOP_MOD_CASE) ? su_cs_toolbox_case.tb_hash : su_cs_toolbox.tb_hash
				)(cscp->csc_arg);
	...
	case a_CSOP_CMD_FIND:
		...
		cscp->csc_varres = ((cscp->csc_flags & a_CSOP_MOD_CASE) ? su_cs_find_case : su_cs_find
				)(cscp->csc_argv[0], cscp->csc_arg);
...
jesubstring_off:
		...
		if(cscp->csc_argv[2] != NIL){
			...
			if(UCMP(64, i, >=, cscp->csc_lhv)){
				if(UCMP(64, i, !=, cscp->csc_lhv))
					cscp->csc_varres = savestrbuf(cscp->csc_varres, S(uz,cscp->csc_lhv));
			}else{
jesubstring_len:
				if(n_poption & n_PO_D_V)
...
c_csop(void *vp){
	...
	if((cp = su_cs_find_c(csc.csc_cmd_name, '?')) != NIL){
		...
		if(*csc.csc_cmd_name == '@'){ /* v15compat */
			n_OBSOLETE2(_("csop: please use ? modifier suffix, not @ prefix"),
				n_shexp_quote_cp(csc.csc_cmd_name, FAL0));
			++csc.csc_cmd_name;
			f = a_CSOP_MOD_CASE;
		}
	...
	for(i = 0; i < NELEM(a_csop_subcmds); ++i){
		if(su_cs_starts_with_case_n(a_csop_subcmds[i].css_name, csc.csc_cmd_name, j)){
			...
			if(UNLIKELY(f & a_CSOP_MOD_MASK)){
				...
				if(UNLIKELY(!(i & a_CSOP_MOD_MASK))){
					...
					break;
				/*
				}else if(UNLIKELY(f2 != a_CSOP_MOD_MASK && f2 != (i & a_CSOP_MOD_MASK))){
	...
	if(LIKELY(!(f & a_CSOP_ERR)))
		n_pstate_err_no = (f & a_CSOP_SOFTOVERFLOW) ? su_ERR_OVERFLOW : su_ERR_NONE;
