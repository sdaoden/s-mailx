...
static struct mx_cred_authtype_info const
	...
	{mx_CRED_AUTHTYPE_LOGIN,
		(a_CREDAUTH_AUTO | a_CREDAUTH_PASS_CLEARTXT | a_CREDAUTH_PASS_REQ | a_CREDAUTH_USER_REQ),
...
/* Automatic selection order for with TLS / without TLS.  Iteration stops either for [x]==_NONE || x == MECH_COUNT */
CTAV(CPROTO_NONE == 0);
CTAV(CPROTO_IMAP == 1);
CTAV(CPROTO_POP3 == 2);
CTAV(CPROTO_SMTP == 3);
...
static u8 const a_credauth_select_notls[CPROTO_SMTP][mx_CRED_AUTHTYPE_MECH_COUNT] = {
	{ /* IMAP */
#ifdef mx_HAVE_GSSAPI
		mx_CRED_AUTHTYPE_GSSAPI,
#endif
...
mx_cred_auth_lookup(struct mx_cred_ctx *credp, struct mx_url *urlp){
	...
	if(issetup){
		...
		/* Authentication type */
		if(authokey == U32_MAX || (s = xok_VLOOK(S(enum okeys,authokey), urlp, OXM_ALL)) == NIL)
		...
		if((ware & a_CREDAUTH_NEED_TLS)
			...
			n_err(_("Authentication %s requires TLS transport: %s\n"), credp->cc_auth, urlp->url_p_u_h_p);
			credp = NIL;
			goto jleave;
		}
	}
...
jpass:
	...
	if(ware & a_CREDAUTH_PASS_REQ){
		if((s = mx_tty_getpass(savecat(urlp->url_u_h.s, _(" requires a password: ")))) != NIL)
js2pass:
			credp->cc_pass.l = su_cs_len(credp->cc_pass.s = savestr(s));
		else{
			n_err(_("Authentication %s requires a password: %s\n"), pstr, urlp->url_u_h.s);
			credp = NIL;
		}
	}
...
boole
mx_cred_auth_type_verify_bits(struct mx_cred_authtype_verify_ctx *cavcp, boole maybe_tls){
	char const *pstr;
				...
				if(caip->cai_flags & a_CREDAUTH_NEED_TLS){
					if(!maybe_tls){
						if(n_poption & n_PO_D_V)
							n_err(_("%s: %s (%s) would require TLS: disabled\n"),
...
jerr:
		n_err(_("%s: no usable authentication types (remain%s)\n"),
			pstr, (n_poption & n_PO_D_V ? su_empty : _("; *verbose* for reasons")));
		rv = FAL0;
	}
...
u32
mx_cred_auth_type_select(enum cproto proto, u32 mechplusbits, boole assume_tls){
	...
	mp = &(assume_tls ? a_credauth_select : a_credauth_select_notls)[S(u32,proto) - 1];
	...
	for(i = mx_CRED_AUTHTYPE_NONE;;)
		if(mechplusbits & (rv = (*mp)[i])){
			...
			break;
		}else if(rv == mx_CRED_AUTHTYPE_NONE || ++i == mx_CRED_AUTHTYPE_MECH_COUNT){
