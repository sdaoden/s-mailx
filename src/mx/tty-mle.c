...
CTA(a_TTY_LINE_MAX <= S32_MAX, "a_TTY_LINE_MAX larger than S32_MAX, but the MLE uses 32-bit arithmetic");
...
# endif
	...
	a_TTY__BIND_FUN_MASK = ((1u << a_TTY__BIND_FUN_SHIFTMAX) - 1) & ~((1u << a_TTY__BIND_FUN_SHIFT) - 1),
# define a_TTY_BIND_FUN_REDUCE(X) ((S(u32,X) & a_TTY__BIND_FUN_MASK) >> a_TTY__BIND_FUN_SHIFT)
# define a_TTY_BIND_FUN_EXPAND(X) ((S(u32,X) & (a_TTY__BIND_FUN_MASK >> a_TTY__BIND_FUN_SHIFT)) << a_TTY__BIND_FUN_SHIFT)
# undef a_X
# define a_X(N,I) a_TTY_BIND_FUN_ ## N = a_TTY_BIND_FUN_EXPAND(I),
...
# endif
CTA(a_TTY_BIND_FUN_EXPAND(a_TTY_BIND_FUN_COMMIT) < (1u << a_TTY__BIND_FUN_SHIFTMAX),
...
CTA(UCMP(64, a_TTY__BIND_LAST, <=, S32_MAX), "Flag bits excess storage datatype" /* And we need one bit free */);
...
enum a_tty_visual_flags{
	...
	a_TTY_VF_REFRESH = a_TTY_VF_MOD_DIRTY | a_TTY_VF_MOD_CURSOR | a_TTY_VF_MOD_CONTENT | a_TTY_VF_MOD_SINGLE,
	...
	a_TTY_VF_ALL_MASK = a_TTY_VF_REFRESH | a_TTY_VF_BELL | a_TTY_VF_MAXWIDTH_POS0 | a_TTY_VF_SYNC,
...
struct a_tty_bind_ctx{
	...
	/* The .tbc_seq'uence with any terminal capabilities resolved; in fact an array of structures, the first entry
	 * of which is {s32 buf_len_iscap;} where the signed bit indicates whether the buffer is a resolved terminal
...
CTA(mx__TERMCAP_QUERY_MAX1 <= U16_MAX, "Enumeration cannot be stored in datatype");
...
struct a_tty_input_ctx_map{
	BITENUM(u32,mx_go_input_flags) ticm_ctx;
...
# endif
	...
	BITENUM(u16,a_tty_config_flags) tl_conf_flags;
	/* Visual data representation handling */
	BITENUM(u16,a_tty_visual_flags) tl_vi_flags;
	...
	su_64( u8 tl__pad2[4]; )
	BITENUM(u32,mx_go_input_flags) tl_goinflags;
	u32 tl_prompt_width;
...
static struct a_tty_input_ctx_map const a_tty_input_ctx_maps[mx__GO_INPUT_CTX_MAX1] = {
...
# ifdef mx_HAVE_KEY_BINDINGS
#  undef a_X
#  define a_X(Q,S) {FAL0, '\0', mx_TERMCAP_QUERY_ ## Q, {'\0', (char)a_TTY_BIND_FUN_REDUCE(a_TTY_BIND_FUN_ ## S),}},
...
/* (More) Actions of `history'.  Return TRUM1 if synopsis shall be shown */
...
static boole a_tty_hist_is_gabby_ok(BITENUM(u32,mx_go_input_flags) gif, char const **gt);

/* Add an entry TODO assumes sigs_all_hold() is held!  FAL0 on allocation failure, TRUM1 on length excess */
static boole a_tty_hist_add(char const *s, BITENUM(u32,mx_go_input_flags) gif);
...
# ifdef mx_HAVE_KEY_BINDINGS
static void a_tty_term_rawmode_timeout(struct a_tty_line *tlp, enum a_tty_term_timeout_mode ttm);
# endif
...
static void a_tty_check_grow(struct a_tty_line *tlp, u32 no  su_DVL_LOC_ARGS_DECL);
...
static void a_tty_copy2paste(struct a_tty_line *tlp, struct a_tty_cell *tcpmin, struct a_tty_cell *tcpmax);
...
/* Search for word boundary, starting at tl_cursor, in "dir"ection (<> 0).  Return <0 when moving is impossible
 * (backward direction but in position 0, forward direction but in outermost column), and relative distance to
 * tl_cursor otherwise */
...
/* Readline core */
static sz a_tty_readline(struct a_tty_line *tlp, uz len, boole *histok_or_nil  su_DVL_LOC_ARGS_DECL);
 ...
 * -- supposed to be used for a subsequent _bind_show(), then */
static boole a_tty_bind_parse(struct a_tty_bind_parse_ctx *tbpcp, boole isbindcmd);
...
static void a_tty__bind_tree_add(u32 hmap_idx, struct a_tty_bind_tree *store[a_TTY_PRIME], struct a_tty_bind_ctx *tbcp);
static struct a_tty_bind_tree *a_tty__bind_tree_add_wc(struct a_tty_bind_tree **treep, struct a_tty_bind_tree *parentp,
...
#ifdef mx_HAVE_REGEX
			if((tlp->tl_conf_flags & a_TTY_CONF_SRCH_ANY) && (tlp->tl_conf_flags & a_TTY_CONF_SRCH_REGEX))
				n_err(_("*line-editor-config*: srch-regex and srch-any are mutual exclusive\n"));
#endif
		}
	}
...
# ifdef mx_HAVE_COLOUR
	/* C99 */{
		...
		if(mx_COLOUR_IS_ACTIVE()){
			...
			if((ccp = mx_colour_pen_create(mx_COLOUR_ID_MLE_POSITION, NIL)) != NIL &&
					(s = mx_colour_pen_to_str(ccp)) != NIL){
				ccol = s->s;
...
# endif /* mx_HAVE_COLOUR */
	...
	if(!(tlp->tl_goinflags & mx_GO_INPUT_PROMPT_NONE))
		tlp->tl_prompt_width = mx_tty_create_prompt(tlp->tl_prompt, tlp->tl_prompt_base, tlp->tl_goinflags);

	/* The bind tree last, it is most expensive and has not really something to do with the line as such */
# ifdef mx_HAVE_KEY_BINDINGS
	/* C99 */{
		char const *name, *cp;
		u8 *destp;
...
jbind_timeout_redo:
		...
		tlp->tl_bind_shcut_prompt_char = &a_tty.tg_bind_shcut_prompt_char[tlp->tl_goinflags & mx__GO_INPUT_CTX_MASK];
	}
# endif /* mx_HAVE_KEY_BINDINGS */
...
# ifdef mx_HAVE_KEY_BINDINGS
static void
a_tty_term_rawmode_timeout(struct a_tty_line *tlp, enum a_tty_term_timeout_mode ttm){
	u32 tiosc;
	u8 i, j;
	NYD2_IN;
...
a_tty_check_grow(struct a_tty_line *tlp, u32 no  su_DVL_LOC_ARGS_DECL){
	...
	if(UNLIKELY((cmax = tlp->tl_count + no) > tlp->tl_count_max)){
		...
		if(LIKELY(i >= *tlp->tl_x_bufsize)){
			...
			i <<= 1;
			tlp->tl_line.cbuf =
			*tlp->tl_x_buf = su_MEM_REALLOC_LOCOR(*tlp->tl_x_buf, i, su_DVL_LOC_ARGS_ORUSE);
			*tlp->tl_x_bufsize = i;
...
a_tty_cell2save(struct a_tty_line *tlp){
	...
	for(tcap = tlp->tl_line.cells, len = 0, i = tlp->tl_count; i > 0; ++tcap, --i)
		len += tcap->tc_count;
...
static void
a_tty_copy2paste(struct a_tty_line *tlp, struct a_tty_cell *tcpmin, struct a_tty_cell *tcpmax){
	char *cp;
...
a_tty_vi__paint(struct a_tty_line *tlp){ /* {{{ TODO optimize */
	...
	u32 f, w, phy_wid_base, phy_wid, phy_base, phy_cur, cnt, ASSERT_INJ(lstcur su_COMMA) cur,
	...
	if(cnt == 0){
		...
		if((f & a_TTY_VF_MOD_DIRTY) && !mx_termcap_cmd(mx_TERMCAP_CMD_ce, phy_cur, -1))
			goto jerr;
	...
	/* If the left hand side of our visual viewpoint consumes less than half of the screen width, show the prompt */
	if(tcp_left == tlp->tl_line.cells)
		f |= a_LEFT_MIN;

	if((f & (a_LEFT_MIN | a_HAVE_PROMPT)) == (a_LEFT_MIN | a_HAVE_PROMPT) && w + tlp->tl_prompt_width < phy_wid){
		phy_base = tlp->tl_prompt_width;
		f |= a_SHOW_PROMPT;
	}

	/* Then search for right boundary.  Dependent upon n_PSO_FULLWIDTH (termcap am/xn) we leave the rightmost
	 * column empty because some terminals [cw]ould wrap the line if we write into that, or not.
	...
	if(--tcp_right == tccp){
		...
		/* Since we did brute-force walk also for the left boundary we may end up in a situation were anything
		 * effectively fits on the screen, including the prompt that is, but where we do not recognize this
		 * since we restricted the search to fit in some visual viewpoint.  Therefore try again to extend the
		 * left boundary to overcome that */
		if(!(f & a_LEFT_MIN)){
	...
	if((f & (a_LEFT_MIN | a_RIGHT_MAX | a_HAVE_PROMPT | a_SHOW_PROMPT)) ==
				(a_LEFT_MIN | a_RIGHT_MAX | a_HAVE_PROMPT) && w + tlp->tl_prompt_width <= phy_wid){
		phy_wid -= (phy_base = tlp->tl_prompt_width);
		f |= a_SHOW_PROMPT;
	}

	/* Try to avoid repainting the complete line - this is possible if the cursor "did not leave the screen" and
	 * the prompt status has not changed.  I.e., after cramping virtual viewpoint, compare relation to physical */
	if((f & (a_TTY_VF_MOD_SINGLE/*FIXME*/ | a_TTY_VF_MOD_CONTENT/* xxx */ | a_TTY_VF_MOD_DIRTY)) ||
...
/* FIXME reposition cursor for paint */
	...
	if((f & a_HAVE_POSITION) && ((f & (a_LEFT_MIN | a_RIGHT_MAX)) != (a_LEFT_MIN | a_RIGHT_MAX) /*||
...
# endif
		...
		if(phy_cur != (w = phy_wid_base) && !mx_termcap_cmd(mx_TERMCAP_CMD_ch, phy_cur = w, 0))
			goto jerr;
		...
		*pos++ = '|';
		if(f & a_LEFT_MIN)
			...
			/* Theoretical line length limit a_TTY_LINE_MAX, choose next power of ten (10 ** 10) to
			 * represent 100 percent; we do not have a macro that generates a constant, and i do not trust
			 * the standard "u type suffix automatically scales": calculate the large number */
			static char const itoa[] = "0123456789"; /* xxx inline itoa */

			u64 const fact100 = S(u64,0x3B9ACA00u) * 10u, fact = fact100 / 100;
	...
	/* Users are used to see the cursor right of the point of interest, so we need some further adjustments unless
	 * in special conditions.  Be aware that we may have adjusted cur at the beginning, too */
...
jcursor:
	if(((f & a_MOVE_CURSOR) || phy_nxtcur != phy_cur) && !mx_termcap_cmd(mx_TERMCAP_CMD_ch, phy_cur = phy_nxtcur, 0))
		goto jerr;
...
a_tty_wboundary(struct a_tty_line *tlp, s32 dir){/* TODO shell token-wise {{{ */
	...
	for(rv = 0, tcap = tlp->tl_line.cells, anynon = FAL0;;){
		...
		if(/*TODO not everywhere iswblank(wc)*/ wc == L' ' || wc == L'\t' || iswpunct(wc)){
			if(anynon)
				break;
		}else
			anynon = TRU1;
...
a_tty_kht(struct a_tty_line *tlp){ /* xxx huge: monster: split {{{ */
	...
	/* Get plain line data; if first expansion/xy, update original content so that ^G gets the origin back */
	orig = tlp->tl_savec;
	a_tty_cell2save(tlp);
	exp = tlp->tl_savec;
	if(orig.s != NIL){
...
jredo:
	...
	if(exp_res == NIL || exp_res[0][0] == '\0'){
		...
		/* No.  But maybe the users' desire was to complete only a part of the shell token of interest! */
		/* TODO This can be improved, we would need to
		...
		if(wedid == TRU1){
			...
			while(i-- != 0){
				...
				if(su_cs_is_space(c))
					break;
				else if((i != sub.l - 1 || c != '*') && su_cs_find_c(word_breaks, c) != NIL){
					li = i + 1;
					break;
				}
			}
		...
		/* A different case, user input includes for example character classes: here fexpand() will go over
		 * glob, and that will not find any match, thus returning NIL; try to wildcard expand this pattern! */
jaster_check:
		if(sub.s[sub.l - 1] != '*'){
			wedid = TRU1;
	...
	/* If the expansion equals the original string, assume the user wants what is usually known as tab completion,
	 * append `*' and restart */
...
jset:
	exp.l = su_cs_len(exp.s = n_shexp_quote_cp(exp.s, ((tlp->tl_conf_flags & a_TTY_CONF_QUOTE_RNDTRIP) != 0)));
...
jmulti:{ /* xxx huge: own function */
		...
		for(idx = 0; (sub.s = exp_res[idx]) != NIL; c1 = c2, ++idx){
			...
			/* Put on screen.  Indent follow lines of same sort.  Leave enough room for filename tagging */
			if((c1 = (c1 != c2))){
#ifdef mx_HAVE_C90AMEND1
				c1 = (iswalnum(c2) != 0);
#else
			...
			if(idx == 0 || c1 || scrwid < lnlen || scrwid - lnlen <= vic.vic_vi_width + 2){
				putc('\n', fp);
				if(scrwid < lnlen)
					++lncnt;
				++lncnt, lnlen = 0;
...
jsep:
		...
		/* A common prefix of 0 means we cannot provide the user any auto completed characters for the multiple
		 * possible results.  Otherwise we can, so extend the visual line content by the common prefix (in
		 * a reversible way) */
...
jnope:
	/* If we provided a default content, but failed to expand, nothing we can "revert to": drop that default */
	if(set_savec){
		tlp->tl_savec.s = NIL;
		tlp->tl_savec.l = 0;
	}
...
a_tty__khist_shared(struct a_tty_line *tlp, struct a_tty_hist *thp){
	...
	if(LIKELY((tlp->tl_hist = thp) != NIL)){
		...
		tlp->tl_defc.s = cp = su_AUTO_ALLOC(i +1);
...
if(0){	/* The byte variant {{{ */
	...
	for(x = i = size = 0;;){
		...
		/* xxx Brute simple search for PE sequence: "ignore charset": ok for supported POSIX compatible sets */
...
# endif
	...
	case a_X(CLEAR_SCREEN):
		tlp->tl_vi_flags |= (mx_termcap_cmdx(mx_TERMCAP_CMD_cl) == TRU1) ? a_TTY_VF_MOD_DIRTY : a_TTY_VF_BELL;
		break;
	...
	case a_X(CANCEL):
		/* Normally this just causes a restart and thus resets the state machine */
...
# endif
	...
	wchar_t wc;
	BITENUM(u32,a_tty_bind_flags) tbf;
	sz rv;
	NYD_IN;
...
# endif
			{
				/* Let me at least once dream of iomon(itor), timer with one-shot, enwrapped with
				 * key_event and key_sequence_event, all driven by an event_loop */
# ifdef mx_HAVE_KEY_BINDINGS
jread_again:
				if(isp == NIL){
					timeout = FAL0;
					...
					/* If current isp has children with and without ismbseq we will restart waiting
					 * for the normal key.  Otherwise check which timeout to use at first */
					if(timeout){
						...
						ASSERT(!timeout);
						ttm = a_TTY_TTM_KEY;
						for(tbtp = isp->tbtp->tbt_children; tbtp != NIL; tbtp = tbtp->tbt_sibling)
							if(tbtp->tbt_ismbseq){
								ttm = a_TTY_TTM_MBSEQ;
								break;
							}
...
# ifdef mx_HAVE_KEY_BINDINGS
				...
				if(rv == 0){
					...
					/* If we were waiting for a tbt_ismbseq to be continued, maybe there is
					 * a normal key child also */
					if(ttm == a_TTY_TTM_MBSEQ){
						for(tbtp = isp->tbtp->tbt_children; tbtp != NIL; tbtp = tbtp->tbt_sibling)
							if(!tbtp->tbt_ismbseq)
								goto jread_again;
...
# endif /* mx_HAVE_KEY_BINDINGS */
				...
				/* As a special case, simulate EOF via EOT (which can happen via type-ahead, for
				 * example when typing "yes\n^@" during sleep of
				 *   $ sleep 5; mail -s byjove $LOGNAME */
				if(*cbufp == '\0'){
					ASSERT((n_psonce & n_PSO_INTERACTIVE) && !(n_pstate & n_PS_ROBOT));
					*cbuf = '\x04';
				}
				++cbufp;
				...
				rv = S(sz,mbrtowc(&wc, cbuf, P2UZ(cbufp - cbuf), &ps[0]));
				if(rv <= 0){
					/* If it is a hard error, or if too many redundant shift sequences overflow our
					 * buffer: perform hard reset */
					if(rv == -1 || sizeof cbuf_base == P2UZ(cbufp - cbuf)){
						a_tty_fun(tlp, a_TTY_BIND_FUN_FULLRESET, &len);
						goto jrestart;
					}
					/* Otherwise, due to the way we deal with the buffer, we need to restore the
					 * mbstate_t from before this conversion */
					ps[0] = ps[1];
					continue;
				}
				cbufp = cbuf;
...
# ifdef mx_HAVE_KEY_BINDINGS
			...
			if(su_cs_is_ascii(wc)){
				...
				for(c = S(char,wc), cp = &(*tlp->tl_bind_shcut_prompt_char)[0]; *cp != '\0'; ++cp)
					if(c == *cp){
						wc = a_tty_vinuni(tlp);
						break;
					}
			...
			/* Does (final) wc exist in the applicable bind tree?  Does it (possibly) want more input? */
			tbtp = (isp != NIL) ? isp->tbtp->tbt_children : (*tlp->tl_bind_tree_hmap)[wc % a_TTY_PRIME];
			...
			/* The special cancel bypass input function is inferior to key-sequence continuation, so test
			 * it thereafter */
			if(su_cs_is_ascii(wc)){
				char const *cp;
				char c;

				for(c = S(char,wc), cp = &(*tlp->tl_bind_shcut_cancel)[0]; *cp != '\0'; ++cp)
					if(c == *cp){
			...
			/* We know the wc read last has no binding.  If there was a partial binding active already,
			 * finalize it if possible, otherwise take over all its characters.  And wc */
			if(isp != NIL){
...
# else /* mx_HAVE_KEY_BINDINGS */
			/* if it is a control byte check whether it is a MLE function.  Remarks: initially a complete
			 * duplicate to be able to switch(), later converted to simply iterate over (an #ifdef'd subset
			 * of) the MLE base_tuple table in order to have "a SPOF" */
...
jbuiltin_redo:
				...
				if(tbbtp == &a_tty_bind_base_tuples[NELEM(a_tty_bind_base_tuples)]){
...
jdone:
	/* We have a completed input line, convert the struct cell data to its plain character equivalent */
	rv = a_tty_cell2dat(tlp);

	if(histok_or_nil != NIL && (rv <= 0 || su_cs_is_space(tlp->tl_line.cbuf[0])))
		*histok_or_nil = FAL0;
...
a_tty_bind_ctx_find(char const *name){
	...
	do if(!su_cs_cmp_case(ticmp->ticm_name, name)){
		rv = ticmp->ticm_ctx;
		goto jleave;
	}while(PCMP(++ticmp, <, &a_tty_input_ctx_maps[NELEM(a_tty_input_ctx_maps)]));

	rv = R(BITENUM(u32,mx_go_input_flags),-1);
jleave:
	NYD2_OU;
	return rv;
}
...
jshadow:
	...
	/* C99 */{
		...
		su_mem_copy(tbcp->tbc_seq = &tbcp->tbc__buf[0], tbpcp->tbpc_seq,
			i = (tbcp->tbc_seq_len = tbpcp->tbpc_seq_len) +1);
		ASSERT(tbpcp->tbpc_exp.l > 0);
		su_mem_copy(tbcp->tbc_exp = &tbcp->tbc__buf[i], tbpcp->tbpc_exp.s,
			j = (tbcp->tbc_exp_len = tbpcp->tbpc_exp.l) +1);
		i += j;
		i = (i + tbpcp->tbpc_cnv_align_mask) & ~tbpcp->tbpc_cnv_align_mask;
		su_mem_copy(tbcp->tbc_cnv = &tbcp->tbc__buf[i], tbpcp->tbpc_cnv,
			tbcp->tbc_cnv_len = tbpcp->tbpc_cnv_len);
...
a_tty_bind_parse(struct a_tty_bind_parse_ctx *tbpcp, boole isbindcmd){ /* {{{ */
	...
	u32 f;
	NYD2_IN;
	LCTA(UCMP(64, a_TRUE_RV, <, U32_MAX), "Flag bits excess storage datatype");
	...
	for(shin.s = UNCONST(char*,tbpcp->tbpc_in_seq), shin.l = UZ_MAX;;){
		...
		if((shs & (n_SHEXP_STATE_OUTPUT | n_SHEXP_STATE_STOP)) == n_SHEXP_STATE_STOP)
			break;
		...
		ep->next = NIL;
		if(!(shs & n_SHEXP_STATE_ERR_UNICODE)){
			i = su_cs_len(ep->seq_dat = n_shexp_quote_cp(n_string_cp(shoup), TRU1));
			if(i >= S32_MAX - 1)
				goto jelen;
			ep->seq_len = S(u32,i);
		}else{
			/* Otherwise use the original buffer, _we_ can only quote it the wrong way (e.g., an initial
			 * $'\u3a' becomes '\u3a'), _then_ */
		...
		if(!mx_visual_info(&vic, mx_VISUAL_INFO_WOUT_CREATE | mx_VISUAL_INFO_WOUT_AUTO_ALLOC)){
			n_err(_("%s: key-sequence seems to contain invalid characters: %s: %s\n"),
				tbpcp->tbpc_cmd, n_string_cp(shoup), tbpcp->tbpc_in_seq);
			f |= a_TTY_BIND_DEFUNCT;
			goto jleave;
		}else if(vic.vic_woulen == 0 || vic.vic_woulen >= (S32_MAX - 2) / sizeof(wc_t)){
jelen:
	...
	/* C99 */{
		...
		for(seql = cnvl = 0, tail = head; tail != NIL; tail = tail->next){
			...
			if((i = tail->cnv_len) & S32_MIN){
				...
				if(i < a_TTY_BIND_CAPEXP_ROUNDUP)
					i = (i + (a_TTY_BIND_CAPEXP_ROUNDUP - 1)) & ~(a_TTY_BIND_CAPEXP_ROUNDUP - 1);
			}else
		...
		*--cp = '\0';
		...
		/* C99 */{
			...
			for(ltbcp = NIL, tbcp = a_tty.tg_bind[gif]; tbcp != NIL; ltbcp = tbcp, tbcp = tbcp->tbc_next)
				if(tbcp->tbc_seq_len == seql && !su_mem_cmp(tbcp->tbc_seq, cpbase, seql)){
					tbpcp->tbpc_tbcp = tbcp;
					break;
				}
		}
	...
	if(isbindcmd){
		...
		for(i = 0; i < NELEM(a_tty_bind_fun_names); ++i)
			if(!su_cs_cmp_case(exp, a_tty_bind_fun_names[i])){
				tbpcp->tbpc_flags |= a_TTY_BIND_FUN_EXPAND(i) | a_TTY_BIND_FUN_INTERNAL |
						(head->next == NIL ? a_TTY_BIND_MLE1CNTRL : 0);
				if((n_poption & n_PO_D_V) && (tbpcp->tbpc_flags & a_TTY_BIND_NOCOMMIT))
...
a_tty_bind_resolve(struct a_tty_bind_ctx *tbcp){ /* {{{ */
	...
	UNINIT(next, NIL);
	for(cp = tbcp->tbc_cnv, isfirst = TRU1, len = tbcp->tbc_cnv_len; len > 0; isfirst = FAL0, cp = next){
		/* C99 */{
			s32 i, j;
		...
		/* C99 */{
			...
			if(/*i > S32_MAX ||*/ i >= P2UZ(next - cp)){
				...
				if(n_poption & n_PO_D_V)
					n_err(_("bind: capability expansion does not start with control: %s: %s\n"),
						capname, tbcp->tbc_seq);
				tbcp->tbc_flags |= a_TTY_BIND_DEFUNCT;
				break;
			}
...
a_tty_bind_show(struct a_tty_bind_ctx *tbcp, FILE *fp){ /* {{{ */
	...
	if((n_poption & n_PO_D_V) &&
		...
		for(cnvdat = tbcp->tbc_cnv, cnvlen = tbcp->tbc_cnv_len; cnvlen > 0;){
			...
			if(entlen & S32_MIN){
				 ...
				 * char buf[]+NUL;} */
				for(bsep = su_empty, u.cp = S(char const*,&UNALIGN(s32 const*,cnvdat)[2]);
						(c = *u.cp) != '\0'; ++u.cp){
	...
	fprintf(fp, "%s%sbind %s %s %s%s%s\n",
		...
		(!(n_poption & n_PO_D_VV) ? su_empty : (flags & a_TTY_BIND_FUN_INTERNAL ? _(" # MLE internal") : su_empty))
		);
...
a_tty_bind_del(struct a_tty_bind_parse_ctx *tbpcp){
	...
	if((ltbcp = tbpcp->tbpc_ltbcp) != NIL)
		...
		a_tty.tg_bind[tbpcp->tbpc_flags & mx__GO_INPUT_CTX_MASK] = tbcp->tbc_next;
	su_FREE(tbcp);
...
a_tty_bind_tree_build(void){
	...
	for(i = 0; i < mx__GO_INPUT_CTX_MAX1; ++i){
		...
		/* Somewhat wasteful, but easier to handle: simply clone the entire primary key onto the secondary one,
		 * then only modify it */
		for(tbcp = a_tty.tg_bind[mx_GO_INPUT_CTX_BASE]; tbcp != NIL; tbcp = tbcp->tbc_next)
			if(!(tbcp->tbc_flags & a_TTY_BIND_DEFUNCT))
...
a_tty_bind_tree_teardown(void){
	...
	su_mem_set(&a_tty.tg_bind_shcut_cancel[0], 0, sizeof(a_tty.tg_bind_shcut_cancel));
	su_mem_set(&a_tty.tg_bind_shcut_prompt_char[0], 0, sizeof(a_tty.tg_bind_shcut_prompt_char));
...
a_tty__bind_tree_add(u32 hmap_idx, struct a_tty_bind_tree *store[a_TTY_PRIME], struct a_tty_bind_ctx *tbcp){ /* {{{ */
	...
	for(cnvdat = tbcp->tbc_cnv, cnvlen = tbcp->tbc_cnv_len; cnvlen > 0;){
		...
		if(entlen & S32_MIN){
			...
			for(ismbseq = FAL0, u.cp = S(char const*,&UNALIGN(s32 const*,cnvdat)[2]); *u.cp != '\0';
					ismbseq = TRU1, ++u.cp)
			...
			/* May be a special shortcut function?  Since bind_tree_build() is easy and clones over
			 * CTX_BASE to the "real" contexts: do not put the same shortcut several times */
...
a_tty__bind_tree_add_wc(struct a_tty_bind_tree **treep, struct a_tty_bind_tree *parentp, wchar_t wc, boole ismbseq){
	...
	for(any = FAL0, xtbtp = NIL, tbtp = *treep; tbtp != NIL; xtbtp = tbtp, tbtp = tbtp->tbt_sibling){
		if(tbtp->tbt_char != wc){
			if(any)
				break;
			continue;
		...
		/* ismbseq MUST be linked before !ismbseq, so record this "parent" sibling, but continue searching for
		 * now.  Otherwise it is impossible that we will find what we look for */
		if(ismbseq){
#if ASSERT_INJOR(1, 0)
...
int
(mx_tty_readline)(BITENUM(u32,mx_go_input_flags) gif, char const *prompt, char **linebuf, uz *linesize, uz n,
...
void
mx_tty_addhist(char const *s, BITENUM(u32,mx_go_input_flags) gif){
	NYD_IN;
...
c_bind(void *vp){ /* {{{ */
	...
	if(cacp->cac_no == 0)
		...
		if((gif = a_tty_bind_ctx_find(c.cp)) == R(BITENUM(u32,mx_go_input_flags),-1)){
