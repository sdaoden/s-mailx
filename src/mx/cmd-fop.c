 ...
 *
 * Copyright (c) 2021 - 2025 Steffen Nurpmeso <steffen@sdaoden.eu>.
...
/* idec and find an open fd, or NIL.  std_or_nil must be a pointer to storage if used, .fof_next will be NIL. */
static struct a_fop_ofd *a_fop_fd(char const *arg, struct a_fop_ofd *std_or_nil, struct a_fop_ofd ***fofppp_or_nil);
...
a_fop_fd(char const *arg, struct a_fop_ofd *std_or_nil, struct a_fop_ofd ***fofppp_or_nil){
...
a_fop__close(struct a_fop_ctx *fcp){
	...
	}else if((fofdp = a_fop_fd(fcp->fc_arg = fcp->fc_argv[0], NIL, &fofdpp)) != NIL){
		boole silence;
...
a_fop__expand(struct a_fop_ctx *fcp){
	...
	if(fcp->fc_argv[0] == NIL ||
		...
		if((fcp->fc_varres = mx_fexpand(fcp->fc_arg, (mx_FEXP_DEF_LOCAL_FILE & ~mx_FEXP_NGLOB))) == NIL){
			fcp->fc_flags |= a_FOP_ERR;
...
jglob_next:
		...
		cpp = mx_fexpand_multi(cap->ca_arg.ca_str.s, (mx_FEXP_DEF_LOCAL_FILE_VAR & ~mx_FEXP_NGLOB));
...
a_fop__open(struct a_fop_ctx *fcp){
	...
	if(fcp->fc_argv[0] == NIL || fcp->fc_argv[1] == NIL || fcp->fc_argv[2] != NIL){
jesyn:
		fcp->fc_flags |= a_FOP_ERR;
		fcp->fc_cmderr = a_FOP_ERR_SYNOPSIS;
		goto jleave;
...
a_fop__pass(struct a_fop_ctx *fcp){
	...
	if(fcp->fc_argv[0] == NIL || fcp->fc_argv[1] == NIL || fcp->fc_argv[2] == NIL || fcp->fc_argv[3] != NIL){
		fcp->fc_flags |= a_FOP_ERR;
		fcp->fc_cmderr = a_FOP_ERR_SYNOPSIS;
		goto jleave;
	}
...
jredo:
	...
	if((fcp->fc_arg[0] == '-' || fcp->fc_arg[0] == '@') && fcp->fc_arg[1] == '\0'){
		...
		cc.cc_fds[(i == 0) ? mx_CHILD_FD_IN : mx_CHILD_FD_OUT] = fileno(fofdp->fof_fp);
	else{
		n_pstate_err_no = su_ERR_INVAL;
		fcp->fc_flags |= a_FOP_ERR;
		fcp->fc_cmderr = a_FOP_ERR_STR_GENERIC;
...
a_fop__rename(struct a_fop_ctx *fcp){
	...
	if(fcp->fc_argv[0] == NIL || fcp->fc_argv[1] == NIL || fcp->fc_argv[2] != NIL){
		...
		if((fcp->fc_varres = mx_fexpand(fcp->fc_arg, mx_FEXP_DEF_LOCAL_FILE)) == NIL ||
				(src = mx_fexpand(fcp->fc_argv[1], mx_FEXP_DEF_LOCAL_FILE)) == NIL){
			fcp->fc_flags |= a_FOP_ERR;
			fcp->fc_cmderr = a_FOP_ERR_STR_NODATA;
...
a_fop__stat(struct a_fop_ctx *fcp){
	...
	if(!((fcp->fc_flags & a_FOP_MOD_NOFOLLOW) ? su_pathinfo_lstat : su_pathinfo_stat)(&pi, fcp->fc_varres)){
		fcp->fc_flags |= a_FOP_ERR;
		fcp->fc_cmderr = a_FOP_ERR_STR_NODATA;
		goto jleave;
	}
	...
	s = n_string_push_cp(s, su_ienc_u64(fcp->fc_iencbuf, pi.pi_size, 10));
	s = n_string_push_c(s, ' ');

	s = n_string_push_cp(s, "st_mode=");
	s = n_string_push_cp(s, su_ienc_u32(fcp->fc_iencbuf, (pi.pi_flags & (su_IOPF_PERM_MASK | su_IOPF_PROT_MASK)), 8));
	s = n_string_push_c(s, ' ');

	s = n_string_push_cp(s, "st_uid=");
	s = n_string_push_cp(s, su_ienc_uz(fcp->fc_iencbuf, pi.pi_uid, 10));
	s = n_string_push_c(s, ' ');
	s = n_string_push_cp(s, "st_gid=");
	s = n_string_push_cp(s, su_ienc_uz(fcp->fc_iencbuf, pi.pi_gid, 10));
	s = n_string_push_c(s, ' ');

	s = n_string_push_cp(s, "st_atime=");
	s = n_string_push_cp(s, su_ienc_s64(fcp->fc_iencbuf, pi.pi_atime.ts_sec, 10));
	s = n_string_push_cp(s, " st_atimensec=");
	s = n_string_push_cp(s, su_ienc_s64(fcp->fc_iencbuf, pi.pi_atime.ts_nano, 10));
	s = n_string_push_c(s, ' ');

	s = n_string_push_cp(s, "st_mtime=");
	s = n_string_push_cp(s, su_ienc_s64(fcp->fc_iencbuf, pi.pi_mtime.ts_sec, 10));
	s = n_string_push_cp(s, " st_mtimensec=");
	s = n_string_push_cp(s, su_ienc_s64(fcp->fc_iencbuf, pi.pi_mtime.ts_nano, 10));
	s = n_string_push_c(s, ' ');

	s = n_string_push_cp(s, "st_ctime=");
	s = n_string_push_cp(s, su_ienc_s64(fcp->fc_iencbuf, pi.pi_ctime.ts_sec, 10));
	s = n_string_push_cp(s, " st_ctimensec=");
	s = n_string_push_cp(s, su_ienc_s64(fcp->fc_iencbuf, pi.pi_ctime.ts_nano, 10));
	s = n_string_push_c(s, ' ');
