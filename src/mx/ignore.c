...
static struct mx_ignore *a_ignore_resolve_self(struct mx_ignore *xself, boole docreate);

/* Delete all retain( else ignor)ed members */
static void a_ignore_del_allof(struct mx_ignore *ip, boole retain);

/* "assign" / "join" */
static boole a_ignore_assijoin(struct mx_ignore *self, struct mx_ignore const *tp, boole isjoin);

/* Logic behind `headerpick' */
static boole a_ignore_addcmd_mux(struct mx_ignore *ip, char const **list, boole retain);

static void a_ignore__show(struct mx_ignore const *ip, boole retain);

/* Logic behind `unheaderpick' */
static boole a_ignore_delcmd_mux(struct mx_ignore *ip, char const **list, boole retain);

static boole a_ignore__delone(struct mx_ignore *ip, boole retain, char const *field);

/* Lookup whether a mapping is contained: TRU1=retained, FAL0=ignored, TRUM1=both; if retain is _not_ TRUM1 then
 * regular expressions are not executed but instead their .ir_input is byte-compared against len bytes of dat.
 * dat[len] is NUL.  If isre=TRUM1 we just look.  Note it does not handle the .it_all "all fields" condition */
static boole a_ignore_lookup(struct mx_ignore const *self, boole retain, char const *dat, uz len, boole isre);
...
a_ignore_del_allof(struct mx_ignore *ip, boole retain){
	...
	STRUCT_ZERO(struct a_ignore_type, itp);

	NYD2_OU;
}

static boole
a_ignore_assijoin(struct mx_ignore *self, struct mx_ignore const *tp, boole isjoin){
#ifdef mx_HAVE_REGEX
...
jedel:
	...
	if((tp = a_ignore_resolve_self(UNCONST(struct mx_ignore*,tp), TRU1)) == NIL){
		rv = FAL0;
		goto jleave;
	}
...
jreign:
	...
	for(i = 0; i < NELEM(titp->it_ht); ++i)
		for(tifp = titp->it_ht[i]; tifp != NIL; tifp = tifp->if_next){
			if(!(rv = a_ignore_lookup(self, retain, tifp->if_field, tifp->if_len, FAL0)) &&
					!(rv = a_ignore_insert(itp, tifp->if_field, tifp->if_len, FAL0)))
				goto jedel;
		}

#ifdef mx_HAVE_REGEX
	for(tirp = titp->it_re; tirp != NIL; tirp = tirp->ir_next){
		if(!(rv = a_ignore_lookup(self, retain, tirp->ir_input, tirp->ir_len, TRU1)) &&
				!(rv = a_ignore_insert(itp, tirp->ir_input, tirp->ir_len, TRU1)))
			goto jedel;
	}
#endif
...
a_ignore__show(struct mx_ignore const *ip, boole retain){
	...
	*ap = NIL;
	...
	su_sort_shell_vpp(su_S(void const**,ring), P2UZ(ap - ring), su_cs_toolbox_case.tb_cmp);

	i = fprintf(n_stdout, "headerpick %s %s", ip->i_name, (retain ? "retain" : "ignore"));
...
a_ignore__delone(struct mx_ignore *ip, boole retain, char const *field){
	...
	if(n_re_could_be_one_cp(field)){
		...
		for(irp = *(lirp = &itp->it_re); irp != NIL; lirp = &irp->ir_next, irp = irp->ir_next)
		...
		for(ifp = *(ifpp = &itp->it_ht[hi]); ifp != NIL; ifpp = &ifp->if_next, ifp = ifp->if_next){
...
static boole
a_ignore_lookup(struct mx_ignore const *self, boole retain, char const *dat, uz len, boole isre){
	boole rv;
#ifdef mx_HAVE_REGEX
	struct a_ignore_re *irp;
#endif
	...
	if(retain && self->i_retain.it_count > 0){
		...
		if(!isre || isre == TRUM1){
			for(ifp = self->i_retain.it_ht[hi]; ifp != NIL; ifp = ifp->if_next)
				if(!su_cs_cmp_case_n(ifp->if_field, dat, len) && ifp->if_field[len] == '\0')
					goto jleave;
		}
		...
		if(isre){
			for(irp = self->i_retain.it_re; irp != NIL; irp = irp->ir_next)
				if((retain == TRUM1 ? su_re_eval_cp(&irp->ir_re, dat, su_RE_EVAL_NONE)
...
static boole
a_ignore_insert(struct a_ignore_type *itp, char const *dat, uz len, boole isre){
	boole rv;
	NYD_IN;
	UNUSED(isre);
	...
	if(isre){
		...
		if(su_re_setup_cp(rep, irp->ir_input, (su_RE_SETUP_EXT | su_RE_SETUP_ICASE | su_RE_SETUP_TEST_ONLY)
				) != su_RE_ERROR_NONE){
...
c_headerpick(void *vp){
	...
	if(*argv == NIL){
		...
		for(ibmp = &a_ignore_bltin_map[0]; ibmp <= &a_ignore_bltin_map[mx__IGNORE_MAX]; ++ibmp){
	...
	if(su_cs_starts_with_case("create", *argv)){
		...
		if(!n_shexp_is_valid_varname(*argv, FAL0) || su_cs_starts_with_case("create", *argv) ||
				su_cs_starts_with_case("remove", *argv) || su_cs_starts_with_case("assign", *argv) ||
...
jecreatname:
	...
	xself = NIL;
	if((rv = -1, su_cs_starts_with_case("assign", *argv)) || (rv = -2, su_cs_starts_with_case("join", *argv))){
...
jectx:
	...
	if(rv != 0){
		if(rv > 0){
			...
			rv = a_ignore_assijoin(xself, self, (rv != -1)) ? su_EX_OK : su_EX_ERR;
		goto jleave;
	}
...
c_retain(void *vp){
	...
	rv = a_ignore_addcmd_mux(mx_IGNORE_TYPE, vp, TRU1) ? su_EX_OK : su_EX_ERR;
...
c_ignore(void *vp){
	...
	rv = a_ignore_addcmd_mux(mx_IGNORE_TYPE, vp, FAL0) ? su_EX_OK : su_EX_ERR;
...
c_unretain(void *vp){
	...
	rv = a_ignore_delcmd_mux(mx_IGNORE_TYPE, vp, TRU1) ? su_EX_OK : su_EX_ERR;
...
c_unignore(void *vp){
	...
	rv = a_ignore_delcmd_mux(mx_IGNORE_TYPE, vp, FAL0) ? su_EX_OK : su_EX_ERR;
...
c_saveretain(void *v){ /* TODO v15 drop */
	...
	rv = a_ignore_addcmd_mux(mx_IGNORE_SAVE, v, TRU1) ? su_EX_OK : su_EX_ERR;
...
c_saveignore(void *v){ /* TODO v15 drop */
	...
	rv = a_ignore_addcmd_mux(mx_IGNORE_SAVE, v, FAL0) ? su_EX_OK : su_EX_ERR;
...
c_unsaveretain(void *v){ /* TODO v15 drop */
	...
	rv = a_ignore_delcmd_mux(mx_IGNORE_SAVE, v, TRU1) ? su_EX_OK : su_EX_ERR;
...
c_unsaveignore(void *v){ /* TODO v15 drop */
	...
	rv = a_ignore_delcmd_mux(mx_IGNORE_SAVE, v, FAL0) ? su_EX_OK : su_EX_ERR;
...
c_fwdretain(void *v){ /* TODO v15 drop */
	...
	rv = a_ignore_addcmd_mux(mx_IGNORE_FWD, v, TRU1) ? su_EX_OK : su_EX_ERR;
...
c_fwdignore(void *v){ /* TODO v15 drop */
	...
	rv = a_ignore_addcmd_mux(mx_IGNORE_FWD, v, FAL0) ? su_EX_OK : su_EX_ERR;
...
c_unfwdretain(void *v){ /* TODO v15 drop */
	...
	rv = a_ignore_delcmd_mux(mx_IGNORE_FWD, v, TRU1) ? su_EX_OK : su_EX_ERR;
...
c_unfwdignore(void *v){ /* TODO v15 drop */
	...
	rv = a_ignore_delcmd_mux(mx_IGNORE_FWD, v, FAL0) ? su_EX_OK : su_EX_ERR;
...
struct mx_ignore *
mx_ignore_new(char const *name, boole auto_cleanup){ struct mx_ignore *self;
	NYD_IN;

	self = a_ignore_new(name, (auto_cleanup ? a_IGNORE_CLEANUP : a_IGNORE_NONE));
...
mx_ignore_is_any(struct mx_ignore const *self){
	...
	rv = (self != NIL && (self->i_retain.it_count != 0 || self->i_retain.it_all || self->i_ignore.it_count != 0 ||
			self->i_ignore.it_all));
...
mx_ignore_lookup(struct mx_ignore const *self, char const *dat){
	...
	if(self == mx_IGNORE_ALL)
		rv = TRUM1;
	else if(*dat == '\0' || (self = a_ignore_resolve_self(UNCONST(struct mx_ignore*,self), FAL0)) == NIL)
		rv = FAL0;
