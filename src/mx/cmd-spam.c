 ...
 *
 * Copyright (c) 2013 - 2022 Steffen Nurpmeso <steffen@sdaoden.eu>.
...
/* This is chosen rather arbitrarily.  It must be able to swallow the first line of a rate response */
...
#if (defined mx_HAVE_SPAM_FILTER && defined mx_HAVE_REGEX) || defined mx_HAVE_SPAM_SPAMC
...
a_spam_action(enum a_spam_action sa, int *ip){
	...
	if((cp = ok_vlook(spam_maxsize)) == NIL || (su_idec_u32_cp(&maxsize, cp, 0, NIL), maxsize) == 0)
	...
	for(curr = 0, ok = TRU1; *ip != 0; --cnt, ++curr, ++ip){
		...
		if(vc.vc_mp->m_size > maxsize){
			if(vc.vc_verbose)
				n_err(_("%s: message %" PRIuZ " exceeds maxsize (% " PRIuZ " > %" PRIuZ "), skip\n"),
			...
			if((vc.vc_ifp = setinput(&mb, vc.vc_mp, NEED_BODY)) == NIL){
				n_err(_("%s`%s': cannot load message %" PRIuZ ": %s\n"),
					vc.vc_esep, a_spam_cmds[sa], vc.vc_mno, su_err_doc(-1));
				ok = FAL0;
				break;
			}
...
# ifdef mx_HAVE_REGEX
	if(vcp->vc_action == a_SPAM_RATE && (cp = ok_vlook(spamfilter_rate_scanscore)) != NIL){
		char const *bp;
		...
		if(sfp->f_score_grpno == 0){
			...
			goto jleave;
		}else if(su_re_setup_cp(su_re_create(&sfp->f_score_regex), bp, (su_RE_SETUP_EXT | su_RE_SETUP_ICASE)
				) != su_RE_ERROR_NONE){
			n_err(_("%s: invalid *spamfilter-rate-scanscore* regex: %s: %s\n"),
				a_spam_cmds[vcp->vc_action], n_shexp_quote_cp(cp, FAL0), su_re_error_doc(&sfp->f_score_regex));
			goto jerr_re;
...
# endif
	...
	if(vcp->vc_action == a_SPAM_FORGET)
		vcp->vc_t.cf.cf_cmd = (vcp->vc_mp->m_flag & MSPAM) ? vcp->vc_t.filter.f_cmd_nospam
				: vcp->vc_t.filter.f_cmd_noham;
...
# ifdef mx_HAVE_REGEX
	...
	if(sfp->f_super.cf_result == NIL){
		n_err(_("%s: *spamfilter-rate-scanscore*: filter does not produce output!\n"));
		goto jleave;
	}

	remp = &sfp->f_score_regex.re_match[sfp->f_score_grpno];

	if(!su_re_eval_cp(&sfp->f_score_regex, sfp->f_super.cf_result, su_RE_EVAL_NONE) || remp->rem_start == -1){
		n_err(_("%s: *spamfilter-rate-scanscore* does not match filter output!\n"),
...
a_spamc_interact(struct a_spam_vc *vcp){
	...
	if(vcp->vc_action != a_SPAM_RATE){
		...
		if((cp = su_cs_find_c(buf = vcp->vc_t.spamc.c_super.cf_result, '/')) != NIL)
			buf[P2UZ(cp - buf)] = '\0';
...
a_spam_cf_setup(struct a_spam_vc *vcp, boole useshell){
	...
	scfp->cf_env[0] = str_concat_csvl(&s, n_PIPEENV_FILENAME_GENERATED, "=", cp, NIL)->s;
	/* v15 compat NAIL_ environments vanish! */
	scfp->cf_env[1] = str_concat_csvl(&s, "NAIL_FILENAME_GENERATED", "=", cp, NIL)->s;
	scfp->cf_env[2] = NIL;
...
jtail:
	...
	if(state & a_RUNNING){
		if(!(state & a_ERRORS) && vcp->vc_action == a_SPAM_RATE && !(state & (a_JUMPED | a_ERRORS))){
			sz i;
			...
			if(i > 0){
				...
				if((cp = su_cs_find_c(vcp->vc_buffer, NETNL[0])) == NIL &&
					...
					scfp->cf_result = su_cs_dup_cbuf(vcp->vc_buffer, P2UZ(cp - vcp->vc_buffer), 0);
/* FIXME consume child output until EOF??? */
		...
		state &= ~a_RUNNING;
		if(mx_child_wait(&cc) && (scfp->cf_exit_status = cc.cc_exit_status) >= su_EX_OK)
			state |= a_GOODRUN;
	}
...
a_spam_rate2score(struct a_spam_vc *vcp, char *buf){
	...
	if(!(ids & su_IDEC_STATE_CONSUMED)){
		...
		++buf; /* '.' */
		if((c1 = buf[0]) != '\0' && (c2 = buf[1]) != '\0' && (c3 = buf[2]) != '\0'){
			buf[2] = '\0';
			if(c3 >= '5'){
				if(c2 == '9'){
					if(c1 == '9'){
