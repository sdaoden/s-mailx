...
mx_tty_getfilename(struct n_string *store, BITENUM(u32,mx_go_input_flags) gif, char const *prompt_or_nil,
	...
	if((n_psonce & (n_PSO_INTERACTIVE | n_PSO_GETFILENAME_QUOTE_NOTED)) == n_PSO_INTERACTIVE){
...
mx_tty_create_prompt(struct n_string *store, char const *xprompt, BITENUM(u32,mx_go_input_flags) gif){
	...
	if(!(n_psonce & n_PSO_ERRORS_NOTED) && n_pstate_err_cnt > 0){
		...
		n_err(_("There are messages in the error ring, manageable via `errors' command\n"));
	}
#endif
...
jeeval:
	...
	for(pwidth = 0; vic.vic_inlen > 0;){
		...
		if(vic.vic_indat[0] == '\\' && vic.vic_inlen > 1 && vic.vic_indat[1] == '['){
			uz i;
		...
		}else if(!mx_visual_info(&vic, mx_VISUAL_INFO_WIDTH_QUERY | mx_VISUAL_INFO_ONE_CHAR)){
	...
	if(mx_COLOUR_IS_ACTIVE()){
		...
		/* */
		if(poff > 0 && rsp != NIL && (((ccp = mx_colour_pen_create(mx_COLOUR_ID_MLE_POSITION, NIL)) != NIL &&
