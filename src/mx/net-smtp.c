...
enum a_netsmtp_flags{
	/* *smtp-config* parser verifies some conditions.  The rest of this file assumes for example that if _AUTH is
	 * not set no authentication types are set */
	a_NETSMTP_EXT_NONE,
	...
	a_NETSMTP_EXT_ALL = a_NETSMTP_EXT_EHLO | a_NETSMTP_EXT_8BITMIME |
			a_NETSMTP_EXT_PIPELINING | a_NETSMTP_EXT_STARTTLS | a_NETSMTP_EXT_AUTH,
	...
	a_NETSMTP_ALL_MASK = a_NETSMTP_EXT_ALL | a_NETSMTP_FORCE_TLS_IFF | mx_CRED_PROTO_AUTHTYPES_SMTP,
	a_NETSMTP_ALL_AVAILABLE_MASK = a_NETSMTP_EXT_ALL | a_NETSMTP_FORCE_TLS_IFF | mx_CRED_PROTO_AUTHTYPES_AVAILABLE_SMTP,
	a_NETSMTP_ALL_AVAILABLE_AUTO_MASK = a_NETSMTP_EXT_ALL | a_NETSMTP_FORCE_TLS_IFF |
			(mx_CRED_PROTO_AUTHTYPES_AVAILABLE_SMTP & mx_CRED_AUTHTYPE_MECH_AUTO_MASK),
...
static int a_netsmtp_read(struct mx_socket *sop, struct a_netsmtp_ctx *nscp, int val, boole ign_eof, boole want_dat);
static void a_netsmtp_parse_caps(struct a_netsmtp_ctx *nscp, char const *cp);

/* Talk to a SMTP server */
static boole a_netsmtp_talk(struct mx_socket *sop, struct mx_send_ctx *scp, struct a_netsmtp_ctx *nscp);
...
do if(!(n_poption & n_PO_D)){\
	...
	\
	__x__ = (X);\
	if((__y__ = a_netsmtp_read(sop, nscp, __x__, IGNEOF, WANTDAT)) != __x__ && (!(IGNEOF) || __y__ != -1))\
		goto jleave;\
}while(0)
...
static int
a_netsmtp_read(struct mx_socket *sop, struct a_netsmtp_ctx *nscp, int val, boole ign_eof, boole want_dat){
	char *cp;
	int rv, len;
	NYD_IN;

	do{
		if((len = mx_socket_getline(&nscp->nsc_buf.s, &nscp->nsc_buf.l, NIL, sop)) < 6){
			if(len >= 0 && !ign_eof)
...
a_netsmtp_parse_caps(struct a_netsmtp_ctx *nscp, char const *cp){
	...
	if(!su_cs_cmp(cp, "8BITMIME"))
		...
		for(buf = savestr(&cp[sizeof("AUTH ") -1]); (cp = su_cs_sep_c(&buf, ' ', TRU1)) != NIL;){
...
static boole
a_netsmtp_talk(struct mx_socket *sop, struct mx_send_ctx *scp, /* TODO split*/ struct a_netsmtp_ctx *nscp){
	enum{
		a_ERROR = 1u<<0,
	...
	if(nscp->nsc_config & a_NETSMTP_EXT_EHLO){
		...
		a_SMTP_OUT(o);
		if(!(n_poption & n_PO_D)){
			nscp->nsc_server_config = a_NETSMTP_EXT_READ_IS_HOT | a_NETSMTP_EXT_EHLO;
...
#ifdef mx_HAVE_TLS
					| (sop->s_use_tls ? a_NETSMTP_EXT_NONE : a_NETSMTP_EXT_STARTTLS | a_NETSMTP_FORCE_TLS)
#endif
			)))
				goto jhelo;
			goto je_ehlo;
...
je_ehlo:
		n_err(_("*smtp-config* settings do not work out; server communication can be viewed via 2x *verbose*\n"));
		goto jleave;
	}
...
/* In a session state; if unencrypted, try to upgrade */
	...
	if(!(nscp->nsc_server_config & a_NETSMTP_EXT_AUTH) || (n_poption & n_PO_D))
		goto jsend;
	if(n_poption & n_PO_D_V)
...
#define a_MAX (2 + sizeof("AUTH PLAIN " NETNL))
		...
		if(scp->sc_credp->cc_pass.l >= (j = UZ_MAX - a_MAX) || i >= (j -= scp->sc_credp->cc_pass.l)){
jerr_cred:
...
#undef a_MAX
		...
		/* C99 */{
			...
			if((s = snprintf(o, sizeof o, "%c%s%c%s",
					'\0', scp->sc_credp->cc_user.s, '\0', scp->sc_credp->cc_pass.s)) < 0)
				goto jerr_cred;
	...
	case mx_CRED_AUTHTYPE_XOAUTH2:{
		...
		if(!mx_oauthbearer_create_icr(&sb, i, scp->sc_urlp, scp->sc_credp, ((f & a_IS_OAUTHBEARER) == 0)))
			goto jerr_cred;
...
#undef a_MAX
		...
		mx_b64_enc_buf(&sb, scp->sc_credp->cc_user.s, scp->sc_credp->cc_user.l, mx_B64_BUF | mx_B64_CRLF);
		a_SMTP_OUT(o);
		break;
	...
	case mx_CRED_AUTHTYPE_LOGIN:
		...
		if(mx_b64_enc_buf(&sb, scp->sc_credp->cc_user.s, scp->sc_credp->cc_user.l,
					mx_B64_AUTO_ALLOC | mx_B64_CRLF) == NIL)
			goto jleave;
		a_SMTP_OUT(sb.s);
		a_SMTP_ANSWER(3, FAL0, FAL0);

		if(mx_b64_enc_buf(&sb, scp->sc_credp->cc_pass.s, scp->sc_credp->cc_pass.l,
					mx_B64_AUTO_ALLOC | mx_B64_CRLF) == NIL)
			goto jleave;
		a_SMTP_OUT(sb.s);
		break;
	...
	case mx_CRED_AUTHTYPE_CRAM_MD5:{
		...
		if((cp = mx_md5_cram_string(&scp->sc_credp->cc_user, &scp->sc_credp->cc_pass, nscp->nsc_dat.s)) == NIL)
			goto jerr_cred;
		a_SMTP_OUT(cp);
		}break;
#endif
	...
	/* Complete authentication; since we assume everything is fine, just pipeline the final 2 "success" answer of
	 * the server already */
	++resp2_cnt;
	if(!(nscp->nsc_server_config & a_NETSMTP_EXT_PIPELINING))
		a_SMTP_ANSWER(2, FAL0, FAL0);
...
jsend:
	...
	if(nscp->nsc_server_config & a_NETSMTP_EXT_8BITMIME){
		...
		sb.s = UNCONST(char*,(scp->sc_hp->h_flags & HF_MESSAGE_8BITMIME) ? " BODY=8BITMIME" : " BODY=7BIT");
	}else if(scp->sc_hp->h_flags & HF_MESSAGE_8BITMIME){
		n_err(_("*smtp-config*: \"8bitmime\" extension needed, but not used/available!\n"));
		goto jleave;
	}
...
mx_smtp_parse_config(struct mx_cred_ctx *credp, struct mx_url *urlp){
	static struct capdis{
		...
		{(a_NETSMTP_EXT_AUTH | (mx_CRED_PROTO_AUTHTYPES_SMTP & mx_CRED_PROTO_AUTHTYPES_AVAILABLE_SMTP &
...
jeavail:
	...
	if(flags & a_NETSMTP_EXT_AUTH){
		...
		if((rv = mx_cred_auth_type_verify_bits(&cavc, ((urlp->url_flags & mx_URL_TLS_REQUIRED) != 0 ||
			...
			if(flags & mx_CRED_AUTHTYPE_NEED_TLS)
				flags |= a_NETSMTP_EXT_STARTTLS | a_NETSMTP_EXT_EHLO | a_NETSMTP_FORCE_TLS;
			rv = TRU1;
		}
	}
