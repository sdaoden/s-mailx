...
(savecatsep)(char const *s1, char sep, char const *s2  su_DVL_LOC_ARGS_DECL){
	...
	l1 = (s1 != NIL) ? su_cs_len(s1) : 0;
	l2 = su_cs_len(s2);
	news = su_MEM_BAG_SELF_AUTO_ALLOC_LOCOR(l1 + (sep != '\0') + l2 +1, su_DVL_LOC_ARGS_ORUSE);
	if(l1 > 0){
...
FL struct str *
(str_concat_cpa)(struct str *self, char const * const *cpa, char const *sep_o_nil  su_DVL_LOC_ARGS_DECL){
	char *cp;
...
n_re_could_be_one_buf(char const *buf, uz len){
	...
	rv = (su_cs_first_of_cbuf_cbuf(buf, len, "^[*+?|$", su_UZ_MAX) != su_UZ_MAX);
...
FL struct str *
(n_str_assign_buf)(struct str *self, char const *buf, uz buflen  su_DVL_LOC_ARGS_DECL){
	NYD_IN;

	/* Special special case */
	if(buflen == UZ_MAX)
		buflen = (buf == NIL) ? 0 : su_cs_len(buf);

	ASSERT(buflen == 0 || buf != NIL);

	if(LIKELY(buflen > 0)){
		self->s = su_MEM_REALLOC_LOCOR(self->s, (self->l = buflen) +1, su_DVL_LOC_ARGS_ORUSE);
...
FL struct str *
(n_str_add_buf)(struct str *self, char const *buf, uz buflen  su_DVL_LOC_ARGS_DECL){
	NYD_IN;
	...
	if(buflen > 0) {
		...
		self->s = su_MEM_REALLOC_LOCOR(self->s, (self->l = nsl) +1, su_DVL_LOC_ARGS_ORUSE);
...
(n_string_reserve)(struct n_string *self, uz noof  su_DVL_LOC_ARGS_DECL){
	...
	if((i = s - l) <= ++noof){
		...
		if(!self->s_auto)
			self->s_dat = su_MEM_REALLOC_LOCOR(self->s_dat, i, su_DVL_LOC_ARGS_ORUSE);
		else{
			char *ndat;
...
FL struct n_string *
(n_string_push_buf)(struct n_string *self, char const *buf, uz buflen  su_DVL_LOC_ARGS_DECL){
	NYD_IN;
	ASSERT(self != NIL);
	ASSERT(buflen == 0 || buf != NIL);

	if(buflen == UZ_MAX)
		buflen = su_cs_len(buf);
...
FL struct n_string *
(n_string_unshift_buf)(struct n_string *self, char const *buf, uz buflen  su_DVL_LOC_ARGS_DECL){
	NYD_IN;
	ASSERT(self != NIL);
	ASSERT(buflen == 0 || buf != NIL);
...
FL struct n_string *
(n_string_insert_buf)(struct n_string *self, uz idx, char const *buf, uz buflen  su_DVL_LOC_ARGS_DECL){
	NYD_IN;
	ASSERT(self != NIL);
	ASSERT(buflen == 0 || buf != NIL);
	...
	if(buflen > 0){
		self = (n_string_reserve)(self, buflen  su_DVL_LOC_ARGS_USE);
		if(self->s_len > 0)
			su_mem_move(&self->s_dat[idx + buflen], &self->s_dat[idx], self->s_len - idx);
...
FL struct n_string *
(n_string_insert_c)(struct n_string *self, uz idx, char c  su_DVL_LOC_ARGS_DECL){
	NYD_IN;
	ASSERT(self != NIL);
	ASSERT(idx <= self->s_len);
...
n_string_cut(struct n_string *self, uz idx, uz len){
	...
	if(len > 0)
		su_mem_move(&self->s_dat[idx], &self->s_dat[idx + len], (self->s_len -= len) - idx);
...
n_string_cp_const(struct n_string const *self){
	...
	if(self->s_size != 0){
		UNCONST(struct n_string*,self)->s_dat[self->s_len] = '\0';
		rv = self->s_dat;
	}else
		rv = su_empty;
