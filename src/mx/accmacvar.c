...
#define a_AMV_HAVE_LOPTS_AKA_LOCAL() /* TODO */\
   (/*mx_go_ctx_is_macro() || a_amv_on_folder_open_lopts != NIL || */\
...
/* TODO We really deserve localopts support for *on-folder-open*, so hack it in
 * TODO today via a static lostack, it should be a field in mailbox, once that
 * TODO is a real multi-instance object */
static struct a_amv_var *a_amv_on_folder_open_lopts;
...
FL boole
temporary_on_folder_open(boole only_new_mail_check){ /* TODO v15: drop */
   struct{
      char const name[22];
      u16 ok;
   } const looks[2] = {
      {"on-folder-open-", ok_v_on_folder_open},
      {"on-folder-newmail-", ok_v_on_folder_newmail}
   };
   struct a_amv_mac_call_args *amcap;
   struct a_amv_mac *amp;
   char const *mn, *cp;
   ...
   var = su_AUTO_ALLOC((len * 2) + sizeof("on-folder-newmail-") -1 +2);
...
jredo:
   ...
   if(mn == mailname && displayname[0] == '+'){
      for(cp = &mailname[len]; cp != mailname; --cp)
         if(cp[-1] == '/'){
            char *x;
            mn = x = &var[sizeof("on-folder-newmail-") -1 + len + 1];
            x[0] = '+';
            su_cs_pcopy(&x[1], cp);
            goto jredo;
         }
   ...
   /* Plain *on-folder-XY* is our last try */
   v15_compat = TRU1;
...
jmac:
   ...
   if(only_new_mail_check){
      if(v15_compat)
         amcap->amca_unroller = &a_amv_on_folder_open_lopts;
...
FL void
temporary_on_folder_close(void){ /* XXX intermediate hack */
   NYD_IN;

   if(a_amv_on_folder_open_lopts != NIL){
      void *save = a_amv_lopts;

      a_amv_lopts = NIL;
      a_amv_lopts_unroll(&a_amv_on_folder_open_lopts);
      ASSERT(a_amv_on_folder_open_lopts == NIL);
      a_amv_lopts = save;
   }
