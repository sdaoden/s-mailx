...
a_main_startup(char const *argv0){ /* {{{ */
	...
	su_state_create(su_STATE_CREATE_V1, argv0, (su_STATE_LOG_SHOW_LEVEL | /*su_STATE_LOG_SHOW_PID |*/ n_LOG_LEVEL),
		su_STATE_ERR_NOPASS);
...
# endif
	...
	if((pwuid = getpwuid(n_user_id = getuid())) == NIL)
		...
		/* */
		if(!(doenv = (ep = ok_vlook(LOGNAME)) == NIL) && (doenv = (su_cs_cmp(pwuid->pw_name, ep) != 0)))
	...
	/* This is not automated just as $TMPDIR is for the initial setting, since we have the pwuid at hand and can
	 * simply use it!  See accmacvar.c! */
...
a_main_setup_vars(void){
	...
	/* Are we in a reproducible-builds.org environment?  That special mode bends some settings (again) */
...
a_main_o_r(struct a_main_ctx *mcp, struct su_avopt *avop){
	...
	/* TODO -r options is set in n_smopts, but may be overwritten by setting from= in an interactive session!
	 * TODO Maybe disable setting of from?  Warn user?  Update manual!! */
...
a_main_usage(FILE *fp){
	...
	fprintf(fp, _(
			...
			". -b, -c, -r, -T, to-addr: ex@am.ple or '(Lovely) Ex <am@p.le>' (-Sfullnames)\n"
	...
	fprintf(fp, _(
			...
			". Bugs/Contact via \"$ %s -Sexpandaddr=shquote '\\$contact-mail'\"\n"),
...
main(int argc, char *argv[]){
	...
	su_avopt_setup(&avo, (argc != 0 ? --argc : argc), C(char const*const*,++argv), a_sopts, a_lopts);
	...
	while((i = su_avopt_parse(&avo)) != su_AVOPT_STATE_DONE){
		...
		case 'f':
			...
			/* User is specifying file to "edit" with Mail, as opposed to reading system mailbox.  If no
			 * argument is given, we read his mbox file.  Check for remaining arguments later */
			n_poption |= n_PO_f_FLAG;
			mc.mc_folder = "&";
			break;
		case 'H':
		...
		case 'O':
			...
			if(n_smopts_cnt == mc.mc_smopts_size)
				mc.mc_smopts_size = a_main_grow_cpp(&n_smopts, mc.mc_smopts_size + 8, n_smopts_cnt);
		...
		case 'X':
			...
			if(mc.mc_X_cnt == mc.mc_X_size)
				mc.mc_X_size = a_main_grow_cpp(&mc.mc_X, mc.mc_X_size + 8, mc.mc_X_cnt);
		...
		case 'Y':
			...
			if(mc.mc_Y_cnt == mc.mc_Y_size)
				mc.mc_Y_size = a_main_grow_cpp(&mc.mc_Y, mc.mc_Y_size + 8, mc.mc_Y_cnt);
...
jgetopt_done:
	...
	/* ...BUT, since we use su_AUTO_ALLOC() for the MTA n_smopts storage we need to allocate the space for them
	 * before we fixate that storage! */
	while(argv[i] != NIL)
		++i;
	...
	if(n_psonce & n_PSO_SENDMODE){
		...
		if((n_poption & (n_PO_EXISTONLY | n_PO_HEADERSONLY)) == (n_PO_EXISTONLY | n_PO_HEADERSONLY)){
	...
	if(resfiles & a_RF_MASK){
		/* *expand() returns a savestr(), but load() only uses the filename for fopen(): safe to do this */
		if(resfiles & a_RF_SYSTEM){
			boole nload;

			if((nload = ok_blook(NAIL_NO_SYSTEM_RC)))
				n_OBSOLETE(_("Please use $MAILX_NO_SYSTEM_RC instead of $NAIL_NO_SYSTEM_RC"));
			if(!nload && !ok_blook(MAILX_NO_SYSTEM_RC) && !mx_go_load_rc(ok_vlook(system_mailrc)))
				goto jleave;
		}
	...
	if((cp != NIL || (cp = ok_vlook(mailx_extra_rc)) != NIL) &&
			(cp = mx_fexpand(cp, mx_FEXP_DEF_LOCAL_FILE_VAR)) != NIL && !mx_go_load_rc(cp))
		goto jleave;
...
je_expandargv:
	...
	if(!(n_psonce & n_PSO_SENDMODE))
		...
		if(mc.mc_Y_cnt > 0 && !mx_go_load_lines(TRU1, mc.mc_Y, mc.mc_Y_cnt))
			...
			n_mail((((n_psonce & n_PSO_INTERACTIVE) ? n_MAILSEND_HEADERS_PRINT : 0) |
...
jleave:
	if(n_exit_status == su_EX_OK && (n_psonce & n_PSO_SEND_ERROR) && ok_blook(posix))
