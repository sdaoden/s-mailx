...
#define EM n_CMD_ARG_EM
    ...
    * Entries in column 0 may be missorted due to POSIX abbrev requirements
    * (or are obsolete) */
{ "next", &c_next, (A | TNDMLST), 0, MMNDEL, NIL
     DS(N_("Goes to the next message (-list) and prints it")) },

   { n_qm, &a_ctab_c_help, (G | M | X | TWYSH), 0, 1, NIL
     DS(N_("Show help [[Option] for the given command]]")) },
   { n_em, &c_shell, (M | SC | V | X | EM | TRAWDAT), 0, 0, NIL
...
{ "headers", &c_headers, (A | M | TMSGLST), 0, MMNDEL, NIL
   ...
   { "list", &a_ctab_c_list, (M | TWYSH), 0, 1, NIL
...
{ "xit", &c_exit, (M | X | TWYSH), 0, 1, NIL
   ...
   /* (Temporarily) Obsolete stuff */

#ifdef mx_HAVE_IMAP
   { "cache", &c_cache, (A | TMSGLST), 0, 0, NIL
     DS(N_("Read specified <message list> into the IMAP cache")) },
   { "connect", &c_connect, (A | TSTRING), 0, 0, NIL
     DS(N_("If disconnected, connect to IMAP mailbox")) },
   { "disconnect", &c_disconnect, (A | TNDMLST), 0, 0, NIL
     DS(N_("If connected, disconnect from IMAP mailbox")) },
   { "imap", &c_imap_imap, (A | TSTRING), 0, MAC, NIL
     DS(N_("Send command strings directly to the IMAP server")) },
   { "imapcodec", &c_imapcodec, (G | M | V | X | TRAWDAT), 0, 0, NIL
