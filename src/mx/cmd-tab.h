...
#define EM mx_CMD_ARG_EM
	...
	{"?", &a_cmd_c_help, (HG | M | X | TWYSH), 0, 1, NIL,
	 N_("Show help [[Option] for the given command]]")},
	{"|", &c_pipe, (A | TARG), 0, MMNDEL, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_pipe),
	 N_("Pipe [<msglist>] to [<command>], honour `ignore' / `retain'")},

{"alias", &c_alias, (M | TWYSH), 0, MAC, NIL,
	 N_("Show all (or [-] <alias>), or append to <alias> :<data>:")},
	...
	{"alternates", &c_alternates, (M | V | TWYSH), 0, MAC, NIL,
	 N_("Show or define alternate <address-list> for the invoking user")},
	...
	{"bind",
	 ...
	 N_("For [<context> (base)], [<show>] or bind <key[:,key:]> [:<data>:]")},

{"copy", &c_copy, (A | M | EM | TARG), 0, 0, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_copy),
	 N_("Copy [<msglist>], but do not mark them for deletion")},
	{"cache",
#ifdef mx_HAVE_IMAP
	 &c_cache,
#else
	...
	{"call_if", &c_call_if, (L | LNMAC | M | X | EM | TARG), 0, 0, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_call),
	 N_("Call macro <name> like `call', but be silent if non-existent")},
	{"cd", &c_chdir, (M | X | TWYSH), 0, 1, NIL,
	 N_("Change working directory to the specified/the login directory")},
	{"certsave",
#ifdef mx_HAVE_TLS
	 &c_certsave,
#else
...
{"chdir", &c_chdir, (M | TWYSH), 0, 1, NIL,
	 N_("Change CWD to the specified/the login directory")},
	...
	{"commandalias", &c_commandalias, (M | X | TWYSH), 0, MAC, NIL,
	 N_("Print/create command <alias> [<command>], or list all aliases")},
	{"connect",
#ifdef mx_HAVE_IMAP
	 &c_connect,
#else
...
#endif
	 (A | TSTRING), 0, 0, NIL,
	 N_("If disconnected, connect to IMAP mailbox")},
	{"Copy", &c_Copy, (A | M | SC | EM | TARG), 0, 0, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_Copy),
	 N_("Like `copy', but derive filename from first sender")},
	{"collapse", &c_collapse, (A | TMSGLST), 0, 0, NIL,
	 N_("Collapse thread views for <msglist>")},
	{"csop",
...
{"discard", &c_ignore, (M | TWYRA), 0, MAC, NIL,
	 N_("Add <headers> to the ignored LIST, or show that list")},
	{"Decrypt", &c_Decrypt, (A | M | SC | TARG), 0, 0, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_Decrypt),
	 N_("Like `decrypt', but derive filename from first sender")},
	{"decrypt", &c_decrypt, (A | M | TARG), 0, 0, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_decrypt),
	 N_("Like `copy', but decrypt first, if encrypted")},
	{"define", &c_define, (M | X | TWYSH), 0, 2, NIL,
	 N_("Define a <macro {>, or list <macro> or all existing ones")},
	{"digmsg", &c_digmsg, (HG | M | X | EM | TARG), 0, 0, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_digmsg),
...
{"edit", &c_edit, (HG | A | I | S | TMSGLST), 0, MMNORM, NIL,
	...
	{"echo", &c_echo, (HG | M | V | X | EM | TWYSH), 0, MAC, NIL,
	 N_("Echo arguments, and a trailing newline, to standard output")},
...
{"else", &c_else, (HG | F | M | X | EM | TWYSH), 0, 0, NIL,
	...
	{"environ", &c_environ, (HG | L | M | V | X | EM | TWYSH), 2, MAC, NIL,
	 N_("<link|unlink|set|unset> :<variable>:, or [vput] lookup <var>")},
	{"errors",
	...
	{"exit", &c_exit, (M | X | TWYSH), 0, 1, NIL,
	 N_("Immediately return [<status>] to the shell without saving")},

{"Followup", &c_Followup, (A | I | L | LNMAC | R | SC | EM | TMSGLST), 0, MMNDEL, NIL,
	 N_("Like `Reply', but derive filename from first sender")},
{"from", &c_from, (A | TMSGLST), 0, MMNORM, NIL,
	 N_("Type (matching) headers of <msglist> (a search specification)")},
	{"File", &c_File, (M | T | TWYRA), 0, 1, NIL,
	 N_("Open a new mailbox readonly, or show the current mailbox")},
	{"file", &c_file, (M | T | TWYRA), 0, 1, NIL,
	 N_("Open a new <mailbox> or show the current one")},
	{"filetype", &c_filetype, (M | TWYSH), 0, MAC, NIL,
	 N_("Create [:<extension> <load-cmd> <save-cmd>:] or list file handlers")},
...
{"followup", &c_followup, (A | I | L | LNMAC | R | SC | TMSGLST), 0, MMNDEL, NIL,
	...
	{"Forward", &c_Forward, (A | I | L | LNMAC | R | SC | EM | TARG), 0, MMNDEL, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_Forward),
	 N_("Like `forward', but derive filename from <address>")},
	{"forward", &c_forward, (A | I | L | LNMAC | R | SC | EM | TARG), 0, MMNDEL, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_forward),
...
{"followupsender", &c_followupsender, (O | A | I | L | LNMAC | R | SC | TMSGLST), 0, MMNDEL, NIL,
 N_("Like `Followup', but always reply to the sender only")},
{"fwddiscard", &c_fwdignore, (O | M | TRAWLST), 0, MAC, NIL,
 N_("Obsoleted by `headerpick'")},
...
{"ghost", &c_commandalias, (O | M | X | TWYRA), 0, MAC, NIL,
 N_("Obsoleted by `commandalias'")},

{"headers", &c_headers, (A | M | TMSGLST), 0, MMNDEL, NIL,
	 N_("Type a page of headers (with the first of <msglist> if given)")},
	...
	{"hold", &c_preserve, (A | SC | W | TMSGLST), 0, MMNDEL, NIL,
	 N_("Save <msglist> in system mailbox instead of *MBOX*")},
	...
	{"imapcodec",
	 ...
	 N_("IMAP mailbox name <e[ncode]|d[ecode]> <rest-of-line>")},
	...
	{"list", &a_cmd_c_list, (M | TWYSH), 0, 0, NIL,
	 N_("List all commands (in lookup order)")},
{"localopts", &c_localopts, (O | M | X | NMAC | NOHIST | TWYSH), 1, 2, NIL,
 N_("Localize variable modifications? [<attribute>] <boolean>")},
	{"Lreply", &c_Lreply, (A | I | L | LNMAC | R | SC | EM | TMSGLST), 0, MMNDEL, NIL,
	 N_("Mailing-list reply to the given <msglist>")},

	{"Mail", &c_Sendmail, (I | L | LNMAC | M | R | SC | EM | TSTRING), 0, 0, NIL,
	 N_("Like `mail', but derive filename from first recipient")},
	{"mail", &c_sendmail, (I | L | LNMAC | M | R | SC | EM | TSTRING), 0, 0, NIL,
	 N_("Compose mail; recipients may be given as arguments")},
	{"mailcap",
	...
	{"mbox", &c_mboxit, (A | W | TMSGLST), 0, MMNDEL, NIL,
	 N_("Indicate that <msglist> is to be stored in *MBOX*")},
	...
	{"More", &c_More, (A | TMSGLST), 0, MMNDEL, NIL,
	 N_("Invoke the pager on the given messages")},
	{"more", &c_more, (A | TMSGLST), 0, MMNDEL, NIL,
	 N_("Invoke the pager on the given messages")},
	{"Move", &c_Move, (A | M | SC | EM | TARG), 0, 0, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_Move),
	 N_("Like `move', but derive filename from first sender")},
	{"move", &c_move, (A | M | EM | TARG), 0, 0, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_move),
...
{"Mv", &c_Move, (O | A | M | SC | TARG), 0, 0, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_Move),
 N_("Like `move', but derive filename from first sender")},
{"mv", &c_move, (O | A | M | TARG), 0, 0, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_move),
 N_("Like `copy', but mark messages for deletion")},

{"next"/*--MKTAB-DFL--*/, &c_next, (A | TNDMLST), 0, MMNDEL, NIL,
	 N_("Goes to the next message (-list) and prints it")},
	{"netrc",
#ifdef mx_HAVE_NETRC
	 &c_netrc,
#else
	...
	{"noop", &c_noop, (A | M | TWYSH), 0, 0, NIL,
	 N_("NOOP command if current `file' is accessed via network")},

{"Print", &c_Type, (A | TMSGLST), 0, MMNDEL, NIL,
	 N_("Like `print', but bypass `ignore' / `retain'")},
{"print", &c_type, (A | TMSGLST), 0, MMNDEL, NIL,
	 N_("Type all messages of <msglist>, honouring `ignore' / `retain'")},
	{"Page", &c_More, (A | TMSGLST), 0, MMNDEL, NIL,
	 N_("Invoke the pager on the given messages")},
	{"page", &c_more, (A | TMSGLST), 0, MMNDEL, NIL,
	 N_("Invoke the pager on the given messages")},
	{"Pipe", &c_Pipe, (A | TARG), 0, MMNDEL, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_pipe),
	 N_("Like `pipe', but do not honour `ignore' / `retain'")},
	{"pipe", &c_pipe, (A | TARG), 0, MMNDEL, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_pipe),
	 N_("Pipe [<msglist>] to [<command>], honour `ignore' / `retain'")},
	...
	{"quit", &c_quit, TWYSH, 0, 1, NIL,
	 N_("Exit session with [<status>], saving messages as necessary")},

{"reply", &c_reply, (A | I | L | LNMAC | R | SC | EM | TMSGLST), 0, MMNDEL, NIL,
	 N_("Reply to originator and recipients of <msglist>")},
	...
	{"remove", &c_remove, (M | TWYSH), 1, MAC, NIL,
	 N_("Remove the named folders")},
	{"rename", &c_rename, (M | TWYSH), 2, 2, NIL,
	 N_("Rename <existing-folder> to <new-folder>")},
	{"Reply", &c_Reply, (A | I | L | LNMAC | R | SC | EM | TMSGLST), 0, MMNDEL, NIL,
	 N_("Reply to originator, exclusively")},
	{"Resend", &c_Resend, (A | L | LNMAC | R | SC | EM | TARG), 0, MMNDEL, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_Resend),
	 N_("Like `resend', but do not add Resent-* header lines")},
	{"resend", &c_resend, (A | L | LNMAC | R | SC | EM | TARG), 0, MMNDEL, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_resend),
	 N_("Resend <msglist> to <user>, add Resent-* header lines")},
	{"Respond", &c_Reply, (A | I | L | LNMAC | R | SC | EM | TMSGLST), 0, MMNDEL, NIL,
	 N_("Reply to originator, exclusively")},
	{"respond", &c_reply, (A | I | L | LNMAC | R | SC | EM | TMSGLST), 0, MMNDEL, NIL,
	 N_("Reply to originators and recipients of <msglist>")},
	{"retain", &c_retain, (M | TWYRA), 0, MAC, NIL,
	 N_("Add <headers> to retained list, or show that list")},
	{"return", &c_return, (M | X | EM | NMAC |TWYSH), 0, 2, NIL,
	 N_("Return control from macro [<return value> [<exit status>]]")},
{"replyall", &c_replyall, (O | A | I | L | LNMAC | R | SC | EM | TMSGLST), 0, MMNDEL, NIL,
 N_("Reply to originator and recipients of <msglist>")},
{"replysender", &c_replysender, (O | A | I | L | LNMAC | R | SC | EM | TMSGLST), 0, MMNDEL, NIL,
 N_("Reply to originator, exclusively")},
{"respondall", &c_replyall, (O | A | I | L | LNMAC | R | SC | EM | TMSGLST), 0, MMNDEL, NIL,
 N_("Reply to originators and recipients of <msglist>")},
{"respondsender", &c_replysender, (O | A | I | L | LNMAC | R | SC | EM | TMSGLST), 0, MMNDEL, NIL,
 N_("Reply to originator, exclusively")},
...
{"set", &c_set, (HG | L | M | X | TWYRA), 0, MAC, NIL,
	...
	{"seen", &c_seen, (A | M | TMSGLST), 0, MMNDEL, NIL,
	 N_("Mark <msglist> as seen")},
	{"setdot", &c_setdot, (A | HG | EM | TARG), 0, MMNDEL, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_setdot),
	 N_("Set \"dot\" to the given <msg>")},
{"shell", &c_dosh, (I | S | EM | TWYSH), 0, 0, NIL,
	 N_("Invoke an interactive shell")},
	...
	{"size", &c_messize, (A | TMSGLST), 0, MMNDEL, NIL,
	 N_("Show size in bytes for <msglist>")},
...
{"source", &c_source, (M | TWYSH), 1, 1, NIL,
	...
	{"spamclear",
	 ...
	 N_("Clear the spam flag for each message in <msglist>")},
	{"spamforget",
#ifdef mx_HAVE_SPAM
	 &c_spam_forget,
#else
...
#endif
	 (A | M | TMSGLST), 0, MMNDEL, NIL,
	 N_("Force the spam detector to unlearn <msglist>")},
	{"spamham",
#ifdef mx_HAVE_SPAM
	 &c_spam_ham,
#else
...
#endif
	 (A | M | TMSGLST), 0, MMNDEL, NIL,
	 N_("Teach the spam detector that <msglist> is ham")},
	{"spamrate",
#ifdef mx_HAVE_SPAM
	 &c_spam_rate,
#else
...
#endif
	 (A | M | TMSGLST), 0, MMNDEL, NIL,
	 N_("Rate <msglist> via the spam detector")},
	{"spamset",
#ifdef mx_HAVE_SPAM
	 &c_spam_set,
#else
...
#endif
	 (A | M | TMSGLST), 0, MMNDEL, NIL,
	 N_("Set the spam flag for each message in <msglist>")},
	{"spamspam",
#ifdef mx_HAVE_SPAM
	 &c_spam_spam,
#else
...
#endif
	 (A | M | TMSGLST), 0, MMNDEL, NIL,
	 N_("Teach the spam detector that <msglist> is spam")},
...
{"Type", &c_Type, (A | TMSGLST), 0, MMNDEL, NIL,
	 N_("Like `type', but bypass `ignore' / `retain'")},
{"type", &c_type, (A | TMSGLST), 0, MMNDEL, NIL,
	 N_("Type all messages of <msglist>, honouring `ignore' / `retain'")},
	{"tls",
#ifdef mx_HAVE_TLS
	...
	{"Top", &c_Top, (A | TMSGLST), 0, MMNDEL, NIL,
	 N_("Like `top', but bypass `ignore' / `retain'")},
	{"top", &c_top, (A | TMSGLST), 0, MMNDEL, NIL,
	 N_("Type first *toplines* of all messages in <msglist>")},
	{"touch", &c_stouch, (A | W | TMSGLST), 0, MMNDEL, NIL,
	 N_("Mark <msglist> for saving in *mbox*")},
{"thread", &c_thread, (O | A | TMSGLST), 0, 0, NIL,
 N_("Obsoleted by `sort thread'")},

{"undelete", &c_undelete, (A | P | TMSGLST), MDELETED, MMNDEL, NIL,
	 N_("Un`delete' <msglist>")},
{"unalias", &c_unalias, (M | TWYSH), 1, MAC, NIL,
	...
	{"uncollapse", &c_uncollapse, (A | TMSGLST), 0, 0, NIL,
	 N_("Uncollapse <msglist> if in threaded view")},
	{"uncolour",
#ifdef mx_HAVE_COLOUR
	 &c_uncolour,
#else
	...
	{"unfiletype", &c_unfiletype, (M | TWYSH), 1, MAC, NIL,
	 N_("Delete file handler for [:<extension>:] (* for all)")},
...
{"unthread", &c_unsort, (O | A | TMSGLST), 0, 0, NIL,
 N_("Obsolete, use `sort none' (Disable sorted mode)")},
...
{"visual", &c_visual, (A | I | S | TMSGLST), 0, MMNORM, NIL,
	...
	{"write", &c_write, (A | TARG), 0, 0, mx_CMD_ARG_DESC_SUBCLASS_CAST(&a_cmd_cad_write),
	 N_("Write (append) [<msglist>] to [<file>]")},
