...
static char const a_colour_ctx_prefixes[mx__COLOUR_CTX_MAX1][8] = {"sum-", "view-", "mle-"};

static struct a_colour_map_id const a_colour_map_ids[mx__COLOUR_CTX_MAX1][mx__COLOUR_IDS] = {{
	...
	{mx_COLOUR_CTX_VIEW, mx_COLOUR_ID_VIEW_MSGINFO, a_COLOUR_TT_NONE, "msginfo"},
	{mx_COLOUR_CTX_VIEW, mx_COLOUR_ID_VIEW_PARTINFO, a_COLOUR_TT_NONE, "partinfo"},
	}, {
...
/* (regexpp may be NIL) */
static char const *a_colour__tag_identify(struct a_colour_map_id const *cmip, char const *ctag, void **regexpp);
...
static struct a_colour_map *a_colour_map_find(enum mx_colour_ctx cctx, enum mx_colour_id cid, char const *ctag);
...
/* Create an ISO 6429 (ECMA-48/ANSI) terminal control escape sequence from user input spec, store it or on error
 * according message in *store */
static boole a_colour_iso6429(enum a_colour_type ct, char **store, char const *spec);
...
a_colour_init(void){
	...
	su_mem_copy(a_colour_g.cg_reset.cp_dat.s = a_colour_g.cg__reset_buf, "\033[0m",
		a_colour_g.cg_reset.cp_dat.l = sizeof("\033[0m") -1); /* (calloc) */
	a_colour_g.cg_type = a_COLOUR_T_UNKNOWN;

	DVL( su_state_on_gut_install(&a_colour__on_gut, FAL0, su_STATE_ERR_NOPASS); )
...
a_colour_ok_to_go(u32 get_flags){
	...
	if(UNLIKELY(!a_colour_g.cg_is_init))
		a_colour_init();
	if(UNLIKELY(a_colour_g.cg_type == a_COLOUR_T_NONE) || !a_colour_termcap_init())
		goto jleave;
...
a_colour_mux(char const * const *argv){
	...
	if((ctag = argv[2]) != NIL && *ctag != '\0'){
		...
		if(cmip->cmi_tt == a_COLOUR_TT_NONE){
			...
			goto jleave;
		}else if((xtag = a_colour__tag_identify(cmip, ctag, &regexp)) == mx_COLOUR_TAG_ERR){
	...
	if(ctag == NIL && mapname[0] == '*' && mapname[1] == '\0'){
		...
		for(i1 = 0; i1 < mx__COLOUR_CTX_MAX1; ++i1)
			for(i2 = 0; i2 < mx__COLOUR_IDS; ++i2)
				for(cmp = *(cmap = &a_colour_g.cg_maps[ct][i1][i2]), *cmap = NIL; cmp != NIL;){
					tmp = cmp;
					cmp = cmp->cm_next;
		...
		for(i2 = 0; i2 < mx__COLOUR_IDS; ++i2){
			...
			for(; cmp != NIL; cmp = cmp->cm_next){
				...
				fprintf(n_stdout, "colour %s %-*s %s %s\n",
					...
					savecat(a_colour_ctx_prefixes[i1], a_colour_map_ids[i1][i2].cmi_name),
						&S(char const*,cmp->cm_buf)[cmp->cm_user_off],
...
static char const *
a_colour__tag_identify(struct a_colour_map_id const *cmip, char const *ctag, void **regexpp){
	NYD2_IN;
	UNUSED(regexpp);

	if((cmip->cmi_tt & a_COLOUR_TT_DOT) && !su_cs_cmp_case(ctag, "dot"))
		ctag = mx_COLOUR_TAG_SUM_DOT;
	else if((cmip->cmi_tt & a_COLOUR_TT_OLDER) && !su_cs_cmp_case(ctag, "older"))
		ctag = mx_COLOUR_TAG_SUM_OLDER;
	else if(cmip->cmi_tt & a_COLOUR_TT_HEADERS){
		char *cp, c;
		uz i;

		/* Can this be a valid list of headers? However, with regular expressions simply use the input as such
		 * if it appears to be a regex */
#ifdef mx_HAVE_REGEX
		if(n_re_could_be_one_cp(ctag) && regexpp != NIL){
			struct su_re *rep;

			if(su_re_setup_cp((rep = su_ALLOC(sizeof(struct su_re))), ctag,
					(su_RE_SETUP_EXT | su_RE_SETUP_ICASE | su_RE_SETUP_TEST_ONLY)) == su_RE_ERROR_NONE)
				*regexpp = rep;
			else{
...
#endif
		{
			...
			for(i = 0; (c = *ctag++) != '\0';){
				...
				if(!isblspc && !su_cs_is_alnum(c) && c != '-' && c != ',')
					goto jetag;
				/* Since we compare header names as they come from the message this lowercasing is
				 * however redundant: we need to casecmp() them */
				if(!isblspc)
...
a_colour_map_find(enum mx_colour_ctx cctx, enum mx_colour_id cid, char const *ctag){
	struct a_colour_map *cmp;
	NYD2_IN;
...
c_colour(void *vp){
	...
	rv = a_colour_mux(vp) ? su_EX_OK : su_EX_ERR;
...
c_uncolour(void *vp){
	...
	rv = a_colour_unmux(vp) ? su_EX_OK : su_EX_ERR;
...
mx_colour_env_gut(void){
	...
	if((cep = mx_go_data->gdc_colour) == NIL)
		goto jleave;
	mx_go_data->gdc_colour_active = ((mx_go_data->gdc_colour = cep->ce_last) != NIL && cep->ce_last->ce_enabled);
...
mx_colour_put(enum mx_colour_id cid, char const *ctag){
	...
	if(mx_COLOUR_IS_ACTIVE()){
		...
		if((cep->ce_current = a_colour_map_find(cep->ce_ctx, cid, ctag)) != NIL)
			fwrite(cep->ce_current->cm_pen.cp_dat.s, cep->ce_current->cm_pen.cp_dat.l, 1, cep->ce_outfp);
	}
...
mx_colour_pen_create(enum mx_colour_id cid, char const *ctag){
	...
	if(mx_COLOUR_IS_ACTIVE() && (cmp = a_colour_map_find(mx_go_data->gdc_colour->ce_ctx, cid, ctag)) != NIL){
...
struct mx_colour_pen *
mx_colour_get_pen(u32 get_flags, enum mx_colour_ctx cctx, enum mx_colour_id cid, char const *ctag){
	struct a_colour_map *cmp;
	struct mx_colour_pen *rv;
	NYD_IN;
	...
	if(a_colour_ok_to_go(get_flags) && (cmp = a_colour_map_find(cctx, cid, ctag)) != NIL){
