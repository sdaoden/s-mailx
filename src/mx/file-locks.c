 ...
 *
 * Copyright (c) 2015 - 2024 Steffen Nurpmeso <steffen@sdaoden.eu>.
...
static int
a_filo_main(void){
	/* Use PATH_MAX not NAME_MAX to catch those "we proclaim the minimum value" problems (SunOS), since the
	 * pathconf(3) value comes too late! */
...
jislink:
	...
	/* So we are here, but then again the file can be a symbolic link!  This is however only true if we do not have
	 * realpath(3) available since that will have resolved the path already otherwise; nonetheless, let readlink(2)
	 * be a precondition for dotlocking and keep this code */
	...
	/* Be aware, even if the error is false!  Note the shared code in file-dotlock.h *requires* that it is possible
	 * to create a filename at least one byte longer than di_lock_name! */
	/* C99 */{
		uz pc;
		int i;
...
jenametool:
			fdls = mx_FILE_DOTLOCK_STATE_NAMETOOLONG | mx_FILE_DOTLOCK_STATE_ABANDON;
			goto jmsg;
		}
	...
	/* We are in the directory of the mailbox for which we have to create a dotlock file for.  Any symbolic links
	 * have been resolved.  We do not know whether we have realpath(3) available,and manually resolving the path is
	 * due especially given that we support the special "%:" syntax to warp any file into a "system mailbox"; there
	 * may also be multiple system mailbox directories...  So what we do is that we fstat(2) the mailbox and check
	 * its UID and GID against that of our own process: if any of those mismatch we must either assume a directory
	 * we are not allowed to write in, or that we run via -u/$USER/%USER as someone else, in which case we favour
	 * our privilege-separated dotlock process */
	...
	/* So let's try and call it ourselves!  Note we do not block signals just like our privsep child does, the user
	 * will anyway be able to remove his file again, and if we are in -u/$USER mode then we are allowed to access
	 * the user's box: shall we leave behind a stale dotlock then at least we start a friendly human conversation.
	 * Since we cannot handle SIGKILL and SIGSTOP malicious things could happen whatever we do */
...
jmsg:
	...
	/* ..then eventually wait until we shall remove the lock again, which will be notified via the read returning */
...
jleave:
	...
	/* And the locker process itself; it will be a (rather cheap) thread only unless the lock has to be placed in
	 * the system spool and we have our privilege-separated dotlock program available, in which case that will be
	 * executed and do "it" */
	fdi.fdi_file_name = fname;
	fdi.fdi_retry = (flm & mx_FILE_LOCK_MODE_RETRY) ? n_1 : su_empty;
	/* Initialize some more stuff; query the two strings in the parent in order to cache the result of the former
	 * and anyway minimalize child page-ins.  Especially uname(3) may hang for multiple seconds when it is called
	 * the first time! */
	...
	for(;;){
		...
		if(fdls == mx_FILE_DOTLOCK_STATE_NONE || (fdls & mx_FILE_DOTLOCK_STATE_ABANDON))
			close(S(int,cpipe[0]));
		...
		case mx_FILE_DOTLOCK_STATE_NOPERM:
			if((n_psonce & n_PSO_INTERACTIVE) || (n_poption & n_PO_D_V))
				emsg = N_("  Cannot create a dotlock file, please check permissions\n");
			serr = su_ERR_ACCES;
			break;
		case mx_FILE_DOTLOCK_STATE_NOEXEC:
			if((n_psonce & (n_PSO_INTERACTIVE | n_PSO_PS_DOTLOCK_NOTED)) == n_PSO_INTERACTIVE ||
					(n_poption & n_PO_D_V)){
		...
		case mx_FILE_DOTLOCK_STATE_PRIVFAILED:
			emsg = N_("  Privilege-separated dotlock program cannot change privileges\n");
			serr = su_ERR_PERM;
			break;
		...
		if(emsg != NIL){
			...
			n_err(_(". failed\n%s%s"), V_(emsg), (b ? su_empty : _("Trying different policy ")));
			if(b)
				didmsg = TRU1;
			emsg = NIL;
		}
...
jleave:
	...
	if(rv == NIL){
		if(flocked){
			if(serr == su_ERR_ROFS)
				rv = R(FILE*,-1);
			else if(serr != su_ERR_AGAIN && serr != su_ERR_EXIST && ok_blook(dotlock_ignore_error)){
				n_OBSOLETE(_("*dotlock-ignore-error*: please use *dotlock-disable* instead"));
				if(n_poption & n_PO_D_V)
