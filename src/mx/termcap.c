 ...
 *
 * Copyright (c) 2016 - 2025 Steffen Nurpmeso <steffen@sdaoden.eu>.
...
/*
 * xxx We are not really compatible with very old and strange terminals since we do not care at all for circumstances
 * indicated by terminal flags: if we find a capability we use it and assume it works.  E.g., if "Co" indicates colours
 * we simply use ISO 6429 also for font attributes etc.  That is, we do not use the ncurses/terminfo interface with all
 * its internal logic.
 */

/* Unless mx_HAVE_TERMINFO or mx_HAVE_TGETENT_NIL_BUF are defined we use this value to space the buffer we pass through
 * to tgetent(3).  Since for (such) elder non-emulated terminals really weird things will happen if an entry would
 * require more than 1024 bytes, do not really mind.  Use a u16 for storage.
 *	find /usr/share/terminfo/ -type f -exec ls -latr '{}' + | awk '{print $5 $9}' | sort -n */
#define a_TERMCAP_ENTRYSIZE_MAX ((4058u + 128) & ~127u) /* ncurses 6.4 */

CTA(a_TERMCAP_ENTRYSIZE_MAX < U16_MAX, "Chosen buffer size exceeds datatype capability");

/* For simplicity we store commands and queries in single continuous control and entry structure arrays: to index
 * queries one has to add mx__TERMCAP_CMD_MAX1 first!  And do not confound with ENTRYSIZE_MAX! */
enum{
...
enum a_termcap_flags{
	a_TERMCAP_F_NONE,
	/* enum mx_termcap_captype values stored here.  Note presence of a type in an a_termcap_ent signals init */
...
struct a_termcap_control{
	u16 tc_flags;
	/* Offset base into a_termcap_namedat[], which stores the two-letter termcap(5) name directly followed by a NUL
	 * terminated terminfo(5) name.  A termcap(5) name may consist of two NULs meaning ERR_NOENT, a terminfo(5)
	 * name may be empty for the same purpose */
	u16 tc_off;
} su_CC_PACKED;
CTA(a_TERMCAP_F__LAST <= U16_MAX, "a_termcap_flags exceed storage datatype in a_termcap_control");

struct a_termcap_ent{
	u16 te_flags;
	u16 te_off; /* in a_termcap_g->tg_dat / value for T_BOOL and T_NUM */
} su_CC_PACKED;
CTA(a_TERMCAP_F__LAST <= U16_MAX, "a_termcap_flags exceed storage datatype in a_termcap_ent");

/* Extended queries without an entry constant in mx_termcap_query (to allow free query/binding of keycodes) */
...
# if !defined mx_HAVE_TGETENT_NIL_BUF && !defined mx_HAVE_TERMINFO
...
CTA(sizeof a_termcap_namedat <= U16_MAX, "Termcap command and query name data exceed storage datatype");
CTA(a_TERMCAP_ENT_MAX1 == NELEM(a_termcap_control), "Control array does not match command/query one to be controlled");
...
static boole a_termcap_ent_query(struct a_termcap_ent *tep, char const *cname, u16 cflags);
SINLINE boole a_termcap_ent_query_tcp(struct a_termcap_ent *tep, struct a_termcap_control const *tcp);

/* Output PTF for both, termcap(5) and terminfo(5) */
static int a_termcap_putc(int c);
#endif

/* Get mx_termcap_cmd or mx_termcap_query constant belonging to (nlen bytes of) name, -1 if not found.
 * min and max have to be used to cramp the result */
static s32 a_termcap_enum_for_name(char const *name, uz nlen, s32 min, s32 max);
#define a_termcap_cmd_for_name(NB,NL) a_termcap_enum_for_name(NB, NL, 0, mx__TERMCAP_CMD_MAX1)
#define a_termcap_query_for_name(NB,NL) a_termcap_enum_for_name(NB, NL, mx__TERMCAP_CMD_MAX1, a_TERMCAP_ENT_MAX1)
...
a_termcap_init_var(struct str const *termvar){
	...
	while((ccp = su_cs_sep_c(&cbp, ',', TRU1)) != NIL){
		...
		/* C99 */{
			...
			if(tci < 0){
				...
				if((f & a_TERMCAP_F_TYPE_MASK) == mx_TERMCAP_CAPTYPE_STRING){
					...
					teep = su_ALLOC(VSTRUCT_SIZEOF(struct a_termcap_ext_ent, tee_name) + kl +1);
...
jlearned:
		...
		if(n_poption & n_PO_D_VV)
			n_err(_("*termcap*: learned %.*s: %s\n"), (int)kl, ccp,
				(tep->te_flags & a_TERMCAP_F_DISABLED ? "<disabled>"
				 : (f & a_TERMCAP_F_TYPE_MASK) == mx_TERMCAP_CAPTYPE_BOOL ? "true" : v));
	}
	...
	else if(a_termcap_g->tg_ents[mx_TERMCAP_CMD_ti].te_flags & a_TERMCAP_F_DISABLED)
...
a_termcap_init_altern(void){
	/* We silently ignore user _F_DISABLED requests for those entries for which we have fallback entries, and which
	 * we need to ensure proper functioning.  This allows users to explicitly disable some termcap(5) capability
...
#define a_OOK(TEP) ((TEP)->te_flags != 0 && !((TEP)->te_flags & a_TERMCAP_F_NOENT))
#define a_SET(TEP,CMD,ALT) (TEP)->te_flags = a_termcap_control[CMD].tc_flags | ((ALT) ? a_TERMCAP_F_ALTERN : 0)

	struct a_termcap_ent *tep;
	NYD2_IN;
	UNUSED(tep);

	/* For simplicity in the rest of this file null flags of disabled commands, as we will not check and try to
	 * lazy query any command */
	/* C99 */{
		uz i;
	...
	/* nd == \033[C (we may not fail, anyway, so use xterm sequence default XXX weak fallback) */
...
SINLINE boole
a_termcap_ent_query_tcp(struct a_termcap_ent *tep, struct a_termcap_control const *tcp){
	ASSERT(a_termcap_g != NIL && !a_termcap_g->tg_disabled);

	return a_termcap_ent_query(tep, &a_termcap_namedat[tcp->tc_off] + 2, tcp->tc_flags);
}
...
# ifndef mx_HAVE_TGETENT_NIL_BUF
#  define a_BUF &a_termcap_g->tg_lib_buf[0]
# else
#  define a_BUF NIL
# endif

	rv = (tgetent(a_BUF, term) > 0);
	if(!rv)
		n_err(_("Unknown ${TERM}inal, using only *termcap*: %s\n"), term);
...
a_termcap_ent_query_tcp(struct a_termcap_ent *tep, struct a_termcap_control const *tcp){
	...
	return a_termcap_ent_query(tep, &a_termcap_namedat[tcp->tc_off], tcp->tc_flags);
}
...
mx_termcap_init(void){
	...
	n_string_reserve(n_string_creat(&a_termcap_g->tg_dat), ((termvar.l + (256 - 64)) & ~127));
	...
	if(!mx_termcap_query(mx_TERMCAP_QUERY_am, &tv) || mx_termcap_query(mx_TERMCAP_QUERY_xenl, &tv)){
		a_termcap_g->tg_fullwidth = TRU1;
		n_psonce |= n_PSO_TERMCAP_FULLWIDTH;

		/* Termcap was not initialized when we did TERMIOS_SETUP_TERMSIZE: we need/should adjust the setting to
		 * reality (without causing a synthesized SIGWINCH or something even more expensive that is) */
...
mx_termcap_destroy(void){
	...
	mx_termcap_suspend(mx_TERMCAP_MODE_TEARDOWN_MASK);
...
void
mx_termcap_resume(BITENUM(u32,mx_termcap_mode) mode){
	NYD_IN;
...
#ifdef mx_HAVE_KEY_BINDINGS
			any |= (mx_termcap_cmdx(mx_TERMCAP_CMD_BE) > FAL0);
#endif
		}
...
void
mx_termcap_suspend(BITENUM(u32,mx_termcap_mode) mode){
	NYD_IN;
	...
	if(a_termcap_g != NIL && !a_termcap_g->tg_disabled){
		...
		if(mode & mx_TERMCAP_MODE_SMART){
#ifdef mx_HAVE_KEY_BINDINGS
			any |= (mx_termcap_cmdx(mx_TERMCAP_CMD_BD) > FAL0);
#endif
		}
...
boole
mx_termcap_cmd(BITENUM(u32,mx_termcap_cmd) cmd, sz a1, sz a2){
	...
	if((flags & mx_TERMCAP_CMD_FLAG_CA_MODE) && !a_termcap_g->tg_ca_mode)
		...
		ASSERT((tep->te_flags & a_TERMCAP_F_TYPE_MASK) == mx_TERMCAP_CAPTYPE_STRING);
		...
		if(tep->te_flags & (a_TERMCAP_F_ARG_IDX1 | a_TERMCAP_F_ARG_IDX2)){
			if(a_termcap_g->tg_disabled){
				if(n_poption & n_PO_D_V){
					...
					if(cnam[2] != '\0')
						cnam += 2;
					n_err(_("*termcap-disable*d (/$TERM not set/unknown): cannot perform CAP: %s\n"),
						cnam);
				}
				goto jleave;
			}
...
mx_termcap_query(enum mx_termcap_query query, struct mx_termcap_value *tvp){
	...
	if(query != mx__TERMCAP_QUERY_MAX1){
		...
		teep = su_ALLOC(VSTRUCT_SIZEOF(struct a_termcap_ext_ent,tee_name) + nlen);
		tep = &teep->tee_super;
		teep->tee_next = a_termcap_g->tg_ext_ents;
		a_termcap_g->tg_ext_ents = teep;
...
mx_termcap_query_for_name(char const *name, enum mx_termcap_captype type){
	...
	rv = a_termcap_query_for_name(name, su_cs_len(name));
	if(rv >= 0){
		struct a_termcap_control const *tcp;

		tcp = &a_termcap_control[(u32)rv];

		if(type != mx_TERMCAP_CAPTYPE_NONE && (tcp->tc_flags & a_TERMCAP_F_TYPE_MASK) != type)
			rv = -2;
		else
...
mx_termcap_name_of_query(enum mx_termcap_query query){
	...
	rv = &a_termcap_namedat[a_termcap_control[mx__TERMCAP_CMD_MAX1 + query].tc_off + 2];
