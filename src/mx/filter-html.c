...
static struct mx_flthtml *a_flthtml_putc_premode(struct mx_flthtml *self, char c);
static struct mx_flthtml *a_flthtml_puts(struct mx_flthtml *self, char const *cp);
static struct mx_flthtml *a_flthtml_putbuf(struct mx_flthtml *self, char const *cp, uz len);

/* Try locate a param'eter in >fh_bdat, store it (non-terminated!) or NIL */
static struct mx_flthtml *a_flthtml_param(struct mx_flthtml *self, struct str *store, char const *param);

/* Expand all entities in the given parameter */
static struct mx_flthtml *a_flthtml_expand_all_ents(struct mx_flthtml *self, struct str const *param);

/* Completely parsed over a tag / an entity, interpret that */
static struct mx_flthtml *a_flthtml_check_tag(struct mx_flthtml *self, char const *s);
static struct mx_flthtml *a_flthtml_check_ent(struct mx_flthtml *self, char const *s, uz l);

/* Input handler */
static sz a_flthtml_add_data(struct mx_flthtml *self, char const *dat, uz len);
...
a_flthtml_dump_hrefs(struct mx_flthtml *self){
	...
	self->fh_flags |= (putc('\n', self->fh_os) == EOF) ? a_FLTHTML_ERROR : a_FLTHTML_NL_1 | a_FLTHTML_NL_2;
...
jput:
	...
	/* Check whether there are HREFs to dump; there is so much messy tagsoup out there that it seems best not to
	 * simply dump HREFs in each _dump(), but only with some gap, let's say half the real screen height */
	if(--self->fh_href_dist < 0 && (f & a_FLTHTML_NL_2) && self->fh_hrefs != NIL)
...
jimg_put:
	...
	case a_FLTHTML_SA_HREF:
		...
		if(param.s != NIL && *param.s != '#'){
			...
			hhp = su_ALLOC(VSTRUCT_SIZEOF(struct mx_flthtml_href,fhh_dat) + param.l +1);
			hhp->fhh_next = self->fh_hrefs;
...
jleave:
	...
	/* The problem is that even invalid tagsoup is widely used, without real searching i have seen e-mail address
	 * in <N@H.D> notation, and more.  Protect us a bit: look around and possibly write the content as such */
junknown:
	switch(*s){
	case '!':
	case '?':
...
a_flthtml_add_data(struct mx_flthtml *self, char const *dat, uz len){
	...
	for(rv = S(sz,len); len > 0; --len){
		 ...
		 * therefore prefix the a_FLTHTML_IGN test and walk until we see the required end tag */
...
jcp_reset:
				...
				if(hot){
					if((i = P2UZ(cp - self->fh_bdat)) > 1 && --i == hftp->fht_len &&
		...
		case '<':
			...
			if(f & (a_FLTHTML_NOPUT | a_FLTHTML_ENT)){
				...
				if(P2UZ(cp - self->fh_bdat) != 4 || su_mem_cmp(self->fh_bdat, "<!--", 4)){
					self->fh_flags = f;
					*cp = '\0';
					self = a_flthtml_puts(self, self->fh_bdat);
					f = self->fh_flags;
		...
		case '>':
			...
			*(cp = self->fh_bdat) = '\0'; /* xxx extra safety */
			...
			if(!(f & a_FLTHTML_PRE) && (self->fh_flags & a_FLTHTML_PRE) && len > 1 && *dat == '\n')
				++dat, --len;
			break;
...
jdo_c:
			 ...
			 * processing if it cannot be an entity XXX is_space() not enough */
				...
				self->fh_flags = f;
				if(c == ';')
					self = a_flthtml_check_ent(self, self->fh_bdat, P2UZ(cp + 1 - self->fh_bdat));
				else
					self = a_flthtml_puts(self, self->fh_bdat);
			}else{
