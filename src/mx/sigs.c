 ...
 * TODO   P.S.: note OpenSSL membio does not support in-kernel TLS.
 */

/* signal_all_ */
static uz ATOMIC a_sigs_all_depth;
static sigset_t a_sigs_all_nset, a_sigs_all_oset;

/* {hold,rele}_sigs() */
static uz ATOMIC a_sigs_hold_sigdepth;
static sigset_t a_sigs_hold_nset, a_sigs_hold_oset;
...
int
c_sleep(void *vp){ /* XXX installs sighdl+ due to outer jumps and SA_RESTART! */
	sigset_t nset, oset;
	 ...
	 * - TODO for now signal_alla_sigs_hold() most often on in robot mode,
	...
	n_pstate_err_no = (su_time_msleep(msec, ignint) > 0) ? su_ERR_INTR : su_ERR_NONE;
...
jeover:
	...
	goto jleave;
jesyn:
	mx_cmd_print_synopsis(mx_cmd_by_name_firstfit("sleep"), NIL);
...
hold_sigs(void){
	...
	if(a_sigs_hold_sigdepth++ == 0){
		sigemptyset(&a_sigs_hold_nset);
		sigaddset(&a_sigs_hold_nset, SIGHUP);
		sigaddset(&a_sigs_hold_nset, SIGINT);
		sigaddset(&a_sigs_hold_nset, SIGQUIT);
		sigprocmask(SIG_BLOCK, &a_sigs_hold_nset, &a_sigs_hold_oset);
	}
...
rele_sigs(void){
	...
	if(--a_sigs_hold_sigdepth == 0)
		sigprocmask(SIG_SETMASK, &a_sigs_hold_oset, NIL);

	NYD2_OU;
}

/* TODO This is (even more!) temporary gracyness */
...
n__sigman_enter(struct n_sigman *self, int flags){
	...
	if(flags >= 0){
		self->sm_flags = R(enum n_sigman_flags,flags);
		self->sm_signo = 0;
...
void
n_sigman_leave(struct n_sigman *self, enum n_sigman_flags reraise_flags){
	u32 f;
	int sig;
	NYD2_IN;
...
# define _X(X) X, sizeof(X) -1
	...
	if(signo != SIGUSR2){
		...
		*cp = '\0';
		i = signo;
		do{
			*--cp = "0123456789"[i % 10]; /* inline itoa */
			i /= 10;
		}while(i != 0);
