...
jwork_msg:
   ...
   if((mft = mx_header_get_mail_followup_to(mp)) != NIL){
      head.h_mft = mft = n_namelist_dup(mft, GTO | gf);
   ...
   /* Cc: */
   np = NIL;
   if(ok_blook(recipients_in_cc) && (cp = hfield1("to", mp)) != NIL)
   ...
   /* To: */
   np = ndup(head.h_mailx_eded_origin, GTO | gf);
   if(!ok_blook(recipients_in_cc) && (cp = hfield1("to", mp)) != NIL)
   ...
   if(np != NIL){
      ...
      if(count(np) == 0)
         head.h_mailx_raw_to = n_namelist_dup(head.h_mailx_eded_sender,
               GTO | gf);
   }
   head.h_to = np;
   ...
   if(hf & HF_LIST_REPLY){
      ...
      if((lpnp = mx_header_list_post_of(mp)) != NIL){
         ...
         if(the_rt != NIL && the_rt->n_flink == NIL &&
            ...
            cp = the_rt->n_name; /* rt is EACM_STRICT tested */
         else
...
a_crese_fwd(void *vp, boole recipient_record){
   ...
   struct header head;
   struct message *mp;
   struct mx_name *recp_base, *recp, *np;
   enum gfield gf;
   int *msgvec, rv;
   ...
   n_pstate_err_no = su_ERR_NONE;
   flags = (n_pstate & n_PS_ARGMOD_LOCAL) ? a_LOCAL : a_NONE;
   ...
   recp_base = lextract(cap->ca_arg.ca_str.s,
         (GTO | GNOT_A_LIST | GFULL | GSKIN));
...
jwork_msg:
   ...
   recp = ndup(recp_base, (GTO | GNOT_A_LIST | gf));
