...
/* ..of a_ft_dp */
#define a_FT_FLAGS \
	(su_CS_DICT_OWNS | su_CS_DICT_CASE | su_CS_DICT_HEAD_RESORT | su_CS_DICT_AUTO_SHRINK | su_CS_DICT_ERR_PASS)
#define a_FT_THRESHOLD 4
...
static struct mx_filetype const a_ft_OBSOLETE_xz = { /* TODO v15 compat */
	...
	"bz2", 3, "bzip2 -cd", sizeof("bzip2 -cd") -1, "bzip2 -cz", sizeof("bzip2 -cz") -1
};
...
static struct su_toolbox const a_ft_tbox = su_TOOLBOX_I9R(&a_ft_clone, &a_ft_delete, &a_ft_assign, NIL, NIL);

/* */
static struct n_strlist *a_ft_dump(char const *cmdname, char const *key, void const *dat);
...
a_ft_clone(void const *t, u32 estate){
	...
	if((rv = S(struct a_ft_dat*,su_ALLOCATE(l, 1, estate))) != NIL){
		cp = S(char*,&rv[1]);
		su_mem_copy(rv->ftd_load.s = cp, tp->ftd_load.s, (rv->ftd_load.l = tp->ftd_load.l) +1);
		cp += tp->ftd_load.l +1;
		su_mem_copy(rv->ftd_save.s = cp, tp->ftd_save.s, (rv->ftd_save.l = tp->ftd_save.l) +1);
	}
...
c_filetype(void *vp){ /* TODO support auto chains: .tar.gz -> .gz + .tar */
	...
	if(argv[1] == NIL){
		if(a_ft_dp != NIL && su_cs_dict_view_find(su_cs_dict_view_setup(&dv, a_ft_dp), key)){
			...
			slp = a_ft_dump("filetype", su_cs_dict_view_key(&dv), su_cs_dict_view_data(&dv));
		...
		if(a_ft_dp == NIL){
			a_ft_dp = su_cs_dict_set_threshold(su_cs_dict_create(&a_ft__d, a_FT_FLAGS, &a_ft_tbox),
					a_FT_THRESHOLD);
			DVL( su_state_on_gut_install(&a_ft__on_gut, FAL0, su_STATE_ERR_NOPASS); )
		}
		...
		do{ /* while(*(argv += 3) != NIL); */
			...
			ftd.ftd_load.l = l = su_cs_len(ftd.ftd_load.s = UNCONST(char*,argv[1]));
...
mx_filetype_trial(struct mx_filetype *res_or_nil, char const *file){
	...
	if(a_ft_dp != NIL){
		su_CS_DICT_FOREACH(a_ft_dp, &dv){
			...
			s = n_string_push_buf(s, su_cs_dict_view_key(&dv), su_cs_dict_view_key_len(&dv));
	...
	s = n_string_trunc(s, l);
	s = n_string_push_buf(s, a_ft_OBSOLETE_xz.ft_ext_dat, a_ft_OBSOLETE_xz.ft_ext_len);
	if(su_pathinfo_stat(&pi, n_string_cp(s)) && su_pathinfo_is_reg(&pi)){
	...
	s = n_string_trunc(s, l);
	s = n_string_push_buf(s, a_ft_OBSOLETE_gz.ft_ext_dat, a_ft_OBSOLETE_gz.ft_ext_len);
	if(su_pathinfo_stat(&pi, n_string_cp(s)) && su_pathinfo_is_reg(&pi)){
	...
	s = n_string_trunc(s, l);
	s = n_string_push_buf(s, a_ft_OBSOLETE_bz2.ft_ext_dat, a_ft_OBSOLETE_bz2.ft_ext_len);
	if(su_pathinfo_stat(&pi, n_string_cp(s)) && su_pathinfo_is_reg(&pi)){
...
#undef a_X1
		...
		if((csave != NIL) | (cload != NIL)){
			n_OBSOLETE("*file-hook-{load,save}-EXTENSION* will vanish, please use the `filetype' command");
			...
			if(((csave != NIL) ^ (cload != NIL)) == 0){
				...
				n_alert(_("Incomplete *file-hook-{load,save}-EXTENSION* for: .%s"), lext);
		}
	}
