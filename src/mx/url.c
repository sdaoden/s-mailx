...
(mx_url_xenc)(char const *cp, boole ispath  su_DVL_LOC_ARGS_DECL){
	...
	for(; (c1 = *cp) != '\0'; ++cp){
		/* (RFC 1738) RFC 3986, 2.3 Unreserved Characters: ALPHA / DIGIT / "-" / "." / "_" / "~"
...
(mx_url_xdec)(char const *cp  su_DVL_LOC_ARGS_DECL){
	...
	np = n = su_MEM_BAG_SELF_AUTO_ALLOC_LOCOR(su_cs_len(cp) +1, su_DVL_LOC_ARGS_ORUSE);
...
mx_url_parse(struct mx_url *urlp, enum cproto cproto, char const *data){
	/* TODO mx_url_parse() is a terrible mess; all the substrings should be accessors of an object instead */
#if defined mx_HAVE_SMTP && defined mx_HAVE_POP3 && defined mx_HAVE_IMAP
# define a_ALLPROTO
#endif
#if defined mx_HAVE_SMTP || defined mx_HAVE_POP3 || defined mx_HAVE_IMAP || defined mx_HAVE_TLS
# define a_ANYPROTO
	char *cp, *x;
#endif
	boole rv;
...
#define a_PROTOX(X,Y,Z) \
	...
	if(!a_OUCH) {Z;}
#define a_PRIVPROTOX(X,Y,Z) do{ a_PROTOX(X, Y, Z); }while(0)
...
#define a_IFs(X,Y) a__IF(0, X, Y, urlp->url_flags |= mx_URL_TLS_OPTIONAL)
	...
	case CPROTO_CERTINFO:
		/* Special `tls' certificate info protocol; allow all protos here, for later getaddrinfo() usage! */
#ifdef mx_HAVE_TLS
		if((cp = su_cs_find(data, "://")) == NIL)
			...
			if((i = P2UZ(&cp[sizeof("://") -1] - data)) + 2 >= sizeof(urlp->url_proto))
				goto jeproto;
...
jurlp_err:
	...
	if((cp = su_cs_find_c(data, ':')) != NIL){ /* TODO URL: use IPAddress! */
		...
		cp = UNCONST(char*,&data[su_cs_len(data)]);
	}
	...
	if(x != NIL && *x != '\0'){
		...
		for(trailsol = FAL0, x2 = savestrbuf(x, i = su_cs_len(x)); i > 0; trailsol = TRU1, --i)
			if(x2[i - 1] != '/')
				break;
		x2[i] = '\0';
...
# ifdef mx_HAVE_IMAP
			if(trailsol){
				urlp->url_path.s = su_AUTO_ALLOC(i + sizeof("/INBOX"));
...
# ifdef mx_HAVE_IMAP
	if(cproto == CPROTO_IMAP && urlp->url_path.s == NIL)
		urlp->url_path.s = savestrbuf("INBOX", urlp->url_path.l = sizeof("INBOX") -1);
# endif
...
# endif /* mx_HAVE_IDNA */
	...
	/* C99 */{
		...
		s->s = su_AUTO_ALLOC(urlp->url_host.l + upl +1);
	...
	if(cproto != CPROTO_NONE && cproto != CPROTO_CERTINFO && !(urlp->url_flags & mx_URL_HAD_USER)){
		/* *user* guaranteed non-empty */
		if((urlp->url_user.s = xok_vlook(user, urlp, OXM_PLAIN | OXM_H_P)) == NIL){
			/* No, check whether .netrc lookup is desired */
# ifdef mx_HAVE_NETRC
			struct mx_netrc_entry nrce;

			if(ok_vlook(v15_compat) == NIL || !xok_blook(netrc_lookup, urlp, OXM_PLAIN | OXM_H_P) ||
...
# endif
	...
	/* C99 */{
		...
		s->s = su_AUTO_ALLOC(i + 1 + urlp->url_h_p.l +1);
		if(i > 0){
	...
	/* C99 */{
		...
		ud = su_AUTO_ALLOC((i = urlp->url_proto_len + sizeof("://") -1 + urlp->url_u_h_p.l) +1);
		urlp->url_proto[urlp->url_proto_len] = ':';
		su_mem_copy(su_cs_pcopy(ud, urlp->url_proto), urlp->url_u_h_p.s, urlp->url_u_h_p.l +1);
	...
	/* C99 */{
		...
		ud = su_AUTO_ALLOC((i = urlp->url_proto_len + sizeof("://") -1 + urlp->url_eu_h_p.l) + 1 +
				urlp->url_path.l +1);
		urlp->url_proto[urlp->url_proto_len] = ':';
		su_mem_copy(su_cs_pcopy(ud, urlp->url_proto), urlp->url_eu_h_p.s, urlp->url_eu_h_p.l +1);
		urlp->url_proto[urlp->url_proto_len] = '\0';
