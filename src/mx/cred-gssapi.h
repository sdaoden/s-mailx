...
/* */
static boole su_CONCAT(su_FILE,_gss)(struct mx_socket *sp, struct mx_url *urlp, struct mx_cred_ctx *credp,
...
/* */
static void su_CONCAT(su_FILE,_gss__error)(char const *s, OM_uint32 maj_stat, OM_uint32 min_stat);
static void su_CONCAT(su_FILE,_gss__error1)(char const *s, OM_uint32 code, int typ);

#elif a_CRED_GSSAPI_H == 2

static boole
su_CONCAT(su_FILE,_gss)(struct mx_socket *sop, struct mx_url *urlp, struct mx_cred_ctx *credp,
# ifdef mx_SOURCE_NET_SMTP
		struct a_netsmtp_ctx *nscp
# elif defined mx_SOURCE_NET_POP3 || defined mx_SOURCE_NET_IMAP
		struct mailbox *mp
...
# endif
	...
	if(INT_MAX - 1 - 5 <= urlp->url_host.l || INT_MAX - 1 - 4 <= credp->cc_user.l){
...
# endif
	...
	su_cs_pcopy(su_cs_pcopy(send_tok.value = buf =
			su_LOFI_ALLOC((send_tok.length = urlp->url_host.l + a_L) +1), a_N), urlp->url_host.s);
...
# undef a_L
	...
	if(maj_stat != GSS_S_COMPLETE){
		su_CONCAT(su_FILE,_gss__error)(savestrbuf(send_tok.value, send_tok.length), maj_stat, min_stat);
		goto jleave;
	}
...
# ifdef mx_SOURCE_NET_IMAP
	if(!(mp->mb_flags & MB_SASL_IR)){
		IMAP_OUT(savecat(tag(1), NETLINE(" AUTHENTICATE GSSAPI")), 0, goto jleave);
		imap_answer(mp, 1);
...
# endif
	...
	for(;;){
		...
		if(maj_stat != GSS_S_COMPLETE && maj_stat != GSS_S_CONTINUE_NEEDED){
			su_CONCAT(su_FILE,_gss__error)("gss_init_sec_context", maj_stat, min_stat);
			goto jleave;
		}
		if(f & a_F_OUTBUF){
			f ^= a_F_OUTBUF;
		...
		if(mx_b64_enc_buf(&out, send_tok.value, send_tok.length, mx_B64_AUTO_ALLOC | mx_B64_CRLF) == NIL)
			goto jleave;
...
# endif
	...
	maj_stat = gss_unwrap(&min_stat, gss_context, &recv_tok, &send_tok, &conf_state, NIL);
	f |= a_F_SEND_TOK;
	if(maj_stat != GSS_S_COMPLETE){
	...
	/* First octet: bit-mask with protection mechanisms (1 = no protection mechanism).
	 * Second to fourth octet: maximum message size in network byte order.
	 * Fifth and following octets: user name string */
	su_LOFI_FREE(buf);
	in.s = buf = su_LOFI_ALLOC((send_tok.length = 4u + credp->cc_user.l) +1);
	su_mem_copy(&in.s[4], credp->cc_user.s, credp->cc_user.l +1);
	in.s[0] = 1;
	in.s[1] = 0;
	in.s[2] = in.s[3] = S(char,0xFF);
	send_tok.value = in.s;
	maj_stat = gss_wrap(&min_stat, gss_context, 0, GSS_C_QOP_DEFAULT, &send_tok, &conf_state, &recv_tok);
	f |= a_F_RECV_TOK;
	if(maj_stat != GSS_S_COMPLETE){
		su_CONCAT(su_FILE,_gss__error)("wrapping data", maj_stat, min_stat);
		goto jleave;
	}
	if(mx_b64_enc_buf(&out, recv_tok.value, recv_tok.length, mx_B64_AUTO_ALLOC | mx_B64_CRLF) == NIL)
		goto jleave;
...
static void
su_CONCAT(su_FILE,_gss__error)(char const *s, OM_uint32 maj_stat, OM_uint32 min_stat){
	NYD2_IN;
...
su_CONCAT(su_FILE,_gss__error1)(char const *s, OM_uint32 code, int typ){
	...
	msg_ctx = 0;
	do{
		maj_stat = gss_display_status(&min_stat, code, typ, GSS_C_NO_OID, &msg_ctx, &msg);
		if(maj_stat == GSS_S_COMPLETE){
