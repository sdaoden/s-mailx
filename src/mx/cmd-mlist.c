...
/* ..of a_ml_dp.. */
#define a_ML_FLAGS (su_CS_DICT_CASE | su_CS_DICT_HEAD_RESORT | su_CS_DICT_AUTO_SHRINK | su_CS_DICT_ERR_PASS)
#define a_ML_THRESHOLD 2

/* ..of a_ml_re_dp.  Only for un-registration + flag change: use a high t.-s.  Usage of NILISVALO is essential for our
 * purpose, since we use view_set_data() to change the subscription state, but which is supposed to only flip the
 * lowermost bit -- see a_ml_dp docu below.  I.e., clone() -> template is char const* == regex, assign(): template is
 * a_ml_regex* == self with bit 1 indicating new subscription state, delete(): a_ml_regex* == self with bit
 * 1 indicating subscription state.  We MUST pass non-NIL to assign()! */
#ifdef mx_HAVE_REGEX
# define a_ML_RE_FLAGS (su_CS_DICT_OWNS | su_CS_DICT_AUTO_SHRINK | su_CS_DICT_ERR_PASS | su_CS_DICT_NILISVALO)
# define a_ML_RE_THRESHOLD 4
#endif
...
/* Vvalue of direct match dict is in fact boole, the regex one uses a_ml_regex* (with boole as first bit, as above) */
static struct su_cs_dict *a_ml_dp, a_ml__d;
#ifdef mx_HAVE_REGEX
static struct su_cs_dict *a_ml_re_dp, a_ml__re_d;

/* Regex are searched in order, subscribed first, then "default"; make this easier by using two dedicated lists.  We
 * will perform automatic head resorting on these lists in the hope that this will place often used matches nearer to
 * the head over time */
...
/* */
static struct n_strlist *a_ml_dump(char const *cmdname, char const *key, void const *dat);
...
static struct su_toolbox const a_ml_re_tbox = su_TOOLBOX_I9R(&a_ml_re_clone, &a_ml_re_delete, &a_ml_re_assign, NIL, NIL);
#endif
...
a_ml_mux(boole subscribe, char const **argv){
	...
	if((key = *argv) == NIL){
		...
		if(a_ml_dp == NIL){
			a_ml_dp = su_cs_dict_set_threshold(su_cs_dict_create(&a_ml__d, a_ML_FLAGS, NIL), a_ML_THRESHOLD);
#ifdef mx_HAVE_REGEX
			a_ml_re_dp = su_cs_dict_set_threshold(su_cs_dict_create(&a_ml__re_d, a_ML_RE_FLAGS,
					&a_ml_re_tbox), a_ML_RE_THRESHOLD);
#endif
			DVL( su_state_on_gut_install(&a_ml__on_gut, FAL0, su_STATE_ERR_NOPASS); )
		}
...
a_ml_unsub_all(struct su_cs_dict_view *dvp, struct su_cs_dict *dp){
	...
	for(su_cs_dict_view_setup(dvp, dp); su_cs_dict_view_is_valid(dvp); su_cs_dict_view_next(dvp)){
...
a_ml_re_assign(void *self, void const *t, u32 estate){
	/* Thanks to NILISVALO we can (mis)use assignment for the sole purpose of flipping the subscription bit and
	 * performing list relinking! */
	struct a_ml_regex **lpp, *lstnp, *nxtnp;
...
#endif
		}else if(a_ml_dp != NIL && su_cs_dict_view_find(su_cs_dict_view_setup(&dv, a_ml_dp), key)){
			goto jtest;
jtest:
			...
			if(u.flags & TRU1){
				...
				goto jenot;
#ifdef mx_HAVE_REGEX
		}else if(a_ml_re_dp != NIL && su_cs_dict_view_find(su_cs_dict_view_setup(&dv, a_ml_re_dp), key)){
			goto jtest;
#endif
		}else{
jenot:
...
mx_mlist_query(char const *name, boole subscribed_only){
	...
	if(a_ml_dp != NIL && su_cs_dict_view_find(su_cs_dict_view_setup(&dv, a_ml_dp), name)){
		u.cvp = su_cs_dict_view_data(&dv);
		if(u.flags & TRU1)
