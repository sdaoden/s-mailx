...
a_attachments_is_msg(char const *file){
	...
	if(file[0] == '#'){
		...
		}else if((su_idec_uz_cp(&ib, &file[1], 10, NIL) & (su_IDEC_STATE_EMASK | su_IDEC_STATE_CONSUMED)
				) != su_IDEC_STATE_CONSUMED || ib == 0 || UCMP(z, ib, >, msgCount))
			rv = -1;
		else
			rv = S(int,ib);
	}
...
static struct mx_attachment *
a_attachments_setup_msg(struct mx_attachment *ap, char const *msgcp, int msgno){
	NYD2_IN;
...
a_attachments_yay(struct mx_attachment const *ap){
	...
	if(ap->a_msgno > 0)
		...
		fprintf(n_stdout, _("Added attachment %s (%s)\n"),
			n_shexp_quote_cp(ap->a_name, FAL0), n_shexp_quote_cp(ap->a_path_user, FAL0));
...
struct mx_attachment *
mx_attachments_find(struct mx_attachment *aplist, char const *name, boole *stat_or_nil){
	int msgno;
	char const *bname;
	boole status, sym;
...
mx_attachments_list_edit(struct mx_attachment *aplist, BITENUM_IS(u32,mx_go_input_flags) gif){
	...
	for(naplist = NIL;;){
		...
		if(aplist != NIL){
			/* TODO If we would create .a_path_user in append() after any expansion then we could avoid
			 * TODO closing+rebuilding the temporary file if the new user input matches the original value! */
	...
	for(attno = 1, ap = aplist; ap != NIL; ++rv, ++attno, ap = ap->a_flink){
		if(ap->a_msgno > 0){
			...
			if(fprintf(fp, "#%" PRIu32 ": %s [%s -- %s%s",
					...
					(ap->a_content_type != NIL ? ap->a_content_type : _("unclassified content")),
					(ap->a_conv_force_b64 ? _(", base64 enforced") : su_empty)) < 0)
				goto jerr;

			if(ap->a_conv == mx_ATTACHMENTS_CONV_TMPFILE){
				/* I18N: input and output character set as given */
				if(fprintf(fp, _(", incs=%s -> oucs=%s (readily converted)"), incs, oucs) < 0)
					goto jerr;
				...
				if(incs != NIL){
					...
					if(fprintf(fp, _(", incs=%s -> oucs=*sendcharsets*"), incs) < 0)
						goto jerr;
				}else if(ap->a_content_type == NIL || !su_cs_cmp_case_n(ap->a_content_type, "text/", 5)){
