...
#define a_CCND_IF_IS_SKIP() \
   (a_CCND_IF_IS_ACTIVE() &&\
      ((S(struct a_ccnd_if_node*,mx_go_data->gdc_ifcond)->cin_flags &\
         a_CCND_IF_F_NOOP) ||\
       !(S(struct a_ccnd_if_node*,mx_go_data->gdc_ifcond)->cin_flags &\
         a_CCND_IF_F_GO)))
...
a_ccnd_if(void *vp, boole iselif){
   ...
   cinp->cin_flags = (a_CCND_IF_F_GO |
         (a_CCND_IF_IS_SKIP() ? a_CCND_IF_F_NOOP : 0));
   if(!iselif)
...
c_elif(void *vp){
   ...
   if((cinp = S(struct a_ccnd_if_node*,mx_go_data->gdc_ifcond)) == NIL ||
         (cinp->cin_flags & a_CCND_IF_F_ELSE)){
      ...
      if(a_CCND_IF_IS_SKIP()){
         if(toggle){
            ...
            if((cinp = cinp->cin_outer) != NIL &&
                  ((cinp->cin_flags & a_CCND_IF_F_NOOP) ||
                   !(cinp->cin_flags & a_CCND_IF_F_GO)))
               toggle = FAL0;
         }
