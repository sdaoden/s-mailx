 ...
 *
 * Copyright (c) 2012/3 - 2023 Steffen Nurpmeso <steffen@sdaoden.eu>.
...
static sz a_qf_state_prefix(struct a_qf_vc *vcp);
static sz a_qf_state_data(struct a_qf_vc *vcp);

static sz
a_qf_dump_prefix(struct quoteflt *self){
	sz rv;
	uz i;
	NYD_IN;

	if((i = self->qf_pfix_len) > 0 && i != fwrite(self->qf_pfix, 1, i, self->qf_os))
		goto jerr;
	rv = i;

	if((i = self->qf_currq.l) > 0 && i != fwrite(self->qf_currq.s, 1, i, self->qf_os))
		goto jerr;
	rv += i;
jleave:
	NYD_OU;
...
a_qf_add_data(struct quoteflt *self, wchar_t wc){
	...
	/* To avoid that the last visual excesses *qfold-max*, which may happen for multi-column characters, use w as
	 * an indicator for this and move that thing to the next line */
	w = wcwidth(wc);
	if(w == -1){
		w = 0;
jbad:
		...
		if(l < 0)
			goto jbad;
		self->qf_datw += S(u32,w);
		self->qf_dat.l += S(uz,l);
	}

	if(self->qf_datw >= self->qf_qfold_max){
		/* If we have seen a nice breakpoint during traversal, shuffle data around a bit so as to restore the
		 * trailing part after flushing */
jneednl:
		if(self->qf_brkl > 0){
...
jflush:
		...
		if(isws || !self->qf_brk_isws || self->qf_brkl == 0){
			if((self->qf_brk_isws = isws) || self->qf_brkl < self->qf_qfold_maxnws){
...
static sz
a_qf_state_prefix(struct a_qf_vc *vcp){
	...
	for(buf = vcp->vc_buf, len = vcp->vc_len; len > 0;){
		/* xxx NULL BYTE! */
		...
		if(i == 1 && su_cs_is_ascii(wc) && su_cs_find_c(self->qf_quote_chars, S(char,wc)) != NIL){
			self->qf_wscnt = 0;
...
static sz
a_qf_state_data(struct a_qf_vc *vcp){
	struct quoteflt *self;
	sz rv;
	char const *buf;
	uz len, i;
...
quoteflt_init(struct quoteflt *self, char const *prefix, boole bypass){
	...
	STRUCT_ZERO(struct quoteflt, self);

	if((self->qf_pfix = prefix) != NIL)
		self->qf_pfix_len = S(u32,su_cs_len(prefix));
...
quoteflt_push(struct quoteflt *self, char const *dat, uz len){
	/* (xxx Ideally the actual push() [and flush()] would be functions on their own, via indirect vtbl call ..) */
	sz rv;
	NYD_IN;
	 ...
	 * - Lookout for a newline */
		...
		while(vc.vc_len > 0){
			...
			case a_QF_CLEAN:
			case a_QF_PREFIX:
				i = a_qf_state_prefix(&vc);
				break;
			default: /* silence cc (`i' unused) */
			case a_QF_DATA:
				i = a_qf_state_data(&vc);
				break;
			}
			if(i < 0)
				goto jerr;
