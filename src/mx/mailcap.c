 ...
 *
 * Copyright (c) 2019 - 2025 Steffen Nurpmeso <steffen@sdaoden.eu>.
...
/* Whether to place as much on a line as possible (Y).  Otherwise (X) we place commands on lines of their own */
...
#define a_MAILCAP_SFIELD_SUPPORTED(X) ((X) == a_MAILCAP_SF_CMD /*|| (X) == a_MAILCAP_SF_TEST*/)
...
CTA(a_MAILCAP_F_MAX <= S32_MAX, "a_mailcap_hdl.mch_flags bit range excessed");
...
static boole a_mailcap__parse_line(struct a_mailcap_load_stack *mclsp, char *dp, uz dl);
static boole a_mailcap__parse_kv(struct a_mailcap_load_stack *mclsp, char *kp, char *vp);
static boole a_mailcap__parse_value(u32 sfield, struct a_mailcap_load_stack *mclsp, struct str *s);
static boole a_mailcap__parse_flag(struct a_mailcap_load_stack *mclsp, char *flag);
...
static void a_mailcap__dump_kv(u32 sfield, struct n_string *s, uz *llp, char const *pre, char const *vp);
static struct n_string *a_mailcap__dump_quote(struct n_string *s, char const *cp, boole quotequote);

/* Expand a command string with embedded formats */
static char const *a_mailcap_expand_formats(char const *format, struct mimepart const *mpp, char const *ct);
...
a_mailcap__load_file(struct a_mailcap_load_stack *mclsp){
	...
	for(f = a_NONE;;){
		if(fgetline(&mclsp->mcls_dat.s, &mclsp->mcls_dat.l, NIL, &len, mclsp->mcls_fp, TRU1) == NIL){
		...
		if(f & (a_CONTI | a_NEWCONTI)){
			if(mclsp->mcls_conti_dat.s == NIL){
				...
				if(!mx_fs_linepool_book(&mclsp->mcls_conti_dat.s, &mclsp->mcls_conti_dat.l,
						mclsp->mcls_conti_len, MAX(len, 256)))
					goto jetoolong;
				su_mem_copy(&mclsp->mcls_conti_dat.s[mclsp->mcls_conti_len], mclsp->mcls_dat.s, len +1);
				mclsp->mcls_conti_len += len;
			}
			f |= a_CONTI;
...
jconti_do:
			/* C99 */{
				...
				x = a_mailcap__parse_line(mclsp, mclsp->mcls_conti_dat.s, mclsp->mcls_conti_len);
				/* Release buffer to linepool, so to swap it in again next, shall this be necessary! */
				mx_fs_linepool_release(mclsp->mcls_conti_dat.s, mclsp->mcls_conti_dat.l);
...
a_mailcap__parse_line(struct a_mailcap_load_stack *mclsp, char *dp, uz dl){
	...
	if(n_str_trim(&s, n_STR_TRIM_BOTH)->l == 0){
		if(n_poption & n_PO_D_V)
			n_err(_("$MAILCAPS: %s: line empty after whitespace removal (invalid RFC 1524 syntax)\n"),
...
jredo:
	...
	if(n_str_trim(&s, n_STR_TRIM_BOTH)->l == 0){
		emsg = (emsg == R(char*,1)) ? N_("ignored: empty key") : N_("ignored: empty value");
		goto jerr;
	}
	(*cpp = s.s)[s.l] = '\0';
...
jentempl:
	...
	if((rv = (kp[0] != 'x' && kp[0] != '\0' && kp[1] != '-')) || (n_poption & n_PO_D_V)){
...
static boole
a_mailcap__parse_value(u32 sfield, struct a_mailcap_load_stack *mclsp, struct str *s){
	char *cp2, *cp3, c;
	boole rv;
	NYD2_IN;
	...
	n_string_push_c(n_string_push_buf(&mclsp->mcls_hdl_buf, s->s, P2UZ(cp3 - s->s)), '\0');
		...
		if(c == '\\')
			...
			case '{':
				if(su_cs_find_c(cp2, '}') == NIL){
					...
					/* We need to skip the entire thing if we are incapable to satisfy the format
					 * request when invoking a command */
			...
			case 's':
					...
					if((n_poption & n_PO_D_V) && (*cp2 == '"' || *cp2 == '\''))
						n_err(_("$MAILCAPS: %s: %s: (maybe!) %%s must not be quoted: %s\n"),
							mclsp->mcls_name_quoted, mclsp->mcls_type_subtype, s->s);
			...
			case 'F':
				...
				/* We need to skip the entire thing if we are incapable to satisfy the format request
			...
			default:
				n_err(_("$MAILCAPS: %s: %s: invalid format %%%c, should be quoted: \\%%%c\n"),
...
jquote:
	...
	n_string_push_c(n_string_push_buf(&mclsp->mcls_hdl_buf, lbuf, P2UZ(cp3 - lbuf)), '\0');
...
a_mailcap__parse_flag(struct a_mailcap_load_stack *mclsp, char *flag){
	...
	for(fap = &a_mailcap_flags[0]; fap < &a_mailcap_flags[NELEM(a_mailcap_flags)]; ++fap)
		if(!su_cs_cmp_case(flag, fap->mcf_name)){
			if((n_poption & n_PO_D_V) && (mclsp->mcls_hdl.mch_flags & fap->mcf_flags))
	...
	if((rv = (flag[0] != 'x' && flag[0] != '\0' && flag[1] != '-')) || (n_poption & n_PO_D_V))
...
a_mailcap__parse_create_hdl(struct a_mailcap_load_stack *mclsp, struct a_mailcap_hdl **ins_or_nil){
	...
	if(mclsp->mcls_hdl.mch_sfields[a_MAILCAP_SF_NAMETEMPLATE] != 0){
		if(f & a_MAILCAP_F_HAS_S_FORMAT)
			f |= mx_MIME_TYPE_HDL_TMPF_NAMETMPL | mx_MIME_TYPE_HDL_TMPF_NAMETMPL_SUFFIX;
		else
	...
	mchp = S(struct a_mailcap_hdl*,su_CALLOC(sizeof(*mchp) + mclsp->mcls_hdl_buf.s_len +1));
	if(mchp == NIL)
		goto jleave;

	su_mem_copy(mchp, &mclsp->mcls_hdl, sizeof(mclsp->mcls_hdl));
	su_mem_copy(&mchp[1], n_string_cp(&mclsp->mcls_hdl_buf), mclsp->mcls_hdl_buf.s_len +1);
...
a_mailcap_gut(boole gut_dp){
	...
	if(a_mailcap_dp != NIL){
		...
		su_CS_DICT_FOREACH(a_mailcap_dp, &csdv){
			...
			for(mchp = S(struct a_mailcap_hdl*,su_cs_dict_view_data(&csdv)); mchp != NIL;){
				tmp = mchp;
				mchp = mchp->mch_next;
				su_FREE(tmp);
			}
...
#undef a_X
	...
	for(mchp = S(struct a_mailcap_hdl const*,dat); mchp != NIL; mchp = mchp->mch_next){
		uz i, lo, lx;
		char const *buf;
...
static void
a_mailcap__dump_kv(u32 sfield, struct n_string *s, uz *llp, char const *pre, char const *vp){
	boole quote;
	uz lo, prel, i, lx;
	NYD2_IN;
	...
	for(i = 0;; ++i)
		if(vp[i] == '\0' && vp[i + 1] == '\0')
			break;
	/* An empty command is an error if no other field follows, a condition we do not know here, so put something;
	 * instead of a dummy field, put success */
	if(i == 0 && sfield == a_MAILCAP_SF_CMD){
		vp = ":\0\0";
		i = 1;
	}

	if(a_MAILCAP_DUMP_SEP_INJ(sfield != a_MAILCAP_SF_CMD || lo + prel + i >= 75, lo + prel + i >= 72)){
		s = n_string_push_buf(s, "\\\n ", sizeof("\\\n ") -1);
		lo = 1;
	}
...
static char const *
a_mailcap_expand_formats(char const *format, struct mimepart const *mpp, char const *ct){
	...
	for(cp = format;;){
		...
		if((c = *cp++) == '\0'){
			...
			case 's':
				...
				s = n_string_push_cp(s, "\"${" n_PIPEENV_FILENAME_TEMPORARY "}\"");
				break;
			case 't':
...
jlist:
	...
	if(!load_only){
		...
		slp = NIL;
		if(!(mx_xy_dump_dict("mailcap", a_mailcap_dp, &slp, NIL, &a_mailcap_dump) &&
...
boole
mx_mailcap_handler(struct mx_mime_type_handler *mthp, char const *ct, enum sendaction action, struct mimepart const *mpp){
	...
	for(; p.mch != NIL; p.mch = p.mch->mch_next){
		...
		if(p.mch->mch_sfields[a_MAILCAP_SF_TEST] != 0){
			if(!(f & a_MAILCAP_F_TEST_ONCE) || !(f & a_MAILCAP_F_TEST_ONCE_DONE)){
				...
				cmdp = &R(char const*,&p.mch[1])[p.mch->mch_sfields[a_MAILCAP_SF_TEST]];
		 ...
		 * XXX up to the shell */
		mthp->mth_shell_cmd = &R(char const*,&p.mch[1])[p.mch->mch_sfields[a_MAILCAP_SF_CMD]];
		if(p.mch->mch_sfield_has_format & (1u << a_MAILCAP_SF_CMD))
			mthp->mth_shell_cmd = a_mailcap_expand_formats(mthp->mth_shell_cmd, mpp, ct);

		if(p.mch->mch_sfields[a_MAILCAP_SF_NAMETEMPLATE] != 0){
			mthp->mth_tmpf_nametmpl = &R(char const*,&p.mch[1])[p.mch->mch_sfields[a_MAILCAP_SF_NAMETEMPLATE]];
			ASSERT(mthp->mth_tmpf_nametmpl[0] == '\0' && mthp->mth_tmpf_nametmpl[1] == 's');
...
jleave:
	NYD_OU;
	return (mchp == NIL ? FAL0 : (mchp->mch_flags & a_MAILCAP_F_LAST_RESORT ? TRUM1 : TRU1));
}
