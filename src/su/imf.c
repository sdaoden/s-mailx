	...
	for(rv = su_IMF_ERR_CONTENT; *acp->ac_.hd != '\0';){
		...
		if(*acp->ac_.hd == ';'){
			if(!(mode & su_IMF_MODE_TOK_SEMICOLON))
				goto jleave;
			if(shtp != NIL && !(shtp->imfsht_mse & su_IMF_STATE_SEMICOLON))
...
jtoken_create:
		...
		*shtpp = shtp;
		...
		shtp->imfsht_mse = (acp->ac_.mse & ~(su__IMF_MODE_MASK | su_IMF_STATE_GROUP));

		if(*acp->ac_.hd == ';' && (mode & su_IMF_MODE_TOK_SEMICOLON)){
			++acp->ac_.hd;
			shtp->imfsht_mse |= su_IMF_STATE_SEMICOLON;
		}
