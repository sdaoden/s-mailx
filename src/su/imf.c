...
su_imf_parse_addr_header(struct su_imf_addr **app, char const *header, BITENUM(u32,su_imf_mode) mode, /* {{{ */
	...
	NYD_IN;
	ASSERT_EXEC((mode & ~su__IMF_MODE_MASK) == 0, mode &= su__IMF_MODE_MASK);
	...
	*(app_base = app) = NIL;
	...
	/* C99 */{
		 ...
		 * We may re-quote display-names and locpar, so reserve two byte for quotes plus one for NUL */
