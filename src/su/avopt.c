 ...
 *
 * Copyright (c) 2018 - 2024 Steffen Nurpmeso <steffen@sdaoden.eu>.
...
jouter:
	 ...
	 * No tests, long option string is expected to have been tested by su_HAVE_DEBUG / su_HAVE_DEVEL code, below */
	ASSERT(*opt == ';');
	if(*++opt != '-')
		rv = *opt;
	else{
	...
	/* C99 */{
		...
		if(self->avo_flags & a_AVOPT_SHORT){
			if((cp = self->avo_opts_short)[0] == '\0'){
				...
				su_log_write(su_LOG_CRIT, "su_avopt_setup(): empty short option string is unsupported");
			}else do{
				if(!su_cs_is_print(*cp)){
					self->avo_flags &= ~a_AVOPT_SHORT;
					self->avo_flags |= a_AVOPT_ERR;
					su_log_write(su_LOG_CRIT,
						"su_avopt_setup(): invalid non-printable bytes in short option string: %s",
						self->avo_opts_short);
				}else if(*cp == '-'){
					self->avo_flags &= ~a_AVOPT_SHORT;
					self->avo_flags |= a_AVOPT_ERR;
					su_log_write(su_LOG_CRIT,
						"su_avopt_setup(): invalid hyphen-minus in short option string: %s",
						self->avo_opts_short);
				}
			}while(*++cp != '\0');
		}
		...
		if(self->avo_flags & a_AVOPT_LONG){
			if(self->avo_opts_long[0] == NIL){
				...
				self->avo_flags |= a_AVOPT_ERR;
				su_log_write(su_LOG_CRIT, "su_avopt_setup(): empty long option array is unsupported");
				...
				if(cp[0] == '\0'){
					self->avo_flags &= ~a_AVOPT_LONG;
					self->avo_flags |= a_AVOPT_ERR;
					su_log_write(su_LOG_CRIT, "su_avopt_setup(): empty long option is unsupported");
					continue;
				}
				...
				for(; *cp != ':' && *cp != ';' && *cp != '\0'; ++cp){
					...
					if(!su_cs_is_print(*cp) || *cp == '='){
						...
						self->avo_flags |= a_AVOPT_ERR;
						su_log_write(su_LOG_CRIT,
							"su_avopt_setup(): invalid content in long option: %s",
				...
				if(cp == self->avo_opts_long[i]){
					self->avo_flags &= ~a_AVOPT_LONG;
					self->avo_flags |= a_AVOPT_ERR;
					su_log_write(su_LOG_CRIT, "su_avopt_setup(): missing actual long option: %s",
						self->avo_opts_long[i]);
				}
				...
				if(*cp != ';' || *++cp == '\0'){
					self->avo_flags &= ~a_AVOPT_LONG;
					self->avo_flags |= a_AVOPT_ERR;
					su_log_write(su_LOG_CRIT, "su_avopt_setup(): missing long option identifier: %s",
						self->avo_opts_long[i]);
					continue;
				}
				...
				if(*cp == '-'){
					...
					for(loi = 0; su_cs_is_digit(*++cp);){
						...
						if(!b && loi >= S32_MAX){
							...
							self->avo_flags |= a_AVOPT_ERR;
							su_log_write(su_LOG_CRIT,
								"su_avopt_setup(): long option identifier not 32-bit: %s",
								self->avo_opts_long[i]);
						}
					}
					...
					if(cp[-1] == '-' || loi == 0){
						...
						self->avo_flags |= a_AVOPT_ERR;
						su_log_write(su_LOG_CRIT,
							"su_avopt_setup(): empty or 0 long option identifier: %s",
							self->avo_opts_long[i]);
					}
				}else{
					if(!su_cs_is_print(*cp)){
						...
						self->avo_flags |= a_AVOPT_ERR;
						su_log_write(su_LOG_CRIT,
							"su_avopt_setup(): invalid long->short option equivalence: %s",
							self->avo_opts_long[i]);
						...
						for(osp = self->avo_opts_short; *osp != '\0'; ++osp){
							if(*osp != *cp){
								...
								if((osp[1] == ':') != arg){
									...
									self->avo_flags |= a_AVOPT_ERR;
									su_log_write(su_LOG_CRIT,
										"su_avopt_setup(): long option %s argument, short does%s: %s",
				...
				if(*cp != ';'){
					...
					self->avo_flags |= a_AVOPT_ERR;
					su_log_write(su_LOG_CRIT,
						"su_avopt_setup(): invalid content after (unseparated?) long option identifier: %s: %s",
...
jleave:
	self->avo_curr = curr;
	self->avo_current_opt = (rv != su_AVOPT_STATE_DONE || !(flags & a_AVOPT_STOP)) ? rv : su_AVOPT_STATE_STOP;
