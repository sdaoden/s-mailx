...
# define a_MEMA_HOPE_SIZE_ADD (a_MEMA_HOPE_SIZE + sizeof(struct a_mema_heap_chunk))
...
do{\
	...
	if(i__ != 0){\
		...
		su_log_write(su_LOG_ALERT | su_LOG_F_CORE,\
			"! SU memory: %p: corrupt lower canary: 0x%02X: %s, line %" PRIu32,\
	...
	if(i__ != 0){\
		...
		a_MEMA_HOPE_INC((C).map_cp);\
		su_log_write(su_LOG_ALERT | su_LOG_F_CORE,\
			"! SU memory: %p: corrupt upper canary: 0x%02X: %s, line %" PRIu32,\
			(C).map_cp, i__, su_DVL_LOC_ARGS_FILE, su_DVL_LOC_ARGS_LINE);\
		a_MEMA_HOPE_DEC((C).map_cp);\
	}\
	if(BAD)\
		su_log_write(su_LOG_ALERT | su_LOG_F_CORE, "! SU memory: ..canary last seen: %s, line %" PRIu32,\
...
# define a_MEMA_MARK_TO_STORE(X) ((S(u32,X) >> su__MEM_ALLOC_MARK_SHIFT) & su__MEM_ALLOC_MARK_MASK)
/*# define a_MEMA_STORE_TO_MARK(MACP) ((MACP)->mac_mark << su__MEM_ALLOC_MARK_SHIFT)*/
...
/* */
static void a_mema_dump_chunk(boole how, char buf[a_MEMA_DUMP_SIZE], void const *vp, uz size);
#endif
...
a_mema_release_free(void){
	...
	if((p.map_hc = a_mema_free_list) != NIL){
		...
		su_log_write(su_LOG_DEBUG | su_LOG_F_CORE,
			"su_mem_set_conf(LINGER_FREE_RELEASE): freed %" PRIuZ " chunks / %" PRIuZ " bytes", c, s);
	}
...
su__mem_check(su_DVL_LOC_ARGS_DECL_SOLE){
	...
	for(p.map_hc = a_mema_heap_list; p.map_hc != NIL; p.map_hc = p.map_hc->mahc_next){
		xp = p;
		++xp.map_hc;
		...
		if(isbad){
			anybad |= isbad;
			su_log_write(su_LOG_ALERT | su_LOG_F_CORE,
				"! SU memory: CANARY ERROR (heap): %p (%" PRIuZ " bytes): %s, line %" PRIu32,
				xp.map_vp, (p.map_c->mac_size - p.map_c->mac_user_off),
				p.map_c->mac_file, p.map_c->mac_line);
		}
	}

	for(p.map_hc = a_mema_free_list; p.map_hc != NIL; p.map_hc = p.map_hc->mahc_next){
		xp = p;
		++xp.map_hc;
		a_MEMA_HOPE_GET_TRACE(map_hc, xp, isbad);
		if(isbad){
			anybad |= isbad;
			su_log_write(su_LOG_ALERT | su_LOG_F_CORE,
				"! SU memory: CANARY ERROR (free list): %p (%" PRIuZ " bytes): %s, line %" PRIu32,
				xp.map_vp, (p.map_c->mac_size - p.map_c->mac_user_off),
				p.map_c->mac_file, p.map_c->mac_line);
		}else{
			uz i;
			...
			while(i-- != 0)
				if(ubp[i] != 0xBA){
					anybad |= 1;
					su_log_write(su_LOG_ALERT | su_LOG_F_CORE,
						"! SU memory: FREED BUFFER MODIFIED: %p (%" PRIuZ " bytes): %s, line %" PRIu32,
						xp.map_vp, (p.map_c->mac_size - p.map_c->mac_user_off),
						p.map_c->mac_file, p.map_c->mac_line);
				}
		}
	...
	if(anybad)
		su_log_write(((a_mema_conf & su_MEM_CONF_ON_ERROR_EMERG) ? su_LOG_EMERG : su_LOG_ALERT) | su_LOG_F_CORE,
...
su__mem_trace(boole dumpmem  su_DVL_LOC_ARGS_DECL){
	...
	for(mark = su__MEM_ALLOC_MARK_MAX;; --mark){
		...
		for(p.map_hc = a_mema_heap_list; p.map_hc != NIL; p.map_hc = p.map_hc->mahc_next){
			...
			if(dumpmem)
				a_mema_dump_chunk(dumpmem, dump, xp.map_vp, p.map_c->mac_size - p.map_c->mac_user_off);
	...
	if(a_mema_free_list != NIL){
		su_log_write(su_LOG_INFO | su_LOG_F_CORE, "Freed memory lingering for release:");
...
su_mem_allocate(uz size, uz no, BITENUM(u32,su_mem_alloc_flags) maf  su_DVL_LOC_ARGS_DECL){
	...
	if(a_MEMA_DBG( UZ_MAX - a_MEMA_HOPE_SIZE_ADD > size && )
		...
		if(LIKELY((rv = malloc(size)) != NIL)){
			...
			a_mema_stats[mark].mas_cnt_max = MAX(a_mema_stats[mark].mas_cnt_max, a_mema_stats[mark].mas_cnt_curr);
			a_mema_stats[mark].mas_mem_all += user_sz;
			a_mema_stats[mark].mas_mem_curr += user_sz;
			a_mema_stats[mark].mas_mem_max = MAX(a_mema_stats[mark].mas_mem_max, a_mema_stats[mark].mas_mem_curr);
#endif /* su_MEM_ALLOC_DEBUG */
		}else
			su_state_err(su_STATE_ERR_NOMEM, maf, _("SU memory: allocation request"));
	}else
		su_state_err(su_STATE_ERR_OVERFLOW, maf, _("SU memory: allocation request"));
...
su_mem_reallocate(void *ovp, uz size, uz no, BITENUM(u32,su_mem_alloc_flags) maf  su_DVL_LOC_ARGS_DECL){
		...
		if(!p.map_c->mac_isfree)
			...
			su_log_write(su_LOG_ALERT | su_LOG_F_CORE,
				"SU memory: reallocation: pointer corrupted!  At %s, line %" PRIu32
					"\n\tLast seen: %s, line %" PRIu32
				su_DVL_LOC_ARGS_USE, p.map_c->mac_file, p.map_c->mac_line);
			su_state_err(su_STATE_ERR_NOMEM, maf, _("SU memory: reallocation of corrupted pointer"));
			goto NYD_OU_LABEL;
		}else{
			su_log_write(su_LOG_ALERT | su_LOG_F_CORE,
				"SU memory: reallocation: pointer freed!  At %s, line %" PRIu32
					"\n\tLast seen: %s, line %" PRIu32
				su_DVL_LOC_ARGS_USE, p.map_c->mac_file, p.map_c->mac_line);
			su_state_err(su_STATE_ERR_NOMEM, maf, _("SU memory: reallocation of a freed pointer"));
			goto NYD_OU_LABEL;
		}
	}
#endif /* su_MEM_ALLOC_DEBUG */
	...
	if(a_MEMA_DBG( UZ_MAX - a_MEMA_HOPE_SIZE_ADD > size && )
		...
		if(UNLIKELY((rv = realloc(ovp, size)) == NIL))
			su_state_err(su_STATE_ERR_NOMEM, maf, _("SU memory: reallocation request"));
#ifdef su_MEM_ALLOC_DEBUG
		else{
			p.map_vp = rv;
		...
		su_state_err(su_STATE_ERR_OVERFLOW, maf, _("SU memory: reallocation request"));
...
su_mem_set_conf(BITENUM(u32,su_mem_conf_option) mco, uz val){
	...
	if((rmco & su_MEM_CONF_LINGER_FREE_RELEASE) || ((rmco & su_MEM_CONF_LINGER_FREE) && !val)){
