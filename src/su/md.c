...
a_md_siphash_setup(void *self, void const *k, uz kl, uz ds){
	...
	if(kl != su_SIPHASH_KEY_SIZE || (ds != su_SIPHASH_DIGEST_SIZE_64 && ds != su_SIPHASH_DIGEST_SIZE_128))
		rv = su_ERR_INVAL;
	else
		rv = su_siphash_setup_custom(S(struct su_siphash*,self), k,
				(ds == su_SIPHASH_DIGEST_SIZE_64 ? su_SIPHASH_DIGEST_64 : su_SIPHASH_DIGEST_128), 0, 0);
...
s32
su__md_install(char const *name, struct su_md_vtbl const *vtblp, su_new_fun cxx_it, u32 estate){
...
boole
su__md_uninstall(char const *name, struct su_md_vtbl const *vtblp, su_new_fun cxx_it){
	...
	if(a_md_lock != NIL){
		...
		for(mdlpp = &a_md_list; *mdlpp != NIL; mdlpp = &(*mdlpp)->mdl_last)
			if(!su_cs_cmp_case(name, (*mdlpp)->mdl_name) && vtblp == (*mdlpp)->mdl_vtbl &&
	...
	if(!su_cs_cmp_case(name, "siphash")){
		...
		for(mdlp = a_md_list; mdlp != NIL; mdlp = mdlp->mdl_last)
			if(!su_cs_cmp_case(name, mdlp->mdl_name)){
				vtblp = mdlp->mdl_vtbl;
				newptf = (mdlp->mdl_cxx_it != NIL ? mdlp->mdl_cxx_it : vtblp->mdvtbl_new);
				break;
			}
