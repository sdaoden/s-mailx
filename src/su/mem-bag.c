...
#define a_MEMBAG_BSZ_BASE MAX(ALIGN_Z(a_MEMBAG__SZA), ALIGN_Z(a_MEMBAG__SZB))
...
a_membag_lofi_free_top(struct su_mem_bag *self){
	...
	boole isheap;
	union {struct su__mem_bag_lofi_chunk *mblc; up u; void *v; void **vp; struct su__mem_bag_lofi_pool *mblp;} p;
...
# if a_MEMBAG_HULL
	/* C99 */{
		...
		for(p.c = S(char*,vp); p.c < maxp;
				p.c += VSTRUCT_SIZEOF(struct su__mem_bag_lofi_chunk,mblc_buf) + sizeof p.c){
...
su_mem_bag_auto_allocate(struct su_mem_bag *self, uz size, uz no, BITENUM_IS(u32,su_mem_bag_alloc_flags) mbaf
	...
	if(LIKELY(S(uz,S32_MAX) / no > size)){
		...
		size *= no;
		chunksz = a_MEMBAG_HULL ? sizeof(up) : ALIGN_Z(size);
...
jhave_pool:;
			...
			DBGX( su_log_write(su_LOG_DEBUG,
				"su_mem_bag_auto_allocate(): huge: " "%" PRIuZ " bytes from %s:%" PRIu32,
		...
		su_state_err(su_STATE_ERR_OVERFLOW, mbaf, _("SU memory bag: auto allocation request"));
	...
	if(LIKELY(S(uz,S32_MAX) / no > size)){
		...
		/* C99 */{
			...
			DBGX( if(realsz > self->mb_bsz)
				su_log_write(su_LOG_DEBUG,
					"su_mem_bag_lofi_allocate(): huge: %" PRIuZ " bytes from %s:%" PRIu32,
					size su_DVL_LOC_ARGS_USE); )
			isheap = (a_MEMBAG_HULL || realsz > self->mb_bsz);
			chunksz = ALIGN_Z(VSTRUCT_SIZEOF(struct su__mem_bag_lofi_chunk, mblc_buf)
					) + (isheap ? sizeof(up) : realsz);
		}
...
jhave_pool:
		...
		su_state_err(su_STATE_ERR_OVERFLOW, mbaf, _("SU memory bag: lofi allocation request"));
...
su_mem_bag_lofi_free(struct su_mem_bag *self, void *ovp  su_DVL_LOC_ARGS_DECL){
	...
	/* C99 */{
		...
		if(ovp == NIL){
			su_log_write(su_LOG_DEBUG, "su_mem_bag_lofi_free(): NIL from %s:%" PRIu32 su_DVL_LOC_ARGS_USE);
			goto NYD_OU_LABEL;
		}

		if((mblcp = self->mb_lofi_top) == NIL){
			su_log_write(su_LOG_DEBUG, "su_mem_bag_lofi_free(): no LOFI memory exists at %s:%" PRIu32
				su_DVL_LOC_ARGS_USE);
			goto NYD_OU_LABEL;
		}
...
jeinval:
			su_log_write(su_LOG_DEBUG, "su_mem_bag_lofi_free(): invalid pointer from %s:%" PRIu32
				su_DVL_LOC_ARGS_USE);
			goto NYD_OU_LABEL;
		}
	}
