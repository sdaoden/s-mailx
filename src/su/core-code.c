...
/* */
#if DVLOR(1, 0)
static void a_core_nyd_printone(void (*ptf)(up cookie, char const *buf, uz blen), up cookie,
...
a_core_evlog(u32 lvl_a_flags, char const *fmt, va_list ap){
	...
	char buf[2048]/* TODO FmtCodec */, ibuf[su_IENC_BUFFER_SIZE], *xbuf, *cursor;
	u32 f;
...
#if DVLOR(1, 0)
static void
a_core_nyd_printone(void (*ptf)(up cookie, char const *buf, uz blen), up cookie, struct su__nyd_info const *nip){
	char buf[80 +1], c;
	union {int i; uz z;} u;
	char const *sep, *cp;
	...
	u.i = snprintf(buf, sizeof(buf) - 1, "%c[%2" PRIu32 "] %.25s (%s%.40s:%" PRIu32 ")\n",
			a_core_nyd_desc[(nip->ni_chirp_line >> su__NYD_ACTION_SHIFT) & su__NYD_ACTION_MASK],
			nip->ni_level, nip->ni_fun, sep, cp, (nip->ni_chirp_line & su__NYD_ACTION_SHIFT_MASK));
	if(u.i > 0){
		u.z = S(uz,u.i);
		if(u.z >= sizeof(buf) -1){
			buf[sizeof(buf) - 2] = '\n';
...
su_state_gut(BITENUM(u32,su_state_gut_flags) flags){
	...
	if(!(flags & su_STATE_GUT_NO_LOCKS) && (flags & su_STATE_GUT_ACT_MASK) == su_STATE_GUT_ACT_NORM){
#ifdef su_USECASE_SU
# if !su_ATOMIC_IS_REAL
...
# endif
		...
		DVL( su_CC_MEM_ZERO(&a_core_thread_main_nyd, sizeof(a_core_thread_main_nyd)); )
	}
...
void
su_nyd_chirp(enum su_nyd_action act, char const *file, u32 line, char const *fun){
	struct su__nyd_control *ncp;
	...
	if(!ncp->nc_skip){
		...
		nip->ni_file = file;
		nip->ni_fun = fun;
		nip->ni_chirp_line = (S(u32,act & su__NYD_ACTION_MASK) << su__NYD_ACTION_SHIFT
				) | (line & su__NYD_ACTION_SHIFT_MASK);
		nip->ni_level = ((act == su_NYD_ACTION_ANYWHERE) ? ncp->nc_level
				: ((act == su_NYD_ACTION_ENTER) ? ++ncp->nc_level : ncp->nc_level--));
	}
