...
/* Reuse our sufficiently spaced match buffer for interaction with underlaying engine: either pass it directly on equal
 * size of match structure, otherwise pass it at &[>base] and copy result backwards */
#define a_RE_MATCH_SIZE ALIGN_Z_OVER(MAX(sizeof(struct su_re_match), sizeof(regmatch_t)))
...
#define a_X(E,S) case E: rv = S; break;
	...
	a_X(su_RE_ERROR_BADRPT, "Invalid use of repetition (like using \"*\" as first character)")
...
#undef a_X
	...
	if((self->re_super = su_ALLOCATE(i, 1, su_MEM_ALLOC_MAYFAIL)) != NIL){
		self->re_match = S(struct su_re_match*,S(void*, &S(u8*,self->re_super)[ALIGN_Z_OVER(sizeof(regex_t))]));
...
su_re_eval_cp(struct su_re *self, char const *input, BITENUM(u8,su_re_eval_flags) flags){
	...
	if((rv = (regexec(S(regex_t*,self->re_super), input,
				((self->re_setup_flags & su_RE_SETUP_TEST_ONLY) ? 0 : self->re_group_count + 1),
			remtp, ef) == 0))){
		...
		if(!direct_use && !(self->re_setup_flags & su_RE_SETUP_TEST_ONLY)){
			...
			for(i = self->re_group_count, remp = self->re_match;; ++remp, ++remtp){
