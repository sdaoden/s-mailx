...
su_time_msleep(uz millis, boole ignint){
	...
	/* C99 */{
		...
		ts.ts_nano = S(sz,millis %= su_TIMESPEC_SEC_MILLIS) * (su_TIMESPEC_SEC_NANOS / su_TIMESPEC_SEC_MILLIS);
		...
		for(;;){
			...
			rv = S(uz,(tsp->ts_sec * su_TIMESPEC_SEC_MILLIS) +
					(tsp->ts_nano / (su_TIMESPEC_SEC_NANOS / su_TIMESPEC_SEC_MILLIS)));
			break;
		}
	}
...
su_time_nsleep(struct su_timespec const *dur, struct su_timespec *rem_or_nil){
	...
	if(dur->ts_sec < 0 || dur->ts_nano < 0 || dur->ts_nano >= su_TIMESPEC_SEC_NANOS){
		rv = su_ERR_INVAL;
		goto jleave;
	}
...
# endif
		...
		millis += S(uz,dur->ts_nano) / (su_TIMESPEC_SEC_NANOS / su_TIMESPEC_SEC_MILLIS);

		if((millis = su_time_msleep(millis, FAL0)) == 0)
			rv = su_ERR_NONE;
		else{
			if(rem_or_nil != NIL){
