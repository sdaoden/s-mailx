...
a__cs_dict(u16 addflags){
   ...
   for(cs_dict<char*,TRU1>::view cdov(cdo); cdov; ++u32, ++cdov){
      ...
      if(!cdu.has_key(cdov.key()))
         a_ERR();
      else if((cp = ienc::convert_up(buf, R(NSPC(su)up,cdu.lookup(cdov.key())))
            ) == NIL)
         a_ERR();
...
a_icodec(void){
   ...
   if(cs::cmp(ccp, "2952715949"))
      a_ERR();
   if((idec::convert_u32(&u32, ccp, max::uz, 0, &ccp) & (idec::state_emask |
         idec::state_consumed)) != idec::state_consumed)
      a_ERR();
   if(*ccp != '\0')
      a_ERR();
   if(u32 != 0xAFFEDEADu)
   ...
   if((ccp = ienc::convert_u32(buf, u32, 0x10)) == NIL)
      ...
      a_ERR();
   else if((idec::convert_u32(&u32, ccp, max::uz, 0, &ccp
   ...
   if((ccp = ienc::convert_u64(buf, u64)) == NIL)
      ...
      a_ERR();
   else if((idec::convert_u64(&u64, ccp, max::uz, 0, &ccp
            ) & (idec::state_emask | idec::state_consumed)
         ) != idec::state_consumed)
      a_ERR();
   else if(*ccp != '\0')
   ...
   if((idec::convert_u64(&u64, ccp, max::uz, 0, &ccp
            ) & (idec::state_emask | idec::state_consumed)
   ...
   s64 s64;
   ccp = "999999999999999999999999999999999999999999999";
   if((idec::convert_s64(&s64, ccp, max::uz, 0, &ccp
            ) & (idec::state_emask | idec::state_consumed)
         ) != (idec::state_eoverflow | idec::state_consumed))
      a_ERR();
   else if(s64 != max::s64)
   ...
   ccp = "999999999999999999999999999999999999999999999";
   if((idec::convert_u32(&u32, ccp, max::uz, 0, &ccp
            ) & (idec::state_emask | idec::state_consumed)
         ) != (idec::state_eoverflow | idec::state_consumed))
      a_ERR();
   else if(u32 != max::u32)
   ...
   s32 s32;
   ccp = "999999999999999999999999999999999999999999999";
   if((idec::convert_s32(&s32, ccp, max::uz, 0, &ccp
            ) & (idec::state_emask | idec::state_consumed)
         ) != (idec::state_eoverflow | idec::state_consumed))
      a_ERR();
   else if(s32 != max::s32)
   ...
   if(((m = idec::convert(&s64, ccp, max::uz, 0,
            (idec::mode_signed_type),
            &ccp)) & (idec::state_emask | idec::state_consumed)
         ) != (idec::state_none))
      a_ERR();
   else if(s64 != max::s64)
      a_ERR();
      ...
      a_ERR();
   ccp = "0x7FFFFFFF ";
   if(((m = idec::convert(&s32, ccp, max::uz, 0,
            (idec::mode_signed_type | idec::mode_limit_32bit),
            &ccp)) & (idec::state_emask | idec::state_consumed)
         ) != (idec::state_none))
      a_ERR();
   else if(s32 != max::s32)
...
a_md(void){
   ...
   a__md_test<siphash::key_size, siphash::digest_size_64,
      NELEM(a_siphash_t64), struct a_siphash_t64
      >("siphash", md::new_by_algo(md::algo_siphash), &a_siphash_t64[0]);

   a__md_test<siphash::key_size, siphash::digest_size_128,
      NELEM(a_siphash_t128), struct a_siphash_t128
...
a__md_siphash(void){ // {{{
   for(uz idx = 0; idx < NELEM(a_siphash_t64); ++idx){
      u8 key[siphash::key_size], digest[siphash::digest_size_max],
         in[NELEM(a_siphash_t64)];
      siphash sh;
...
a__md_test(char const *name, md *mdp, TDAT const *tdat){ // {{{
   ...
   if(mdp->property(md::prop_key_size_min) > KSZ ||
         mdp->property(md::prop_key_size_max) < KSZ){
      a_ERR();
      goto jleave;
   }

   if(mdp->property(md::prop_digest_size_min) > DGSTSZ ||
         mdp->property(md::prop_digest_size_max) < DGSTSZ){
      a_ERR();
      goto jleave;
   }
