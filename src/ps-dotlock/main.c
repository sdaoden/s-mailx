...
main(int argc, char **argv){
	...
	if(argc != 12 ||
			...
			fstat(STDIN_FILENO, &st) == -1 || !S_ISFIFO(st.st_mode) ||
			fstat(STDOUT_FILENO, &st) == -1 || !S_ISFIFO(st.st_mode)){
jeuse:
		fprintf(stderr,
	...
	/* C99 */{
		...
		if(i == 0 || strncmp(fdi.fdi_file_name, fdi.fdi_lock_name, i) ||
				fdi.fdi_lock_name[i] == '\0' || strcmp(fdi.fdi_lock_name + i, ".lock"))
			goto jeuse;
	}
	...
	if(stat(fdi.fdi_file_name, &st) == -1)
		goto jmsg;
	fdi.fdi_uid = S(uz,st.st_uid);
	fdi.fdi_gid = S(uz,st.st_gid);
