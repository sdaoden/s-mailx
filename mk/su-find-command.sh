...
fc__pathsrch() { # pname=$1 exec=$2 varname=$3 verbok=$4
	...
	if [ "${fcps__exec}" != "${fcps__exec#* }" ]; then
		[ -n "${VERBOSE}" ] && [ ${fcps__verbok} -ne 0 ] &&
			msg ' . ${%s} ... %s (has spaces, CANNOT be found)' "${fcps__pname}" "${fcps__exec}"
		return 1
	fi

	# It may be an absolute path, check that first
	if [ "${fcps__exec}" != "${fcps__exec#/}" ] && [ -f "${fcps__exec}" ] && [ -x "${fcps__exec}" ]; then
		[ -n "${VERBOSE}" ] && [ ${fcps__verbok} -ne 0 ] &&
			msg ' . ${%s} ... %s' "${fcps__pname}" "${fcps__exec}"
		[ -n "${fcps__varname}" ] && eval "${fcps__varname}"="${fcps__exec}"
		return 0
	fi
	...
	do
		...
		if [ -f "${fcps__path}/${fcps__exec}" ] && [ -x "${fcps__path}/${fcps__exec}" ]; then
			[ -n "${VERBOSE}" ] && [ ${fcps__verbok} -ne 0 ] &&
				msg ' . ${%s} ... %s' "${fcps__pname}" "${fcps__path}/${fcps__exec}"
			[ -n "${fcps__varname}" ] && eval "${fcps__varname}"="${fcps__path}/${fcps__exec}"
			return 0
		fi
	done
	return 1
...
fc__acmd() {
	fca__pname=${1} fca__dotest=${2} fca__dofail=${3} fca__verbok=${4} fca__varname=${5}
	...
	if [ "${fca__dotest}" -ne 0 ]; then
		...
		if [ -n "${fca__dotest}" ]; then
			...
			if fc__pathsrch "${fca__pname}" "${fca__dotest}" "${fca__varname}" ${fca__verbok}; then
				return 0
			fi
			msg 'WARN: ignoring non-executable ${%s}=%s' "${fca__pname}" "${fca__dotest}"
		fi
	fi

	if fc__pathsrch "${fca__pname}" "${fca__pname}" "${fca__varname}" ${fca__verbok}; then
		return 0
	fi
