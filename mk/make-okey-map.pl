...
sub hash_em{
	die "hash_em: open: $^E" unless my $pid = open2 *RFD, *WFD, $MAILX;
	foreach my $e (@ENTS){
...
sub dump_map{ # {{{
	...
	print F '/*@ ', scalar basen($OUT), ', generated by ', scalar basen($0),
		"; see include/mx/okeys.h for more. */\n\n";
...
_EOT
	...
	if(%chains){
		...
		print F 'CTAV(4 == FIELD_SIZEOF(struct a_amv_var_chain_map_bsrch, avcmb_prefix));', "\n";
	...
	print F '/* Unfortunately init of varsized buffer impossible: define "subclass"es */', "\n";
	...
	foreach(@skeys){
		...
		my $fa = join '|', @{$e->{flags}};
		$f .= '|' . $fa if length $fa;
		print F "} const a_amv_$e->{vstruct} = {NIL, a_amv_$e->{vstruct}_val, a_X(0 su_COMMA) $f, \"$e->{name}\"};\n\n"
	}
	...
	foreach(@skeys){
		my $e = $i3vals{$_};
		print F "${S}{", $e->{enum}, ', {0,}, ', (!$e->{bool} ? $e->{i3val} : "NIL"), "},\n"
	}
	print F "};\n";
...
sub dump_test_adapts{ # {{{
	die "$TADA: open: $^E" unless open F, '>', $TADA;
	print F '#@ ', scalar basen($TADA), ', generated by ', scalar basen($0), "; see mx/okeys.h for more.\n\n";

	print F "OKEY_BLTIN_ADAS=''\\\n";
	foreach my $e (@ENTS){
		if($e->{norc}){
			printf F "'-Sno$e->{name} '\\\n"
		}elsif($e->{rc}){
			printf F "'-S$e->{name}=$e->{rc} '\\\n"
		}
	}
	print F "\n";

	die "$TADA: close: $^E" unless close F
} # }}}
