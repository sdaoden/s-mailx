...
sub main_fun{
	...
	while(@ARGV > 0){
		...
		die "Cannot open $f for reading: $^E" unless open $fd, '<:raw', $f;
		while(<$fd>){
			chomp;
			if($i){
				if($_ =~ /^(.*?)\*\/[[:space:]]*(.*)$/){
		...
		die "Cannot close $f after reading: $^E" unless close $fd;

		die "Cannot open $f for writing: $^E" unless open $fd, '>:raw', $f;
		while(@lines){
			$i = shift @lines;
			die "Cannot write to $f: $^E" unless print $fd $i, "\n"
		}
		die "Cannot close $f after writing: $^E" unless close $fd
	}
	exit 0
}
