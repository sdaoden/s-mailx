...
_cc_flags_generic() {
	...
	if cc_check -fstrict-overflow; then # && feat_yes DEVEL; then
...
# Include $rc, but only take from it what was not overwritten by the user from
...
if run_check ftruncate 'ftruncate(2)' '#define mx_HAVE_FTRUNCATE' << \!
#include <unistd.h>
#include <sys/types.h>
# include <errno.h>
int main(void){
...
if feat_yes DOTLOCK; then
	if run_check prctl_dumpable 'prctl(2) + PR_SET_DUMPABLE' '#define mx_HAVE_PRCTL_DUMPABLE' << \!
#include <sys/prctl.h>
# include <errno.h>
int main(void){
...
!
	then
		:
	elif run_check procctl_trace_ctl 'procctl(2) + PROC_TRACE_CTL_DISABLE' '#define mx_HAVE_PROCCTL_TRACE_CTL' << \!
...
!
	then
		:
	elif run_check prtrace_deny 'ptrace(2) + PT_DENY_ATTACH' '#define mx_HAVE_PTRACE_DENY' << \!
...
!
	then
		:
	elif run_check setpflags_protect 'setpflags(2) + __PROC_PROTECT' '#define mx_HAVE_SETPFLAGS_PROTECT' << \!
#include <priv.h>
# include <errno.h>
int main(void){
...
OPT_FLOCK=1
if link_check flock_lock 'flock(2) file locking (via sys/file.h)' '#define mx_HAVE_FLOCK <sys/file.h>' << \!
#include <sys/file.h>
#include <unistd.h>
int main(void){
...
then
	:
elif link_check flock_lock 'flock(2) file locking (via fcntl.h)' '#define mx_HAVE_FLOCK <fcntl.h>' << \!
#include <fcntl.h>
#include <unistd.h>
int main(void){
	flock(3, LOCK_SH | LOCK_NB);
...
int main(void){
	...
	sol = sizeof soe;
	if(getsockopt(sockfd, SOL_SOCKET, SO_ERROR, &soe, &sol) == -1 && errno == ENOSYS)
		return 1;
	if(setsockopt(sockfd, SOL_SOCKET, SO_KEEPALIVE, (void*)0, 0) == -1 && errno == ENOSYS)
		return 1;
	return 0;
}
!
...
feat_yes NET &&
	link_check nonblocksock 'non-blocking sockets' '#define mx_HAVE_NONBLOCKSOCK' << \!
#include <sys/types.h>
...
if feat_yes NET; then
	link_check getaddrinfo 'getaddrinfo(3)' '#define mx_HAVE_GETADDRINFO' << \!
#include <sys/types.h>
#include <sys/socket.h>
#include <stdio.h>
#include <netdb.h>
...
!
	...
	val_idna_idnkit() {
		link_check idna 'OPT_IDNA->VAL_IDNA: idnkit' '#define mx_HAVE_IDNA n_IDNA_IMPL_IDNKIT' '-lidnkit' << \!
#include <stdio.h>
#include <idn/api.h>
...
int main(void){
	...
	r = idn_encodename(IDN_ENCODE_APP, "does.this.work", ace_name, sizeof(ace_name));
	if (r != idn_success) {
