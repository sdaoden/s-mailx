...
synopsis() {
	...
	echo >&${f} "mdocmx ($VERSION): reference extension mdoc manual preprocessor"
	echo >&${f}
	echo >&${f} "  ${p} -h"
	echo >&${f} "  ${p} [:-v:] [-t | -T Sh|sh|Ss|ss  [-c]] [FILE]"
	echo >&${f}
	echo >&${f} ". Bugs/Contact via $CONTACT."
	exit ${ex}
}
...
function mx_check_line(){
	...
	for(arg_parse(-1); arg_parse(0);){
		...
		dbg("POP mac<" mcl_mac "> " mcl_i " key <" ARG "> stack size=" mx_stack_cnt)
