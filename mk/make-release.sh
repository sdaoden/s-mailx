...
: ${MAILX:='s-nail -Snofollowup-to -Sreply-to=mailx -a ~/src/www.git/steffen.asc -Ssmime-sign'}
: ${ACCOUNT:=ich}
...
current_version() {
	VERSION=$(VERSION= TOPDIR= awk=$awk grep=$grep sed=$sed cmp=$cmp mv=$mv $SHELL mk/make-version.sh create)
}
...
update_stable_hook() {
	...
	TOPDIR= awk=$awk grep=$grep sed=$sed cmp=$cmp mv=$mv $SHELL mk/make-version.sh create
	...
	#
	echo 'mx-config.7: integrating mx-config.7.in'
	TOPDIR= awk=$awk $SHELL mk/make-man-config.sh mx-config.7 mx-config.7x
	$mv -f mx-config.7x mx-config.7
	echo 'mx-config.7: expanding MKREL'
	< mx-config.7 > mx-config.7x $awk '
		BEGIN { written = 0 }
		/\.\\"--MKREL-START--/, /\.\\"--MKREL-END--/ {
			if (written++ != 0)
				next
	...
	if [ -n "$have_perl" ]; then
		$make d-okeys && $git add -f src/mx/gen-okeys.h gen-test-ada.sh
...
update_release_hook() {
	...
	$sed -e '/^\.\\"--MKREL-START--/d' \
		...
		-e '}' \
		-e '/^\.$/d' \
		< mx-config.7 > mx-config.7x
	$mv -f mx-config.7x mx-config.7

	$mv -f mx-config.7x mx-config.7
	$SHELL mk/mdocmx.sh < mx-config.7 > mx-config.7x
	$mv -f mx-config.7x mx-config.7
	$git add mx-config.7
	# And generate the HTML manual, while here
	if [ -z "$grappa" ] && command -v $roff >/dev/null 2>&1; then
		echo 'mx-config.7: creating HTML manual'
		< mx-config.7 MDOCMX_ENABLE=1 $roff -Thtml -mdoc > /tmp/mx-config-manual.html
	fi
	...
	if [ -n "$have_perl" ]; then
		$make d-okeys-nv && $git add -f src/mx/gen-okeys.h gen-test-ada.sh
	...
	$git rm -f .gitignore .mailmap TODO \
		\
		mk/make-news-anchors.sh mk/make-man-config.sh mk/make-release.* \
		\
		mk/mdocmx.sh \
		\
